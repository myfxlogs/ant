...
COMMON_FUNC_MAP: Dict[str, str] = {
    ...
    # ── Array functions ──────────────────────────────────────────────
    "ArraySize": "len",
    "ArrayInitialize": "TRANSPILER-GAP: ArrayInitialize — use list comprehension: arr = [val]*len(arr)",
    "ArrayResize": "TRANSPILER-GAP: ArrayResize — use arr.extend([0]*(n-len(arr)))",
    "ArrayCopy": "TRANSPILER-GAP: ArrayCopy — use arr[:] = src[:]",
    "ArrayGetAsSeries": "TRANSPILER-GAP: ArrayGetAsSeries — Python lists are always reverse-indexed via bars[]",
    "ArraySetAsSeries": "TRANSPILER-GAP: ArraySetAsSeries — Python lists are always reverse-indexed via bars[]",
    # ── Timer / events ───────────────────────────────────────────────
    "EventSetTimer": "self.ctx.set_timer",
    "EventKillTimer": "self.ctx.kill_timer",
    # ── Time functions (MQL4 core) ───────────────────────────────────
    ...
    "Seconds": "lambda: __import__('datetime').datetime.now().second",
    "TimeCurrent": "self.broker.server_time",
    "TimeLocal": "self.broker.server_time",
    # ── Math functions (MQL4 core) ───────────────────────────────────
    "NormalizeDouble": "round",
    # ── String conversion functions ──────────────────────────────────
    "DoubleToStr": "lambda d, digits: str(round(float(d), digits))",
    "TimeToStr": "TRANSPILER-GAP: TimeToStr — use datetime.utcfromtimestamp(t).strftime(fmt)",
    "StrToTime": "TRANSPILER-GAP: StrToTime — use int(datetime.strptime(s, fmt).timestamp())",
    "IntegerToString": "str",
    "StringToInteger": "int",
    # ── Order/info helper ────────────────────────────────────────────
    "OrderPrint": "TRANSPILER-GAP: OrderPrint — use self.ctx.log()",
    ...
    # PositionSelect + loop → for pos in self.broker.positions() (handled by transpiler)
    "PositionSelect": "TRANSPILER-GAP: PositionSelect — use for pos in self.broker.positions()",
    "PositionSelectByTicket": "TRANSPILER-GAP: PositionSelectByTicket",
    # PositionGet* → MQL5 stateful pattern. SDK: pos.attr directly.
    "PositionGetDouble": "TRANSPILER-GAP: PositionGetDouble — use pos.open_price",
    "PositionGetInteger": "TRANSPILER-GAP: PositionGetInteger — use pos.ticket",
    "PositionGetString": "TRANSPILER-GAP: PositionGetString — use pos.symbol",
    ...
    # ── MQL5 History functions ───────────────────────────────────────
    # HistorySelect + loop → for deal in self.broker.deals(from, to) (handled by transpiler)
    "HistorySelect": "TRANSPILER-GAP: HistorySelect — use self.broker.deals(from_ms, to_ms)",
    "HistorySelectByPosition": "TRANSPILER-GAP: HistorySelectByPosition",
    "HistoryDealsTotal": "len(self.broker.deals())",
    "HistoryOrdersTotal": "len(self.broker.history_orders())",
    "HistoryDealSelect": "TRANSPILER-GAP: HistoryDealSelect — use deal iteration",
    # HistoryDealGet*/HistoryOrderGet* → MQL5 stateful. SDK: deal.attr / order.attr.
    "HistoryDealGetTicket": "TRANSPILER-GAP: HistoryDealGetTicket — use deal.ticket",
    "HistoryDealGetDouble": "TRANSPILER-GAP: HistoryDealGetDouble — use deal.price",
    "HistoryDealGetInteger": "TRANSPILER-GAP: HistoryDealGetInteger — use deal.ticket",
    "HistoryDealGetString": "TRANSPILER-GAP: HistoryDealGetString — use deal.symbol",
    "HistoryOrderGetTicket": "TRANSPILER-GAP: HistoryOrderGetTicket — use order.ticket",
    "HistoryOrderGetDouble": "TRANSPILER-GAP: HistoryOrderGetDouble — use order.price",
    "HistoryOrderGetInteger": "TRANSPILER-GAP: HistoryOrderGetInteger — use order.ticket",
    "HistoryOrderGetString": "TRANSPILER-GAP: HistoryOrderGetString — use order.symbol",
    # ── MT5 trade support (not available in SDK) ─────────────────────
    "OrderSendAsync": "TRANSPILER-GAP: OrderSendAsync — use self.broker.order_send (sync)",
    "OrderCalcMargin": "TRANSPILER-GAP: OrderCalcMargin — broker-side, not available",
    "OrderCalcProfit": "TRANSPILER-GAP: OrderCalcProfit — broker-side, not available",
    "OrderCheck": "TRANSPILER-GAP: OrderCheck — broker-side, not available",
    # OrderGet* → MQL5 stateful. SDK: order.attr directly.
    "OrderGetDouble": "TRANSPILER-GAP: OrderGetDouble — use order.open_price",
    "OrderGetInteger": "TRANSPILER-GAP: OrderGetInteger — use order.ticket",
    "OrderGetString": "TRANSPILER-GAP: OrderGetString — use order.symbol",
    # ── SymbolInfo getters → symbol_info().attr ──────────────────────
    "SymbolInfoDouble": "TRANSPILER-GAP: SymbolInfoDouble — use self.broker.symbol_info(sym).attr",
    "SymbolInfoInteger": "TRANSPILER-GAP: SymbolInfoInteger — use self.broker.symbol_info(sym).attr",
    "SymbolInfoString": "TRANSPILER-GAP: SymbolInfoString — use self.broker.symbol_info(sym).attr",
