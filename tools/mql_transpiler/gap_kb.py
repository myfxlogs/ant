...
KB_PATH = os.path.join(os.path.dirname(__file__), "kb", "unmapped_patterns.py")
...
def _load_kb() -> dict:
    """Load KB from a Python data file (not JSON — project constraint)."""
    if not os.path.exists(KB_PATH):
        return {}
    try:
        with open(KB_PATH) as f:
            data = f.read()
        # Evaluate as Python literal (safe — only dicts/lists/str/int/float).
        import ast as _ast
        return _ast.literal_eval(data) if data.strip() else {}
    except Exception:
        return {}


def _save_kb(kb: dict) -> None:
    """Save KB as a Python data file (not JSON)."""
    with open(KB_PATH, "w") as f:
        f.write("# MQL Transpiler Knowledge Base — Python data format\n")
        f.write("# auto-generated by gap_kb.py\n")
        f.write(repr(kb))
        f.write("\n")
