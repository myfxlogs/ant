...
// Colors/booleans are merged from mqlConstantsColors (constants_colors.go)
// at var-init time to ensure api_registry.go's init() sees the full set.
var MQLConstants = func() map[string]Value {
	m := map[string]Value{
	...
	// VM-CODE-HYGIENE-1: merge color/boolean constants from constants_colors.go.
	for k, v := range mqlConstantsColors {
		m[k] = v
	}
	return m
}()
