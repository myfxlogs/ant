...
// ── Annealed Gaussian Search ──

type AnnealedGaussianOptimizer struct {
	space    ResolvedSpace
	dims     int
	maxEvals int
	gamma    float64
...
func NewAnnealedGaussianOptimizer(space ResolvedSpace, maxEvals int) *AnnealedGaussianOptimizer {
	return &AnnealedGaussianOptimizer{
...
func (tp *AnnealedGaussianOptimizer) Ask(batchSize int) [][]int {
...
func (tp *AnnealedGaussianOptimizer) Tell(results []OptimizerResult) {
...
func (tp *AnnealedGaussianOptimizer) Best() ([]int, float64) {
	if tp.bestIdx < len(tp.history) { return tp.history[tp.bestIdx], tp.bestScore }
	return nil, 0
}

func (tp *AnnealedGaussianOptimizer) Evaluations() int { return tp.evalsUsed }
func (tp *AnnealedGaussianOptimizer) MaxEvals() int    { return tp.maxEvals }
func (tp *AnnealedGaussianOptimizer) Done() bool       { return tp.evalsUsed >= tp.maxEvals }

func (tp *AnnealedGaussianOptimizer) tpeSample() []int {
...
func (tp *AnnealedGaussianOptimizer) randomVector() []int {
	vec := make([]int, tp.dims)
