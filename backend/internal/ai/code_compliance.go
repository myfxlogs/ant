...
// complianceRules defines all 13 forbidden-code rules.
var complianceRules = []ComplianceRule{
	...
	{Name: "no_file_open", Pattern: `\bopen\s*\([^)]*['"][rwa]`, Message: "禁止文件 I/O (open)", Severity: "block"},
...
// HasRequiredSignature checks that the code has the required initialize + on_bar functions.
func (s *CodeComplianceScanner) HasRequiredSignature(code string) (bool, []string) {
	var missing []string
	if !regexp.MustCompile(`def\s+initialize\s*\(`).MatchString(code) {
		missing = append(missing, "缺少 initialize(state) 函数")
	}
	if !regexp.MustCompile(`def\s+on_bar\s*\(`).MatchString(code) {
		missing = append(missing, "缺少 on_bar(state, bar, history) 函数")
	}
