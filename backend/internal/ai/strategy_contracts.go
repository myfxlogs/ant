...
// contextContractText returns the event-driven run(context) contract + MANDATORY safety rules.
// Safety rules are placed FIRST and use strong language to prevent the LLM from ignoring them.
func contextContractText() string {
	var sb strings.Builder

	sb.WriteString("## ⛔ 代码生成铁律 — 违反任何一条输出都会被拒绝 ⛔\n\n")

	sb.WriteString("### 铁律 1：每个 @param / @strategy 必须从 context 读取，禁止硬编码\n")
	sb.WriteString("```python\n")
	sb.WriteString("# @param fast_period 20 range=10:50:10\n")
	sb.WriteString("# @param slow_period 50 range=30:80:10\n")
	sb.WriteString("# @param vol_bias_open 1.2 range=1.0:1.8:0.1\n")
	...
	sb.WriteString("    # ✅ 每个 @param 和 @strategy 都必须从 context 读取\n")
	sb.WriteString("    fast_period    = int(context.get('fast_period', 20))\n")
	sb.WriteString("    slow_period    = int(context.get('slow_period', 50))\n")
	sb.WriteString("    vol_bias_open  = float(context.get('vol_bias_open', 1.2))\n")
	sb.WriteString("    vol_bias_close = float(context.get('vol_bias_close', 0.9))\n")
	sb.WriteString("    entry_pct = float(context.get('entryPct', 0.25))\n")
	sb.WriteString("    sl_pct    = float(context.get('stopLossPct', 0.02))\n")
	sb.WriteString("    tp_pct    = float(context.get('takeProfitPct', 0.04))\n")
	sb.WriteString("\n")
	sb.WriteString("    # ❌ 绝对禁止：硬编码值来匹配 @param 声明\n")
	sb.WriteString("    # ema20 = calc_ema(close, 20)        ← 20 必须用 fast_period\n")
	sb.WriteString("    # ema50 = calc_ema(close, 50)        ← 50 必须用 slow_period\n")
	sb.WriteString("    # open_long = vol_ratio > 1.2        ← 1.2 必须用 vol_bias_open\n")
	sb.WriteString("```\n\n")

	sb.WriteString("### 铁律 1.5：position dict 的 key 名 — 必须用引擎注入的字段，禁止编造\n")
	sb.WriteString("引擎注入的 `context['position']` 是一个 dict（或 None），字段如下：\n")
	sb.WriteString("```python\n")
	sb.WriteString("position = context.get('position')  # None 表示无持仓\n")
	sb.WriteString("if position is not None:\n")
	sb.WriteString("    # ✅ 正确的字段名：\n")
	sb.WriteString("    pos_side   = position['side']        # 'buy'=long, 'sell'=short\n")
	sb.WriteString("    pos_volume = position['volume']      # 手数\n")
	sb.WriteString("    pos_price  = position['open_price']  # 开仓价格\n")
	sb.WriteString("    pos_sl     = position.get('sl', 0)   # 止损价（可能为 0）\n")
	sb.WriteString("    pos_tp     = position.get('tp', 0)   # 止盈价（可能为 0）\n")
	sb.WriteString("    pos_ticket = position.get('ticket', 0)  # 唯一 ticket ID\n")
	sb.WriteString("\n")
	sb.WriteString("    # ❌ 绝对禁止：编造不存在的字段名\n")
	sb.WriteString("    # dir    = position.get('direction')   ← 没有 'direction'，用 'side'\n")
	sb.WriteString("    # typ    = position.get('type')        ← 没有 'type'，用 'side'\n")
	sb.WriteString("    # price  = position.get('price')       ← 没有 'price'，用 'open_price'\n")
	sb.WriteString("    # entry  = position.get('entry_price') ← 没有 'entry_price'，用 'open_price'\n")
	sb.WriteString("```\n\n")

	sb.WriteString("### 铁律 2：持仓时必须每根 bar 返回止损止盈，不能设为 0\n")
	sb.WriteString("```python\n")
	sb.WriteString("    if position is not None:\n")
	sb.WriteString("        entry_price = position['open_price']\n")
	sb.WriteString("        if position['side'] == 'buy':                         # ✅ 用 'side'，不是 'type' 或 'direction'\n")
	sb.WriteString("            stop_loss   = entry_price * (1 - sl_pct)          # ✅ 每根 bar 都设\n")
	sb.WriteString("            take_profit = entry_price * (1 + tp_pct)          # ✅ 每根 bar 都设\n")
	sb.WriteString("        else:\n")
	sb.WriteString("            stop_loss   = entry_price * (1 + sl_pct)\n")
	sb.WriteString("            take_profit = entry_price * (1 - tp_pct)\n")
	sb.WriteString("    # ❌ 绝对禁止：持仓时 'stop_loss': 0.0, 'take_profit': 0.0\n")
	sb.WriteString("```\n\n")

	sb.WriteString("### 铁律 3：仓位大小用 initial_balance，不能用当前 balance\n")
	sb.WriteString("```python\n")
	sb.WriteString("    initial_balance = float(context.get('initial_balance', 10000.0))\n")
	sb.WriteString("    if signal in ('buy', 'sell'):\n")
	sb.WriteString("        volume_ordered = (initial_balance * entry_pct) / close[-1]\n")
	sb.WriteString("    # ❌ 绝对禁止：volume = (context.get('balance') * entry_pct) / close[-1]\n")
	sb.WriteString("```\n\n")

	sb.WriteString("### 铁律 4：变量必须先定义，再使用\n")
	sb.WriteString("```python\n")
	sb.WriteString("def run(context):\n")
	sb.WriteString("    # ✅ 所有变量在函数开头一次性定义（在任何 return 之前）\n")
	sb.WriteString("    fast_period = int(context.get('fast_period', 20))\n")
	sb.WriteString("    entry_pct   = float(context.get('entryPct', 0.25))\n")
	sb.WriteString("    initial_balance = float(context.get('initial_balance', 10000.0))\n")
	sb.WriteString("    close = context['close']\n")
	sb.WriteString("    # ... 然后才是策略逻辑\n")
	sb.WriteString("```\n\n")

	sb.WriteString("### 铁律 5：函数签名与返回值\n")
	sb.WriteString("```python\n")
	sb.WriteString("def run(context):\n")
	sb.WriteString("    # context 键: open, high, low, close, volume (列表)\n")
	sb.WriteString("    #             position (dict or None), balance (float)\n")
	sb.WriteString("    return {\n")
	sb.WriteString("        'signal': 'buy',       # 'buy' | 'sell' | 'hold'\n")
	sb.WriteString("        'volume': 1.0,        # 手数\n")
	sb.WriteString("        'stop_loss': 0.0,     # 止损价格（持仓时禁止为 0）\n")
	sb.WriteString("        'take_profit': 0.0,   # 止盈价格（持仓时禁止为 0）\n")
	sb.WriteString("    }\n")
	sb.WriteString("```\n\n")

	sb.WriteString("### 铁律 6：沙箱限制\n")
