...
func strategyContractText() string {
	...
	sb.WriteString("| 预注入名称 | 说明 |\n")
	sb.WriteString("|-----------|------|\n")
	sb.WriteString("| `np` | NumPy（np.mean, np.array, np.where, np.maximum 等）|\n")
	sb.WriteString("| `math` | Python math 模块（math.sqrt, math.log, math.exp 等）|\n")
	sb.WriteString("| `sum, min, max, len, abs, pow, round` | 内置函数 |\n")
	sb.WriteString("| `range, zip, enumerate, sorted, filter, map` | 迭代工具 |\n")
	sb.WriteString("| `int, float, bool, str, list, dict, tuple, set` | 类型构造器 |\n")
	sb.WriteString("| `any, all, isinstance, reversed` | 内置辅助 |\n\n")
	sb.WriteString("## 禁止事项\n")
	sb.WriteString("1. **禁止任何 import/from 语句** — 沙箱 AST 检查拒绝所有 import\n")
	sb.WriteString("2. 不使用 eval()、exec()、compile()、open()、globals()、locals()\n")
	sb.WriteString("3. 不使用 dunder 属性（__开头）\n")
	sb.WriteString("4. 不使用 global/nonlocal 语句\n")
	sb.WriteString("5. np 和 math 已预注入，直接使用，无须 import\n\n")
	return sb.String()
}
