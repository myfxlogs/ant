...
func postConnectSetup(ctx context.Context, cfg mdtick.AccountConfig, gw Gateway, deps RunnerDeps, mgr *Manager, log *zap.Logger) error {
	...
	// Demand-driven: gateway starts recvLoop but does not pre-subscribe symbols.
	// Strategies add symbols on-demand via mtHub.SubscribeSymbols.
	if err := gw.Subscribe(ctx, nil, mgr.HandleTick); err != nil {
