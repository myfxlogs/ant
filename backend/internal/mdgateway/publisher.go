...
func (p *Publisher) PublishTick(ctx context.Context, t *mdtick.Tick) error {
	...
	payload, err := proto.Marshal(&antv1.TickPayload{
...
func (p *Publisher) PublishBar(ctx context.Context, b *mdtick.Bar) error {
	...
	msg := natsgo.NewMsg(subj)
	payload, err := proto.Marshal(&antv1.BarPayload{
		Broker:        b.Broker,
		Canonical:      b.Canonical,
		Period:         b.Period,
		OpenTsUnixMs:  b.OpenTsUnixMs,
		CloseTsUnixMs: b.CloseTsUnixMs,
		Open:           b.Open.String(),
		High:           b.High.String(),
		Low:            b.Low.String(),
	...
	if err != nil {
		return fmt.Errorf("marshal bar payload: %w", err)
	}
	msg.Data = payload
...
func (p *Publisher) PublishBarRevision(ctx context.Context, b *mdtick.Bar) error {
	...
	if err != nil {
		return fmt.Errorf("marshal bar revision payload: %w", err)
	}
	msg.Data = payload
