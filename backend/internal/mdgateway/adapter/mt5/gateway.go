...
func New(cfg mdtick.AccountConfig, normalizer *mdtick.Normalizer, log *zap.Logger, client *mt5client.MT5Client) *Gateway {
	return &Gateway{
		cfg:        cfg,
...
func (g *Gateway) Connect(ctx context.Context) error {
	if g.client == nil {
		return fmt.Errorf("mt5: client not configured")
	}
	g.mu.Lock()
	defer g.mu.Unlock()
...
func (g *Gateway) Subscribe(ctx context.Context, symbols []string, handler mdtick.TickHandler) error {
	...
	for _, sym := range symbols {
		// MT5 Subscribe takes an extra interval parameter (MT4 does not).
...
// MT5 uses pb.Quote; MT4 uses pb.QuoteEventArgs.
func (g *Gateway) normalize(q *pb.Quote, userID string, normalizer *mdtick.Normalizer) *mdtick.Tick {
	if normalizer != nil {
