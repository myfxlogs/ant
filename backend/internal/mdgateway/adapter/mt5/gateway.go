// Package mt5 — MT5 gateway adapter (mtapi gRPC direct, ADR-0003).
// Connects via pb.MT5Client and pb.ConnectionClient.
package mt5
...
func (g *Gateway) Connect(ctx context.Context) error {
	...
	if err != nil { return fmt.Errorf("mt5: dial %s: %w", addr, err) }
	...
	g.mu.Unlock()
	g.log.Info("mt5: connected", zap.String("account", g.cfg.AccountID))
	return nil
}
...
func (g *Gateway) Disconnect(ctx context.Context) error {
	...
	g.client = nil; g.connCli = nil; g.streamCli = nil; g.sessionID = ""
	return nil
}

func (g *Gateway) Subscribe(ctx context.Context, symbols []string, handler mdgateway.TickHandler) error {
	g.mu.RLock(); streamCli := g.streamCli; g.mu.RUnlock()
	if streamCli == nil { return fmt.Errorf("mt5: not connected") }
	...
	go func() {
		for {
			...
			if err != nil { g.log.Warn("mt5: stream recv", zap.Error(err)); return }
			...
			handler(&mdtick.Tick{
				...
				Broker: g.cfg.Broker, Platform: "mt5",
				...
				BidVolume: float64(q.GetVolume()), AskVolume: 0,
			})
		}
	}()
	return nil
...
func (g *Gateway) HealthCheck(ctx context.Context) error {
	...
	if g.conn == nil { return fmt.Errorf("mt5: not connected") }
	return nil
}
