...
func (s *StrategyExecutionServer) dispatchCloseOrder(ctx context.Context, cfg LiveStrategyConfig, sig *antv1.StrategySignal, activeSess *ActiveSession) {
	// VM-TRADE-CONTEXT-3: CloseBy requires both tickets. The gateway does not
	// yet support atomic CloseBy, so fail-closed rather than silently degrading
	// to a single-ticket close (which would leave the opposite position open).
	if sig.GetOppositeTicket() != 0 {
		s.log.Warn("LiveStrategyRunner: CloseBy not supported by gateway — dropping (fail-closed)",
			zap.Int64("ticket", sig.GetExecutedTicket()),
			zap.Int64("opposite_ticket", sig.GetOppositeTicket()))
		if activeSess != nil {
			activeSess.RecordError("CloseBy not supported by gateway")
		}
		return
	}
