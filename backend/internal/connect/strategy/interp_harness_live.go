...
// generateInterpLiveHarness generates a Go source file (package main) that:
//   - reads serialized IR from stdin (u32 length prefix + IR bytes)
//   - reads ExecuteLiveRequest from stdin (length-prefixed proto binary, after IR)
...
func main() {
	...
	if _, err := io.ReadFull(os.Stdin, irLenBuf[:]); err != nil {
		fmt.Fprintf(os.Stderr, "interp live: read IR length: %v\n", err)
		os.Exit(1)
	}
	irLen := binary.LittleEndian.Uint32(irLenBuf[:])
	irData := make([]byte, irLen)
	if _, err := io.ReadFull(os.Stdin, irData); err != nil {
		fmt.Fprintf(os.Stderr, "interp live: read IR data: %v\n", err)
		os.Exit(1)
	}

	ir := interp.DeserializeIR(irData)
	if ir == nil {
		fmt.Fprintln(os.Stderr, "interp live: deserialize IR failed")
		os.Exit(1)
	}
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "interp live: read first request: %v\n", err)
		os.Exit(1)
	}
	...
	for {
		...
		if err := writeResponse(os.Stdout, resp); err != nil {
			fmt.Fprintf(os.Stderr, "interp live: write response: %v\n", err)
			break
		}
		req, err = readRequest(os.Stdin)
		if err != nil {
			...
			fmt.Fprintf(os.Stderr, "interp live: read request: %v\n", err)
			break
		}
	}
