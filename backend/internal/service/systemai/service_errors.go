...
func FriendlyError(err error) string {
	...
	if errors.Is(err, errBaseURLEmpty) {
		return "请先填写 Base URL（模型服务地址）。"
	}
	if s, ok := matchErrType(errType, low, msg); ok {
		return s
	}
	if s, ok := matchErrString(low, msg); ok {
		return s
	}
	return "拉取模型失败：" + msg
}

func matchErrType(errType, low, msg string) (string, bool) {
	switch {
	case errType == "insufficient_quota" || errType == "rate_limit_error" ||
		strings.Contains(low, "status 429"):
		return "配额受限或被限流：厂商已拒绝调用。请检查计费/速率限制或稍后重试。", true
	case errType == "authentication_error" || errType == "invalid_api_key" ||
		strings.Contains(low, "status 401"):
		return "鉴权失败：请检查 API Key 是否正确，或确认网关是否需要密钥。", true
	case errType == "invalid_request_error":
		if strings.Contains(low, "model") && (strings.Contains(low, "not found") || strings.Contains(low, "not exist") || strings.Contains(low, "unknown")) {
			return "模型名称不存在于当前供应商，请检查模型名称或切换到其他厂商。", true
		}
		if strings.Contains(low, "context") || strings.Contains(low, "token") || strings.Contains(low, "too long") {
			return "输入内容超出模型上下文限制，请缩短消息或清除对话历史后重试。", true
		}
		if strings.Contains(low, "stream") {
			return "当前模型不支持流式输出，正在自动切换非流式模式，请稍候重试。", true
		}
		return "请求参数有误：" + extractMessage(msg), true
	case errType == "server_error":
		return "模型服务内部错误，请稍后重试或切换到其他厂商。", true
	}
	return "", false
}

func matchErrString(low, msg string) (string, bool) {
	switch {
	case strings.Contains(low, "free-tier") || strings.Contains(low, "free tier"):
		return "免费额度已耗尽：请在厂商控制台关闭「仅使用免费档」或更换付费 Key。", true
	case strings.Contains(low, "unauthorized"):
		return "鉴权失败：请检查 API Key 是否正确，或确认网关是否需要密钥。", true
	case strings.Contains(low, "endpoint not found") || strings.Contains(low, "status 404"):
		return "模型端点不存在：请确认 Base URL 与服务协议匹配（部分服务需要 /v1）。", true
	case strings.Contains(low, "timeout"):
		return "请求超时：模型服务未在 60s 内响应，请检查网络或切换到其他厂商（如 DeepSeek）", true
	case strings.Contains(low, "unreachable"):
		return "无法连接到模型服务：请检查 Base URL、网络或网关。", true
	case strings.Contains(low, "invalid /models response") || strings.Contains(low, "cannot parse json"):
		return "模型服务返回格式不兼容 /models 协议。", true
	case strings.Contains(low, "no models"):
		return "模型服务未返回可用模型，请检查账号权限或服务配置。", true
	case strings.Contains(low, "user location is not supported"):
		return "User location is not supported for the API use. The upstream may block this region (egress IP); try a supported network, proxy, or another provider.", true
	case strings.Contains(low, "base url"):
		return "Base URL 格式无效：请填写完整地址，例如 https://api.example.com/v1。", true
	case strings.Contains(low, "api key") && (strings.Contains(low, "expired") || strings.Contains(low, "revoked") || strings.Contains(low, "suspended") || strings.Contains(low, "disabled") || strings.Contains(low, "billing") || strings.Contains(low, "forbidden")):
		return "API Key 已失效（过期/被撤销/欠费），请更新密钥或联系供应商。", true
	case strings.Contains(low, "status 400"):
		return "模型服务拒绝了请求，可能是参数不兼容或模型暂时不可用。详情：" + msg, true
	}
	return "", false
}
