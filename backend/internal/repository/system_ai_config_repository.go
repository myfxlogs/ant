...
const systemAIConfigSelectCols = `
	...
	reasoning_effort, enabled, has_secret, created_at, updated_at, updated_by
`
...
func scanSystemAIConfigRow(scanner pgx.Row) (*SystemAIConfigRow, error) {
	...
	err := scanner.Scan(
		...
		&r.ReasoningEffort, &r.Enabled, &r.HasSecret, &r.CreatedAt, &r.UpdatedAt, &r.UpdatedBy,
	)
	if err != nil {
...
func (r *SystemAIConfigRepository) Upsert(ctx context.Context, row *SystemAIConfigRow, updatedBy string) error {
	...
	_, err := r.db.Exec(ctx, `
		INSERT INTO system_ai_configs (
			user_id, provider_id, name, base_url, organization, models, default_model,
			temperature, timeout_seconds, max_tokens, purposes, primary_for, reasoning_effort,
			enabled, updated_by
	...
	`,
		...
		row.Temperature, row.TimeoutSeconds, row.MaxTokens, purposes, primaryFor, row.ReasoningEffort,
		row.Enabled, updatedBy)
	if err != nil {
