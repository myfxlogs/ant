...
func main() {
	...
	for i:=0;i<120;i++{time.Sleep(5*time.Second);var s string;pool.QueryRow(ctx,"SELECT status FROM strategy_experiments WHERE id=$1",exp.ID).Scan(&s)
		if s=="COMPLETED"||s=="FAILED"{rows,_:=pool.Query(ctx,"SELECT rank,score,grade,parameters FROM strategy_experiment_candidates WHERE experiment_id=$1 ORDER BY rank",exp.ID)
			...
			if cnt>0&&!same{fmt.Println("PASS: DE pipeline working, scores differentiated")}else{fmt.Println("FAIL: identical scores")}
			return}}
