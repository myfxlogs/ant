...
| 角色 | 职责 | 决策权 |
|------|------|--------|
| **Devin CLI** | 项目第一负责人、唯一技术决策者；设计/定稿/架构/合规/方向/审计/验收 | **最高**：技术判断、架构、合规、方向与最终质量 |
| **Devin IDE / 其他 agent** | 施工方：按 AGENTS.md + STATE.md + registry 落地 | **无**：遇设计疑问回找 Devin CLI |
...
- **常驻工作流**：每项功能/任务 → Devin CLI 完成设计 SSOT → 给施工方编号化施工提示词（S1–Sn/T1–Tn）→ 施工方只施工不做决策 → 完成后 Devin CLI 复审验收（A–F），不达标退回。
- **角色变更（2026-08-26，D-006）**：项目第一负责人/唯一技术决策者/独立复审验收方由 Claude 整体移交给 Devin CLI（业主授权）。Claude 不再担任任何固定角色；历史 `⚠️待Claude复审` 条目语义不变（=待独立复审），不强制改写。

### 施工提示词语法

固定头部：立项背景（触发 + 证据链）→ 设计 SSOT 声明 → 约束与目标 → 边界/不做；正文 S1–Sn/T1–Tn，每步含目标、精确代码坐标（file:line/字段/方法签名）和落点；验收含机检五件套、race×3、check-lines 零警告、先红后绿；固定尾部"勿部署，停手等 Devin CLI 复审"，禁 `--no-verify`。

### 派工纪律

- **详细施工提示词文件可一次性全部落档**（让施工方提前看全貌、理解依赖链、预判风险）；落档后静态可读，不等于开工。
- **开工指令一次只发 1 个**，完成验收后再发下一个；依赖链上前序 batch 未验收前不发后续开工指令。
- **开工指令固定单行格式**：`[角色:施工] 开工：读 <派工单路径> @<落档commit>，按 S1 施工。串行，勿部署，完成报证据等复审。` 输出时**只给当前可派发的那一条**——即使被问"模板/下一条"，也不得把后续任务的转发指令一并列出（防误转发乱序）。完工回报格式：`[施工完成:<任务ID>] @<commit-hash>` + 机检/mutation 证据。
- 后续 batch 的详细文件里写的代码坐标可能因前序 batch 施工而漂移，前序验收后需更新坐标再发开工指令。
...
- 状态标记：`❓待核` / `🟦open` / `✅done` / `⚠️待独立复审`；`✅done` 只有 Devin CLI 独立复审后才权威。历史 `⚠️待Claude复审` 条目语义不变（=待独立复审），不强制改写。
...
- **收工顺序固定**：自审 → 更新 registry/STATE.md → 清扫私有记忆 → 追加 handover LOG → commit+push（D-007 后 Devin CLI 自主执行，无需业主逐次授权）。
- **破坏性操作保留确认**：`rm -rf`、`git reset --hard`、`git clean -fd`、force-push、删表/删分支/历史重写等不可逆操作仍需逐次向业主确认（安全红线，D-007 不覆盖）。
