...
| **C3** verify log 含**真实 go test 输出** | 必须含 `^ok\s+<package>\s+<N.Ns>` 或 `^--- PASS: Test\w+`；裸 `PASS` 单行**不算**（防手写）；不得含 `[no test files]` |
| **C4** verify log **不得是模板复制** | 同 milestone 下两张卡片的 log 指纹（去 commit/时间戳后 md5）相同 = 自动 FAIL |
...
❌ 测试函数体仅 `t.Log("XXX requires Y (M<m>.<x>-<y>)")` 一行 = 桩测试 = FAIL
   （t.Log 不是断言；无 t.Error/t.Fatal/require/assert 任一 = 该测试不算实施）
❌ `t.Skip("将在卡片 M<m>.<x>-<y> 中实施")` 引用的卡片 ID 等于本测试所属卡片 = 循环
   引用 = FAIL（M10.5-11 测试不许 t.Skip 引用 M10.5-11 自己）
❌ closure 报告含 "待后续实施" / "将在 X 中完成" / "stub" 段落但 milestone 仍标 ☑
❌ verify log 是其它卡片 log 的逐字复制（去 commit/时间戳后 md5 相同 = 模板伪造）
❌ verify log 出现 "All N packages: ok" / 裸单行 PASS 但无 "ok <pkg> <N.Ns>" 或 "--- PASS: TestXxx"
   （手写 PASS 字符串骗 grep = 立即作废 + Cascade 强制降级 + builder agent 信誉记 1 次违规）
❌ M10.5-14 / 任何 "红队/二次审计" 类卡片由 builder agent 自验 = 立即作废
   （此类卡片必须由 Cascade 跑 + 写 docs/handover/M<m>-REVIEW-by-Cascade-v?.md，否则 ROADMAP 该卡保持 🅒）
```

机器化探测（**五件套必须全 0 命中**）：
```bash
make detect-stubs                 # grep 全仓库 stub 关键词（含 Errorf("not yet implemented")）
make detect-skip-tests            # 列出无 milestone 引用的 t.Skip
make detect-orphan-test-claims    # 卡片提及但代码中不存在的 Test 函数
make detect-deadcode              # internal/ 下 0 import 的死包（R0-2，2026-05-26 起强制）
make detect-fakecomplete          # ROADMAP ☑ 卡片是否配对 handover log ≥30 行（R0-3）
make detect-layering              # connect/*.go 不得直接 import pgxpool/clickhouse.Conn/sqlx（R0-4）
make detect-spec-drift            # spec 中 LOC 声明 vs 代码 wc -l 偏差（R0-5）
```

> **2026-05-26 审计教训**：旧 detect-stubs 正则只扫 `Printf("...stub...")` 漏 `Errorf("not yet implemented")`，导致 MT5 PlaceOrder 全 stub 但卡片标 ☑、`controlplane`/`quantengine` 死代码未发现、46 张卡片缺 handover 仍 ☑。**单一来源验收不够**——必须五件套并行 + 双 reviewer 审计。详见 `docs/audit/2026-05-26-项目真实状态-合并报告.md` 和 `docs/plan/REMEDIATION-2026-05-26.md`。
