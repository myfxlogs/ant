    ...
    <div style={{ padding: '0 4px' }}>
      ...
      <div style={{ padding: 12, borderRadius: 6, border: '1px solid #e8e8e8', background: '#fcfcfd' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          ...
          <Button type="primary" size="small" loading={tuningRunning}
            disabled={!canRun} onClick={onRunTuning}
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 10, color: '#8c8c8c' }}>
              {enabledSweepDims.length}/{sweepDimensions.length} enabled · Cartesian: {cartesianSize.toLocaleString()}
            </span>
          </div>
          {sweepDimensions.map(d => (
            <label key={d.key} style={{
              ...
              background: d.enabled ? 'rgba(24,144,255,0.06)' : 'transparent',
