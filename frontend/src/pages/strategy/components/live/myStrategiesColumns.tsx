import { Tag, Typography, Space, Button, Tooltip, Badge, Dropdown, message } from 'antd';
import type { ColumnsType } from 'antd/es/table';
import { PlayCircleOutlined, PauseCircleOutlined, ThunderboltOutlined, EditOutlined, FileTextOutlined, HeartOutlined, DeleteOutlined, MoreOutlined, CopyOutlined } from '@ant-design/icons';
import { useTranslation } from 'react-i18next';
import type { TemplateOption, ScheduleRow } from '../../hooks/libraryTypes';
...
export interface ColumnCallbacks {
  templateById: Map<string, TemplateOption>;
  fmtAccount: (id: string) => string;
...
export function useMyStrategiesColumns(cb: ColumnCallbacks): ColumnsType<JoinedRow> {
  const { t } = useTranslation();
  const { templateById, fmtAccount, onToggleActive, onManualTrigger, onEdit, onEditParams, onDelete, onShowLogs, onHealthCheck, onStop, fallbackCopy } = cb;
    ...
    },
    {
      ...
      render: (_: unknown, row: JoinedRow) => {
        const menuItems = [
          ...
          { key: 'editCode', icon: <EditOutlined />, label: t('strategy.live.editStrategy', { defaultValue: 'Edit Strategy' }) },
