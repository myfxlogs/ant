import { Modal, Form, Input, Select, Space, Button, DatePicker, InputNumber, Row, Col, Typography, Tag } from 'antd';
...
export const StrategyTemplateBacktestModal: React.FC<StrategyTemplateBacktestModalProps> = ({
  open, template, form, submitting, accounts, symbols, symbolsLoading,
  quickRange, watchedRange, requiredParams = [], paramValues = {}, onParamValuesChange,
  onCancel, onSubmit, onApplyQuickRange, onSetQuickRange, onAccountChange,
}) => {
  const { t } = useTranslation();
  return (
    <Modal
      title={template ? t(BACKTEST_MODAL_TITLE_WITH_NAME_KEY, { name: template.name }) : t(ACTIONS_BACKTEST_KEY)}
      open={open} onCancel={onCancel} onOk={onSubmit} confirmLoading={submitting} width={720}
    >
      <Form form={form} size="small" layout="vertical" initialValues={{ timeframe: DEFAULT_TIMEFRAME, initialCapital: 10000 }}>
        ...
        {template && Array.isArray(template?.parameters) && template.parameters.length > 0 && (
          <div style={{ marginBottom: 12, padding: 8, borderRadius: 4, background: '#fafafa', border: '1px solid #f0f0f0' }}>
            <Typography.Text strong>{t(BACKTEST_PARAMETERS_TITLE_KEY, '策略参数')}</Typography.Text>
        ...
        <Form.Item name="title" label={t(BACKTEST_FIELDS_TITLE_KEY)}><Input readOnly /></Form.Item>
        <Row gutter={8}>
          <Col flex="260px">
            <Form.Item name="accountId" label={t(BACKTEST_FIELDS_ACCOUNT_KEY)} rules={[{ required: true, message: t(BACKTEST_VALIDATION_ACCOUNT_REQUIRED_KEY) }]}>
              <Select size="small" placeholder={t(BACKTEST_PLACEHOLDERS_ACCOUNT_KEY)}
                onChange={async (v) => { form.setFieldsValue({ symbol: '' }); await onAccountChange(String(v)); }}
                options={(accounts || []).map((a: any) => ({
                  value: String(a.id), label: `${a.login ?? a.id} (${a.mtType ?? ''})${a.isDisabled ? t(BACKTEST_ACCOUNT_DISABLED_SUFFIX_KEY) : ''}`,
                  disabled: !!a.isDisabled,
                }))} />
            </Form.Item>
          </Col>
          <Col flex="260px">
            <Form.Item name="symbol" label={t(BACKTEST_FIELDS_SYMBOL_KEY)} rules={[{ required: true, message: t(BACKTEST_VALIDATION_SYMBOL_REQUIRED_KEY) }]}>
              <Select size="small" showSearch allowClear loading={symbolsLoading} placeholder={t(BACKTEST_PLACEHOLDERS_SYMBOL_KEY)} options={symbols} optionFilterProp="label" />
            </Form.Item>
          </Col>
        </Row>
        <Form.Item name="extraSymbols" label={t(BACKTEST_FIELDS_EXTRA_SYMBOLS_KEY, '辅助标的（可多选）')}>
          <Select size="small" mode="multiple" allowClear loading={symbolsLoading} placeholder={t(BACKTEST_PLACEHOLDERS_EXTRA_SYMBOLS_KEY, '可选，配对/轮动策略常用')} options={symbols} optionFilterProp="label" maxTagCount="responsive" />
        </Form.Item>
        <Row gutter={8}>
          <Col flex="160px">
            <Form.Item name="timeframe" label={t(BACKTEST_FIELDS_TIMEFRAME_KEY)} rules={[{ required: true, message: t(BACKTEST_VALIDATION_TIMEFRAME_REQUIRED_KEY) }]}>
              <Select size="small" options={TIMEFRAMES.map(tf => ({ value: tf, label: tf }))} />
            </Form.Item>
          </Col>
          <Col flex="220px">
            <Form.Item name="initialCapital" label={t(BACKTEST_FIELDS_INITIAL_CAPITAL_KEY)} rules={[{ required: true, message: t(BACKTEST_VALIDATION_INITIAL_CAPITAL_REQUIRED_KEY) }]}>
        ...
        <Form.Item label={t(BACKTEST_FIELDS_RANGE_KEY)}>
          <div style={{ marginBottom: 4 }}>
            <Space size="small" wrap>
              {(['1D', '3D', '1W', '1Y', 'CUSTOM'] as QuickRangeKey[]).map(k => (
                <Button key={k} type={quickRange === k ? 'primary' : 'default'} size="small" onClick={() => onApplyQuickRange(k)}>
                  {quickRangeLabel(t, k)}
                </Button>
              ))}
            </Space>
          </div>
          <Input size="small" readOnly style={{ maxWidth: 420 }}
            value={watchedRange?.[0] && watchedRange?.[1] ? `${watchedRange[0].format('YYYY-MM-DD HH:mm')} → ${watchedRange[1].format('YYYY-MM-DD HH:mm')}` : ''}
