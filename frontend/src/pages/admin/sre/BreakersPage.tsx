...
export default function BreakersPage() {
  ...
  const columns = [
    ...
    { title: '总 P&L', dataIndex: 'total_pnl', key: 'pnl', width: 100, render: (v: number) => (v ?? 0).toFixed(2) },
    { title: '亏损 %', dataIndex: 'loss_percent', key: 'loss', width: 100, render: (v: number) => `${(v ?? 0).toFixed(2)}%` },
