  ...
  return (
    <>
      <Card title={<span style={{ color: 'var(--color-text)', fontWeight: 500 }}>{t('analytics.summary.cards.tradeStats')}</span>} className="glass-card mt-6">
        <Row gutter={[16, 16]}>
          <Col xs={12} sm={8} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t('analytics.summary.tradeStats.totalTrades')}</span>} value={tradeStats?.totalTrades || 0} valueStyle={{ color: 'var(--color-text)', fontSize: '20px' }} /></Col>
          <Col xs={12} sm={8} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t('analytics.summary.tradeStats.wins')}</span>} value={tradeStats?.winningTrades || 0} suffix={<span style={{ color: 'var(--color-text-muted)', fontSize: '14px' }}> ({tradeStats?.winRate?.toFixed(0) || 0}%)</span>} valueStyle={{ color: '#00A651', fontSize: '20px' }} /></Col>
          <Col xs={12} sm={8} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t('analytics.summary.tradeStats.losses')}</span>} value={tradeStats?.losingTrades || 0} valueStyle={{ color: '#E53935', fontSize: '20px' }} /></Col>
          ...
          <Col xs={24} sm={12} md={6}>
            <div className="flex items-center gap-2"><ClockCircleOutlined size={16} stroke={1.5} color="#8A9AA5" /><span style={{ color: 'var(--color-text-muted)', fontSize: '14px' }}>{t('analytics.summary.tradeStats.avgHolding')}</span></div>
            <div className="text-lg font-semibold mt-1" style={{ color: 'var(--color-text)' }}>{formatHoldingTime(tradeStats?.averageHoldingTime) || '-'}</div>
          </Col>
        </Row>
        <Row gutter={[16, 16]} className="mt-4">
          ...
          <Col xs={12} sm={8} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t('analytics.summary.tradeStats.avgProfit')}</span>} value={tradeStats?.averageProfit || 0} prefix="$" valueStyle={{ color: '#00A651', fontSize: '20px' }} /></Col>
      ...
      <Card title={<span style={{ color: 'var(--color-text)', fontWeight: 500 }}>{t('analytics.summary.cards.riskMetrics')}</span>} className="glass-card mt-6">
        <Row gutter={16}>
          <Col xs={12} sm={6} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t('analytics.summary.risk.maxDrawdown')}</span>} value={Math.abs(riskMetrics?.maxDrawdown || 0)} precision={2} prefix="$" valueStyle={{ color: '#E53935', fontSize: '20px' }} /></Col>
          ...
          <Col xs={12} sm={6} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t('analytics.summary.risk.var95')}</span>} value={riskMetrics?.valueAtRisk || 0} precision={2} prefix="$" valueStyle={{ color: 'var(--color-text)', fontSize: '20px' }} /></Col>
        </Row>
      </Card>
    </>
  );
