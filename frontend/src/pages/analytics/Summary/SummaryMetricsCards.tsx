    ...
    <>
      <Card title={<span style={{ color: 'var(--color-text)', fontWeight: 500 }}>{t(SUMMARY_CARDS_TRADE_STATS_KEY)}</span>} className="glass-card mt-6">
        ...
        <Row gutter={[16, 16]} className="mt-4">
          ...
          <Col xs={12} sm={8} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t(SUMMARY_TRADE_STATS_AVG_TRADING_VOLUME_KEY)}</span>} value={tradeStats?.averageVolume || 0} precision={2} valueStyle={{ color: 'var(--color-text)', fontSize: '20px' }} /></Col>
          <Col xs={12} sm={8} md={4}><Statistic title={<span style={{ color: 'var(--color-text-muted)' }}>{t(SUMMARY_TRADE_STATS_AVG_TRADING_PROFIT_KEY)}</span>} value={tradeStats?.averageProfit || 0} prefix="$" precision={2} valueStyle={{ color: '#00A651', fontSize: '20px' }} /></Col>
