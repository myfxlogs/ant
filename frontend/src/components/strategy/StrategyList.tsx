    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <Typography.Text style={{ fontSize: 13, fontWeight: 600 }}>
          <FileTextOutlined style={{ marginRight: 6 }} />策略模板
        </Typography.Text>
        {hasCode && <Button size="small" type="primary" onClick={onSave}>保存当前策略</Button>}
      </div>
      {templates.length > 0 ? (
            ...
            <div key={tpl.id}
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <Button size="small" type="link" icon={<SendOutlined />}
                  ...
                  style={{ fontSize: 10, padding: '0 4px', height: 20 }}>对话修改</Button>
                <Button size="small" type="link" icon={<CodeOutlined />}
                  onClick={() => setViewCode(tpl)}
                  style={{ fontSize: 10, padding: '0 4px', height: 20 }}>源码</Button>
                <Button size="small" type="link" icon={<EditOutlined />}
                  onClick={() => { setEditId(tpl.id); setEditName(tpl.name); }}
                  style={{ fontSize: 10, padding: '0 4px', height: 20 }}>重命名</Button>
                <Popconfirm title="确定删除此策略？" okText="删除" cancelText="取消"
                  okButtonProps={{ danger: true }} onConfirm={() => onDelete(tpl.id)}>
                  <Button size="small" type="link" icon={<DeleteOutlined />}
                    style={{ fontSize: 10, padding: '0 4px', height: 20, color: '#ff4d4f' }}>删除</Button>
                </Popconfirm>
              </div>
            </div>
          ))}
      ...
      {/* Code viewer modal */}
      <Modal title={viewCode?.name || '策略源码'} open={!!viewCode} onCancel={() => setViewCode(null)}
        footer={<Button onClick={() => { copy(viewCode?.code || ''); }} icon={<CopyOutlined />}>{copied ? '已复制' : '复制全部'}</Button>}
