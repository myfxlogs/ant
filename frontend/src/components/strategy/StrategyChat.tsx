...
import { Button, Input, Tag, Typography, Space, Collapse } from 'antd';
...
type ChatMsg = {
  ...
  metrics?: BacktestMetricsMsg | null;
};
...
export default function StrategyChat({ symbol, timeframe, sessionId, onApplyCode }: Props) {
  ...
  // ── Single handler: Conversate RPC ──
  const handleSend = useCallback(() => {
    const msg = draft.trim();
    if (!msg || busy) return;
    if (!hasSymbol) { addMsg('ai', { text: t('strategy.gen.planPrerequisiteMsg', '请先选择交易品种和时间周期。') }); return; }
    setDraft(''); setBusy(true);
    addMsg('user', { text: msg });

    const curCode = code;
    const abort = conversate(
      { message: msg, conversationId: sessionId, symbol, timeframe, plan, currentCode: curCode, backtestMetricsJson: metrics ? JSON.stringify(metrics) : '' },
      {
        onDelta: () => {},
        onPlan: (p) => { setPlan(p); addMsg('ai', { plan: p }); },
        onCode: (c) => {
          setCode(c); setPrevCode(curCode);
          addMsg('ai', { code: c, prevCode: curCode });
          setBusy(false);
        },
        onPreviousCode: (c) => setPrevCode(c),
        onToolCall: () => {},
        onToolResult: (tr: ToolResult) => {
          addMsg('ai', { toolResults: [tr as unknown as ToolResult] });
          if (tr.name === 'backtest' && tr.outputJson) try {
            ...
            if (out.run_id) pythonStrategyApi.watchBacktestRun(out.run_id, (u: BacktestRunUpdate) => {
              if (isSucceededRun(u.run) && u.metrics) setMetrics({
                totalReturn: u.metrics.totalReturn, sharpeRatio: u.metrics.sharpeRatio,
                maxDrawdown: u.metrics.maxDrawdown, winRate: u.metrics.winRate,
                totalTrades: u.metrics.totalTrades, profitFactor: u.metrics.profitFactor,
              });
            });
          } catch {}
        },
  ...
  }, [draft, busy, hasSymbol, plan, code, sessionId, symbol, timeframe, metrics, t]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fafbfc' }}>
      ...
      <div style={{
        ...
        {hasSymbol
          ? <Tag color="blue" style={{ fontSize: 10, margin: 0 }}>{symbol} · {timeframe}</Tag>
          : <Tag color="warning" style={{ fontSize: 10, margin: 0 }}>选择品种</Tag>
        }
        {modelOptions.length > 0 && (
          <Select size="small" value={selectedModel || undefined}
            onChange={async (v) => {
              ...
              try { await aiApi.setPrimary({ providerId: pid, model }); } catch {}
            }}
        ...
        {code && !busy && (
          <Button size="small" type="primary" ghost style={{ marginLeft: 'auto', fontSize: 11 }}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            ...
            <div style={{
              ...
              background: m.role === 'user' ? '#1677ff' : '#52c41a', color: '#fff',
            }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {/* Text */}
              {m.text && (
                <div style={{ fontSize: 12, lineHeight: '20px', color: '#262626', whiteSpace: 'pre-wrap' }}>
                  {m.text}
                </div>
              )}
              ...
              {m.plan && (
                <div style={{
                  ...
                  whiteSpace: 'pre-wrap', lineHeight: '20px',
                }}>
                  ...
                  <Collapse ghost size="small" items={[{
                    key: 'tools', label: (
                      <span style={{ fontSize: 10, color: '#8c8c8c' }}>
                        🔧 工具执行 ({m.toolResults.length})
                      </span>
                    ),
                    children: m.toolResults.map((tr, j) => (
                      <div key={j} style={{
              ...
              {/* Metrics */}
              {m.metrics && !m.code && (
                <div style={{ fontSize: 10, color: '#389e0d', marginTop: 4, padding: '4px 8px', background: '#f6ffed', borderRadius: 4 }}>
                  📊 Sharpe {m.metrics.sharpeRatio?.toFixed(2)} · 回撤 {((m.metrics.maxDrawdown ?? 0) * 100).toFixed(1)}% · 胜率 {((m.metrics.winRate ?? 0) * 100).toFixed(0)}% · {m.metrics.totalTrades}笔
                </div>
              )}
              ...
              {m.code && (
                <div style={{ marginTop: 4, position: 'relative' }}>
                  <div style={{
                    ...
                    padding: '3px 10px', background: '#282c34', borderRadius: '6px 6px 0 0',
                  }}>
                    <span style={{ fontSize: 10, color: '#abb2bf', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <CodeOutlined /> Python
                    </span>
                    <Button size="small" type="text" icon={<CopyOutlined />}
                      onClick={() => handleCopy(m.code!)}
                      style={{ color: '#abb2bf', fontSize: 11 }}>
                      {copied ? '已复制' : '复制'}
                    </Button>
                  </div>
                  <SyntaxHighlighter language="python" style={atomOneDark}
                    ...
                    wrapLines
                    customStyle={{
                      margin: 0, borderRadius: '0 0 6px 6px', fontSize: 11,
                      padding: '8px 0', maxHeight: 300,
                    }}
                    lineNumberStyle={{ fontSize: 10, minWidth: '2em', color: '#636d83' }}>
                    {m.code}
                  </SyntaxHighlighter>
                  <Button size="small" type="link" style={{ fontSize: 10, padding: 0, marginTop: 2 }}
        ...
        {busy && messages.length > 0 && messages[messages.length - 1]?.role === 'user' && (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{ width: 28, height: 28, borderRadius: 14, background: '#52c41a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <RobotOutlined style={{ color: '#fff', fontSize: 14 }} />
            </div>
            <LoadingOutlined style={{ color: '#1677ff' }} />
            <span style={{ fontSize: 11, color: '#8c8c8c' }}>思考中...</span>
          </div>
        )}
        <div ref={chatEndRef} />
      </div>
      ...
      <div style={{ padding: '10px 14px', borderTop: '1px solid #e8e8e8', flexShrink: 0, background: '#fff' }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          <TextArea
            ...
            disabled={busy || !hasSymbol}
            style={{ fontSize: 12, borderRadius: 8 }}
          />
          <Button type="primary" icon={<SendOutlined />}
            onClick={handleSend} loading={busy}
            disabled={!draft.trim() || !hasSymbol}
