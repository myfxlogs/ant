// Auto-generated supplementary keys for marketplace
const MarketplaceExtra = {
  "marketplace": {
    "title": "探索策略",
    "seoTitle": "策略市場",
    "seoDesc": "發現併購買 MT4/MT5 交易策略。支援 IC Markets、Pepperstone、XM 等 30+ 經紀商。AI 輔助策略生成與最佳化。回測驗證，實盤表現追蹤。",
    "publish": {
      "priceModel": {
        "once": "一次性購買"
      },
      "title": "釋出到市場",
      "titlePlaceholder": "例如：金叉策略",
      "descriptionPlaceholder": "描述您的策略邏輯、入場/出場規則...",
      "assetClass": {
        "label": "資產類別"
      },
      "riskLevel": {
        "label": "風險等級"
      },
      "tagsPlaceholder": "輸入並按回車新增標籤",
      "codeSnippet": "策略預覽（公開）",
      "codeSnippetPlaceholder": "可選：分享策略片段或概要（所有人可見）",
      "includeBacktestSnapshot": "包含最新回測結果",
      "winRate": "勝率"
    },
    "messages": {
      "published": "策略已釋出到市場！",
      "publishFailed": "釋出策略失敗"
    }
  }
} as const;
export default MarketplaceExtra;
