const aiCore = {
  ai: {
    agentPrompts: {
      style: {
        title: '市场状态/风格推荐',
        prompt: `你是资深量化投研分析师。请基于以下信息，推荐策略范式：趋势/均值回复/短线，并说明理由、适用条件与不适用场景。
...
{{baseInfo}}`
      ...
      signals: {
        title: '信号与指标设计',
        prompt: `你是量化因子与信号工程师。请在不依赖外部数据（除非用户提供宏观事件表）的前提下，设计可实现的交易信号。
...
{{baseInfo}}`
      ...
      risk: {
        title: '风控与执行约束',
        prompt: `你是交易风控与执行专家。请根据以下信息，设计仓位管理、止损止盈、最大回撤控制、冷却期/交易频率限制等规则。
...
{{baseInfo}}`
      ...
      code: {
        title: '代码生成 Agent',
        prompt: `你是 AntTrader Python 策略代码工程师。请生成一份可运行的 AntTrader Python 策略代码，要求：
...
请你将市场/信号/风控三个分析结论落到代码中（如果上游结论未提供，也请给出合理默认）。`
    ...
    client: {
      errors: {
        ...
        edgeGatewayTimeout: '站点前置网关超时（常见为 Cloudflare 524 等）：请求在到达应用前被中断，长耗时的「生成代码」等步骤更容易触发。请在辩论页代码步骤使用「重新尝试生成代码」，或先返回上一步再进入生成代码；仍失败时需运维调大网关/源站超时。',
