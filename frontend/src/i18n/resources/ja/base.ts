// Auto-generated from proto/ant/v1/i18n/base_ja.textproto
// DO NOT EDIT MANUALLY — run: npx tsx scripts/i18n-build.ts
const Base = {
  "admin": {
    "dashboard": {
      "logs": {
        "moduleMap": {
          "accountManagement": "アカウント管理",
          "systemConfig": "システム設定",
          "trading": "取引",
          "userManagement": "ユーザー管理"
        },
        "actionType": "アクション",
        "failed": "失敗",
        "module": "モジュール",
        "status": "ステータス",
        "success": "成功",
        "target": "対象",
        "time": "時間"
      },
      "errors": {
        "loadFailed": "ダッシュボードデータの読み込みに失敗しました"
      },
      "riskMetrics": {
        "orderCloseFailed": "平仓失败",
        "orderCloseSuccess": "決済成功",
        "orderSendFailed": "注文失敗",
        "orderSendSuccess": "注文成功",
        "riskValidateError": "エラー",
        "riskValidatePass": "通過",
        "riskValidateReject": "拒否",
        "riskValidateTotal": "検証総数",
        "title": "リスク検証指標"
      },
      "riskWindow": {
        "noData": "暂无窗口指标数据",
        "noRejectData": "この期間に拒否はありません",
        "orderCloseFailed": "決済失敗",
        "orderCloseSuccess": "決済OK",
        "orderSendFailed": "注文失敗",
        "orderSendSuccess": "注文OK",
        "rejectCount": "拒否数",
        "rejectRiskCodesHeader": "リスクコード",
        "title": "リスク管理ウィンドウ",
        "validateError": "エラー",
        "validatePass": "通過",
        "validateReject": "拒否",
        "validateTotal": "合計"
      },
      "activeSubs": "有効なサブスクリプション",
      "activeUsers": "アクティブユーザー",
      "loadFailed": "ダッシュボードデータの読み込みに失敗しました",
      "marketRevenue": "マーケット収益",
      "marketSales": "マーケット販売",
      "marketStrategies": "マーケットストラテジー",
      "monthlyRevenue": "月間収益",
      "mtAccounts": "MTアカウント",
      "onlineAccounts": "オンラインアカウント",
      "orderCloseFailed": "注文決済失敗",
      "orderCloseSuccess": "注文決済成功",
      "orderSendFailed": "注文送信失敗",
      "orderSendSuccess": "注文送信成功",
      "recentLogs": "最近のログ",
      "rejectCount": "却下数",
      "title": "管理ダッシュボード",
      "todayProfit": "本日の損益",
      "todayTrades": "本日の取引",
      "totalRevenue": "総収益",
      "totalUsers": "総ユーザー数",
      "validateError": "検証エラー",
      "validatePass": "検証合格",
      "validateReject": "検証却下",
      "validateTotal": "検証合計",
      "verifiedUsers": "認証済みユーザー"
    },
    "userManagement": {
      "drawer": {
        "labels": {
          "createdAt": "作成日時",
          "email": "メール",
          "id": "ID",
          "lastLogin": "最終ログイン",
          "mtAccountCount": "MTアカウント",
          "nickname": "ニックネーム",
          "role": "役割",
          "status": "ステータス"
        },
        "title": "ユーザー詳細"
      },
      "form": {
        "placeholders": {
          "email": "メールを入力",
          "nickname": "ニックネームを入力",
          "password": "输入密码"
        },
        "accountNumber": "口座番号",
        "accountNumberInvalid": "5-6桁、先頭ゼロなし、4と7は不可",
        "accountNumberPlaceholder": "例: 123568",
        "email": "メール",
        "nickname": "ニックネーム",
        "password": "パスワード",
        "role": "役割",
        "status": "ステータス"
      },
      "passwordForm": {
        "placeholders": {
          "confirmPassword": "再次输入新密码",
          "newPassword": "新しいパスワードを入力"
        },
        "validation": {
          "confirmPasswordRequired": "パスワード確認が必要です",
          "newPasswordRequired": "新しいパスワードが必要です",
          "passwordMin8": "パスワードは8文字以上必要です",
          "passwordMismatch": "パスワードが一致しません",
          "passwordMustContainLettersAndNumbers": "密码必须包含字母和数字"
        },
        "confirmPassword": "パスワード確認",
        "newPassword": "新しいパスワード",
        "submit": "パスワード更新"
      },
      "actions": {
        "changePassword": "修改密码",
        "details": "詳細",
        "disable": "無効化",
        "enable": "有効化"
      },
      "deleteConfirm": {
        "batchDeleteConfirm": "{{count}}人のユーザーを削除しますか？この操作は元に戻せません。",
        "batchDeletePartial": "{{deleted}}人削除、{{failed}}人失敗",
        "batchDeleteSuccess": "{{count}}人のユーザーを削除しました",
        "title": "このユーザーを削除しますか？この操作は元に戻せません。"
      },
      "filters": {
        "rolePlaceholder": "役割でフィルター",
        "searchPlaceholder": "メールまたは名前で検索",
        "statusPlaceholder": "按状态筛选"
      },
      "messages": {
        "loadUsersFailed": "ユーザーの読み込みに失敗しました",
        "newPasswordIs": "新密码为: {{password}}",
        "passwordUpdateFailed": "パスワード更新に失敗しました",
        "passwordUpdatedSuccess": "パスワードを更新しました",
        "userCreateFailed": "ユーザー作成に失敗しました",
        "userCreatedSuccess": "ユーザーを作成しました",
        "userDeleteFailed": "ユーザー削除に失敗しました",
        "userDeletedSuccess": "ユーザーを削除しました",
        "userDisabled": "ユーザーを無効化しました",
        "userEnabled": "ユーザーを有効化しました",
        "userUpdateFailed": "ユーザー更新に失敗しました",
        "userUpdatedSuccess": "ユーザーを更新しました"
      },
      "modals": {
        "createTitle": "ユーザー作成",
        "editTitle": "ユーザー編集",
        "passwordTitle": "修改密码"
      },
      "pagination": {
        "total": "共 {{total}} 位用户"
      },
      "roles": {
        "audit": "审计",
        "customerService": "カスタマーサポート",
        "operation": "運用",
        "superAdmin": "スーパー管理者",
        "user": "ユーザー"
      },
      "status": {
        "active": "アクティブ",
        "suspended": "已停用"
      },
      "table": {
        "actions": "操作",
        "createdAt": "作成日時",
        "email": "メール",
        "id": "ID",
        "mtAccountCount": "MTアカウント",
        "nickname": "ニックネーム",
        "role": "役割",
        "status": "ステータス"
      },
      "addUser": "ユーザー追加",
      "title": "ユーザー管理"
    },
    "account": {
      "columns": {
        "action": "操作",
        "balance": "残高",
        "broker": "ブローカー",
        "createdAt": "作成日時",
        "detail": "詳細",
        "equity": "有効証拠金",
        "id": "ID",
        "login": "ログイン",
        "margin": "証拠金",
        "server": "サーバー",
        "status": "ステータス",
        "time": "時間",
        "type": "タイプ",
        "user": "ユーザー"
      },
      "errors": {
        "freezeFailed": "凍結に失敗しました",
        "loadFailed": "アカウントの読み込みに失敗しました",
        "unfreezeFailed": "凍結解除に失敗しました"
      },
      "auditLogs": "監査ログ",
      "confirmFreeze": "このアカウントを凍結しますか？",
      "detail": "詳細",
      "freeze": "凍結",
      "frozen": "アカウントは凍結されています",
      "offline": "オフライン",
      "online": "オンライン",
      "searchPlaceholder": "アカウントを検索",
      "status": "ステータス",
      "title": "アカウント管理",
      "unfreeze": "凍結解除",
      "unfrozen": "アカウントの凍結が解除されました"
    },
    "aiGateway": {
      "columns": {
        "apiKey": "APIキー",
        "baseUrl": "ベースURL"
      },
      "errors": {
        "loadModels": "モデルの読み込みに失敗しました",
        "loadProviders": "プロバイダーの読み込みに失敗しました",
        "toggleFailed": "切り替えに失敗しました"
      },
      "addModel": "モデルを追加",
      "addProvider": "プロバイダーを追加",
      "addProviderPending": "プロバイダー追加機能はバックエンドのサポート待ちです",
      "apiKeyEditHint": "既存のキーを保持するには空のままにしてください",
      "apiKeyEditPlaceholder": "空のままで保持",
      "apiKeyHint": "APIキー、保存時は暗号化されます",
      "apiKeyLabel": "APIキー",
      "baseUrl": "ベースURL",
      "baseUrlRequired": "ベースURLを入力してください",
      "configured": "設定済み",
      "confirmDeleteModel": "このモデルを削除しますか？",
      "description": "AIプロバイダー、モデル、料金を管理します。ユーザーは利用可能なモデルから選択し、ウォレットのトークンで課金されます。",
      "discover": "探索",
      "discoverFailed": "モデルの検出に失敗しました",
      "displayName": "表示名",
      "displayNamePlaceholder": "DeepSeek Chat",
      "displayNameRequired": "表示名を入力してください",
      "editModel": "モデルを編集",
      "editProvider": "プロバイダーを編集",
      "modelName": "モデル名",
      "modelNameRequired": "モデル名を入力してください",
      "models": "モデル",
      "noModels": "モデルがありません",
      "noModelsDiscovered": "モデルが検出されませんでした。API キーとベース URL を確認してください。",
      "notConfigured": "未設定",
      "priceInput": "入力価格 ($/1M)",
      "priceOutput": "出力価格 ($/1M)",
      "provider": "プロバイダー",
      "providerId": "プロバイダーID",
      "providerIdRequired": "プロバイダーIDを入力してください",
      "title": "AIゲートウェイ管理"
    },
    "billing": {
      "columns": {
        "amount": "金額",
        "autoRenew": "自動更新",
        "balanceAfter": "変更後の残高",
        "balanceBefore": "変更前の残高",
        "createdAt": "作成日時",
        "cycle": "サイクル",
        "description": "説明",
        "periodEnd": "期間終了",
        "periodStart": "期間開始",
        "plan": "プラン",
        "price": "価格",
        "status": "ステータス",
        "time": "日時",
        "type": "種別",
        "user": "ユーザー"
      },
      "activeCount": "アクティブ",
      "activeSubs": "アクティブなサブスクリプション",
      "filterByPlan": "プランでフィルター",
      "filterByStatus": "ステータスでフィルター",
      "filterByType": "タイプでフィルター",
      "monthlyRevenue": "月間収益",
      "planEnterprise": "エンタープライズ",
      "planFree": "無料",
      "planPro": "プロ",
      "planRevenue": "プラン別収益詳細",
      "statusActive": "アクティブ",
      "statusCancelled": "キャンセル済み",
      "statusExpired": "期限切れ",
      "subscriptions": "サブスクリプション",
      "title": "課金管理",
      "totalRevenue": "総収益",
      "txDeposit": "入金",
      "txPlatformFee": "プラットフォーム手数料",
      "txPurchase": "購入",
      "txRecords": "取引",
      "txSale": "販売",
      "txWithdrawal": "出金",
      "walletTransactions": "ウォレット取引"
    },
    "config": {
      "messages": {
        "disabled": "已禁用",
        "enabled": "已启用",
        "loadFailed": "加载配置失败",
        "operationFailed": "操作失败",
        "updateFailed": "更新配置失败",
        "updated": "配置已更新"
      },
      "placeholders": {
        "apiKey": "输入API Key",
        "baseUrl": "输入Base URL",
        "configValue": "输入配置值",
        "description": "输入描述",
        "json": "输入JSON",
        "model": "输入模型名称"
      },
      "providerOptions": {
        "custom": "自定义 / OpenAI 兼容",
        "deepseek": "DeepSeek",
        "zhipu": "智谱AI"
      },
      "validation": {
        "apiKeyRequired": "API Key不能为空",
        "greenMaxFailedRunsNonNegative": "绿色最大失败次数需≥0",
        "greenSuccessRateRange": "绿色成功率需在0-100之间",
        "jsonEmpty": "JSON不能为空",
        "jsonInvalid": "JSON格式无效",
        "minSampleSizeNonNegative": "最小样本量需≥0",
        "modelRequired": "模型名称不能为空",
        "yellowNotGreaterThanGreen": "黄色阈值不能超过绿色阈值",
        "yellowSuccessRateRange": "黄色成功率需在0-100之间"
      },
      "aiProviderCatalog": "AI提供商目录",
      "apiKey": "APIキー",
      "baseUrlLabel": "Base URL",
      "configItem": "配置项",
      "description": "説明",
      "econAIConfig": "经济日历AI配置",
      "editConfig": "编辑配置: {{key}}",
      "enableToggle": "有効化",
      "fillTemplate": "填充模板",
      "formatJson": "格式化JSON",
      "maxAccountsPerUser": "每用户最大账户数",
      "modelName": "模型名称",
      "off": "关",
      "on": "开",
      "provider": "提供商",
      "status": "ステータス",
      "strategyHealthConfig": "策略健康度配置",
      "thresholdDesc": "阈值描述",
      "thresholdInfo": "阈值说明",
      "title": "系统配置",
      "toggle": "切换",
      "updatedAt": "更新时间",
      "value": "值"
    },
    "deposit": {
      "table": {
        "action": "操作",
        "amount": "USDT額",
        "amountUsd": "USDクレジット",
        "block": "ブロック",
        "confirmations": "確認数",
        "reviewNote": "レビューノート",
        "status": "ステータス",
        "time": "時間",
        "txHash": "Txハッシュ",
        "user": "ユーザー"
      },
      "allStatuses": "全ステータス",
      "approve": "承認",
      "approveFailed": "入金の承認に失敗しました。",
      "approveTitle": "入金を承認",
      "approveWarning": "承認すると、すぐにユーザーのウォレットに入金されます。",
      "approved": "入金が承認され、ウォレットに反映されました。",
      "reject": "拒否",
      "rejectFailed": "入金の却下に失敗しました。",
      "rejectTitle": "入金を拒否",
      "rejected": "入金が却下されました。",
      "reviewNoteLabel": "審査メモ（任意）",
      "reviewNotePlaceholder": "この審査に関するメモを追加...",
      "statusApproved": "承認済み",
      "statusPending": "保留中",
      "statusRejected": "拒否済み",
      "title": "入金管理"
    },
    "jurisdiction": {
      "messages": {
        "countryAddFailed": "国の追加に失敗しました",
        "countryAdded": "国を追加しました",
        "countryRemoveFailed": "国の削除に失敗しました",
        "countryRemoved": "国を削除しました",
        "kycUpdateFailed": "KYCステータス更新に失敗しました",
        "kycUpdated": "KYCステータスを更新しました",
        "overrideUpdateFailed": "更新制裁豁免失败",
        "overrideUpdated": "上書き設定を更新しました"
      },
      "actions": "操作",
      "addCountry": "国を追加",
      "addSanctionedCountry": "制裁国を追加",
      "addedBy": "追加者",
      "confirmGrantOverride": "このユーザーに上書き許可を付与しますか？",
      "confirmRevokeOverride": "このユーザーの上書き許可を取り消しますか？",
      "country": "国",
      "countryCode": "国コード",
      "countryLabel": "国",
      "disclaimer": "免責事項",
      "emptyKYC": "KYCレコードがありません",
      "emptySanctions": "制裁国はありません",
      "filterByKYCStatus": "KYCステータスでフィルター",
      "grantOverride": "上書き許可",
      "kycStatus": "KYCステータス",
      "kycStatusTab": "ユーザーKYCステータス",
      "override": "上書き",
      "overrideWarning": "此用户来自受制裁国家，授予豁免将允许交易。",
      "pending": "保留中",
      "questionnaire": "アンケート",
      "rejected": "拒否",
      "revokeOverride": "上書き取消",
      "sanctioned": "制裁済み",
      "sanctionedCountries": "制裁対象国",
      "sanctionedCountriesTab": "制裁対象国",
      "setKYC": "KYC設定",
      "setKYCStatus": "KYCステータス設定",
      "title": "管轄権管理",
      "unverified": "未確認",
      "userEmail": "メール",
      "userKYCStatus": "ユーザーKYCステータス",
      "verified": "確認済み"
    },
    "logs": {
      "actions": {
        "create": "作成",
        "delete": "削除",
        "disable": "無効化",
        "enable": "有効化",
        "freeze": "凍結",
        "unfreeze": "凍結解除",
        "update": "更新"
      },
      "columns": {
        "action": "操作",
        "actionType": "アクションタイプ",
        "details": "詳細",
        "ip": "IPアドレス",
        "module": "モジュール",
        "status": "ステータス",
        "target": "ターゲット",
        "time": "時刻"
      },
      "errors": {
        "loadFailed": "ログの読み込みに失敗しました"
      },
      "modules": {
        "accountManagement": "アカウント管理",
        "systemConfig": "システム設定",
        "trading": "トレーディング",
        "userManagement": "ユーザー管理"
      },
      "filterAction": "操作で絞り込み",
      "filterModule": "モジュールで絞り込み",
      "title": "操作ログ"
    },
    "settings": {
      "columns": {
        "action": "操作",
        "key": "設定キー",
        "value": "値"
      },
      "actionFailed": "操作に失敗しました",
      "addManagedSetting": "管理設定の追加",
      "addSetting": "設定の追加",
      "confirmDelete": "削除してよろしいですか？",
      "deleteFailed": "削除に失敗しました",
      "deleted": "削除しました",
      "keyPlaceholder": "例: allowed_models, disable_live_trading, permission.rule.1",
      "permissionAddRule": "ルールの追加: キーを使って設定を作成します",
      "permissionExample": "例:",
      "permissionFormat": "形式:",
      "permissionRules": "パーミッションルール (permission.rule.N)",
      "saveFailed": "保存に失敗しました",
      "saveSuccess": "保存に成功しました",
      "settingKey": "設定キー",
      "title": "エージェント管理設定",
      "valuePlaceholder": "例: claude-sonnet-5,deepseek-v4"
    },
    "wallet": {
      "columns": {
        "amount": "金額",
        "balance": "残高",
        "balanceAfter": "調整後残高",
        "currency": "通貨",
        "description": "説明",
        "email": "メールアドレス",
        "frozen": "凍結額",
        "nickname": "ニックネーム",
        "time": "日時",
        "type": "種類",
        "walletNumber": "ウォレット番号"
      },
      "errors": {
        "noUserSelected": "ユーザーが選択されていません"
      },
      "messages": {
        "adjustFailed": "調整に失敗しました",
        "adjustSuccess": "残高の調整に成功しました"
      },
      "accountNumber": "口座番号",
      "add": "追加",
      "adjustBalance": "残高調整",
      "adjustFailed": "調整に失敗しました",
      "adjustReason": "理由",
      "adjustSuccess": "残高を調整しました",
      "deduct": "控除",
      "noMatch": "該当するユーザーが見つかりません",
      "noUsers": "ユーザーが見つかりません",
      "reason": "調整理由...",
      "searchPlaceholder": "メールまたは口座番号で検索...",
      "tabDepositAddresses": "入金アドレス",
      "tabWallets": "ユーザーウォレット",
      "title": "ウォレット管理",
      "transactions": "取引履歴",
      "unassigned": "未割り当て",
      "userList": "ユーザー一覧",
      "walletDetail": "ウォレット詳細",
      "walletFor": "ウォレット -"
    },
    "analytics": {
      "activeBuyers": "アクティブバイヤー",
      "name": "名前",
      "newStrategies": "新規ストラテジー",
      "newSubs": "新規サブスクライバー",
      "platformRev": "プラットフォーム収益",
      "providerRev": "プロバイダー収益",
      "refundRate": "返金率",
      "topByRevenue": "収益上位のストラテジー",
      "topBySubs": "購読者上位のストラテジー",
      "topProvidersRev": "収益上位のプロバイダー",
      "topProvidersStrat": "ストラテジー数上位のプロバイダー",
      "totalStrategies": "ストラテジー総数",
      "totalTx": "トランザクション",
      "value": "値"
    },
    "autogen": {
      "actions": "アクション",
      "allStatus": "すべてのステータス",
      "approve": "承認",
      "approveFailed": "承認に失敗しました",
      "approved": "タスクが承認され公開されました",
      "confirmApprove": "承認して公開しますか？",
      "confirmReject": "このタスクを拒否しますか？",
      "enqueue": "キューに追加",
      "error": "エラー",
      "loadFailed": "タスクの読み込みに失敗しました",
      "quality": "品質",
      "refresh": "更新",
      "reject": "拒否",
      "rejectFailed": "拒否に失敗しました",
      "rejected": "タスクが拒否されました",
      "status": "ステータス",
      "strategyType": "タイプ",
      "strategyTypes": "ストラテジータイプ（カンマ区切り）",
      "symbol": "シンボル",
      "symbols": "シンボル（カンマ区切り）",
      "timeframe": "TF",
      "timeframes": "時間足（カンマ区切り）",
      "title": "AI ストラテジー生成タスク",
      "triggerBatch": "バッチトリガー",
      "triggerFailed": "トリガーに失敗しました"
    },
    "coupon": {
      "codePlaceholder": "クーポンコード（例: SUMMER20）",
      "colActions": "アクション",
      "colCode": "コード",
      "colExpires": "有効期限",
      "colMinPurchase": "最低購入",
      "colStatus": "ステータス",
      "colType": "タイプ",
      "colUsage": "使用状況",
      "colValue": "値",
      "create": "クーポンを作成",
      "createFailed": "クーポンの作成に失敗しました",
      "createTitle": "クーポンを作成",
      "created": "クーポンを作成しました",
      "disable": "無効化",
      "disableFailed": "クーポンの無効化に失敗しました",
      "disabled": "クーポンは無効化されました",
      "expiresPlaceholder": "有効期限（ISO 8601、空欄 = 無期限）",
      "fillRequired": "必須項目を入力してください",
      "loadFailed": "クーポンの読み込みに失敗しました",
      "maxUsesPlaceholder": "最大使用回数（0 = 無制限）",
      "minPurchasePlaceholder": "最低購入金額（0 = なし）",
      "valuePlaceholder": "割引額（例: 20 で 20%、50 で $50）"
    },
    "depositAddresses": {
      "address": "アドレス",
      "all": "すべてのステータス",
      "assignedAt": "割り当て日時",
      "availablePool": "プール内利用可能",
      "import": "アドレスをインポート",
      "importFailed": "インポートに失敗しました",
      "importHint": "オフラインマシンで hdgen ツールを使用して deposit_addresses.bin を生成し、ここにアップロードしてください。",
      "index": "インデックス",
      "network": "ネットワーク",
      "received": "受領 USDT",
      "status": "ステータス",
      "total": "アドレス総数",
      "user": "ユーザー ID"
    },
    "header": {
      "admin": "管理",
      "adminMode": "管理员模式",
      "adminPanel": "管理后台",
      "backToUser": "返回用户端",
      "logout": "ログアウト"
    },
    "marketplace": {
      "colActions": "アクション",
      "colFeatured": "特集",
      "colPrice": "価格",
      "colPublisher": "公開者",
      "colRevenue": "収益",
      "colSales": "販売数",
      "colStatus": "ステータス",
      "colTitle": "タイトル",
      "feature": "特集",
      "featureDesc": "特集表示の優先度を設定します。数値が大きいほど目立ちます。",
      "featureFailed": "ストラテジーの特集に失敗しました",
      "featureSuccess": "ストラテジーを特集しました",
      "featureTitle": "ストラテジーを特集",
      "filterStatus": "すべてのステータス",
      "loadFailed": "ストラテジーの読み込みに失敗しました",
      "searchPlaceholder": "タイトルで検索...",
      "unfeature": "特集を解除",
      "unfeatureFailed": "特集解除に失敗しました",
      "unfeatureSuccess": "特集を解除しました"
    },
    "refund": {
      "approve": "承認して実行",
      "approveTitle": "返金を承認",
      "approved": "返金が承認され実行されました",
      "colActions": "アクション",
      "colAmount": "金額",
      "colDate": "日付",
      "colReason": "理由",
      "colStatus": "ステータス",
      "colStrategy": "ストラテジー",
      "colUser": "ユーザー",
      "filterStatus": "すべてのステータス",
      "loadFailed": "返金リクエストの読み込みに失敗しました",
      "processFailed": "返金の処理に失敗しました",
      "reject": "拒否",
      "rejectTitle": "返金を拒否",
      "rejected": "返金リクエストが拒否されました",
      "reviewNotePlaceholder": "レビューメモ（拒否の場合は任意、承認の場合は推奨）..."
    },
    "sidebar": {
      "accountManagement": "アカウント管理",
      "agentSettings": "エージェント設定",
      "aiGateway": "AIゲートウェイ",
      "analytics": "分析",
      "autogenTasks": "AI生成タスク",
      "billing": "課金管理",
      "coupons": "クーポン管理",
      "dashboard": "ダッシュボード",
      "deposits": "入金管理",
      "jurisdiction": "管轄権管理",
      "marketplace": "マーケットプレース",
      "monitoring": "監視・アラート",
      "operationLogs": "操作ログ",
      "refunds": "返金管理",
      "shareManagement": "シェア分析",
      "sre": "SREコントロール",
      "strategies": "戦略管理",
      "sweep": "スイープ管理",
      "systemConfig": "システム設定",
      "tradingMonitor": "取引監視",
      "userManagement": "ユーザー管理",
      "walletManagement": "ウォレット管理"
    },
    "trading": {
      "accounts": "アカウント",
      "activeUsers": "アクティブユーザー",
      "byPlatform": "プラットフォーム別",
      "closedOrders": "決済済み",
      "connectedAccounts": "接続済み",
      "loadFailed": "加载交易统计失败",
      "netProfit": "純利益",
      "orders": "注文",
      "pendingOrders": "挂单",
      "platform": "プラットフォーム",
      "profitStats": "損益統計",
      "title": "取引監視",
      "totalAccounts": "総アカウント数",
      "totalLoss": "総損失",
      "totalOrders": "総注文数",
      "totalProfit": "総利益",
      "totalUsers": "総ユーザー数",
      "totalVolume": "総取引量",
      "volume": "数量"
    },
    "walletCalculator": {
      "fillResult": "結果を入力",
      "selectModel": "モデルを選択（価格基準）",
      "title": "トークン ↔ USD 計算機",
      "tokenAmount": "トークン数量",
      "usdAmount": "USD金額"
    }
  },
  "strategy": {
    "backtest": {
      "diagnostic": {
        "suggestion": {
          "dll": "DLL imports are not supported — remove external DLL calls and use built-in MQL functions",
          "iCustom": "iCustom (custom indicator) is not supported — replace with a built-in indicator (iMA/iRSI/iMACD etc.) or implement the logic manually"
        },
        "aiFailed": "AI 修正に失敗しました",
        "aiFix": "AI 修正",
        "aiNoResult": "AI がコードを返しませんでした",
        "allSilenced": "すべての警告を意図的なものとして確認済み",
        "apply": "適用して再実行",
        "applyFailed": "修正の適用に失敗しました",
        "compatible": "互換",
        "coverage": "カバレッジ",
        "defenseA": "構造検証",
        "diffHint": "下の AI 生成コードを確認してください。適用すると新しいバージョンが作成され、バックテストが再実行されます。",
        "diffPreview": "AI 修正プレビュー",
        "fatal": "重大な問題",
        "fixApplied": "Fix applied — re-running backtest",
        "fixAppliedCompileWarn": "修正を適用しましたが、コンパイルに警告があります",
        "info": "品質ヒント",
        "invariant": "不変条件違反",
        "lookahead": "ルックアヘッドバイアス",
        "noCode": "修正するストラテジーコードがありません",
        "saveFirst": "AI 修正を適用するには、まずストラテジーを保存してください",
        "silenceHint": "Acknowledge as intentional — hide this warning",
        "statistical": "統計ヒント",
        "suggestionLabel": "建议",
        "unknown": "診断",
        "unsupported": "非対応",
        "warning": "リスク警告"
      },
      "autoGate": "自動ゲート評価",
      "cancelFailed": "キャンセルに失敗しました",
      "canceled": "バックテストがキャンセルされました",
      "lotSize": "ロットサイズ",
      "notPublishable": "公開不可",
      "publishable": "公開可能",
      "strategyParameters": "戦略パラメータ"
    },
    "live": {
      "diag": {
        "execState": {
          "accepted_unconfirmed": "承認（未確認）",
          "confirmed": "確認済み",
          "deterministic_rejected": "拒否",
          "idle": "アイドル",
          "outcome_unknown": "不明",
          "submitting": "送信中"
        },
        "lifecycle": {
          "order_confirmed": "注文確認済み",
          "order_outcome_unknown": "結果不明",
          "order_rejected": "注文拒否",
          "order_submitted": "注文送信済み",
          "order_submitting": "注文送信中",
          "signal_generated": "シグナル生成"
        },
        "source": {
          "account_summary": "口座サマリー",
          "order_update": "注文更新",
          "position_snapshot": "ポジションスナップショット",
          "profit_stream": "プロフィットストリーム"
        },
        "state": {
          "active": "アクティブ",
          "dataStarvation": "データ飢餓",
          "error": "エラー",
          "noEvaluations": "評価なし",
          "warning": "警告"
        },
        "ago": "前",
        "barCount": "Bar評価",
        "brokerAccountOrders": "ブローカーアカウント注文",
        "evalCount": "評価回数",
        "execution": "実行",
        "executionState": "実行状態",
        "financialAge": "金融経過時間",
        "financialFresh": "金融鮮度",
        "financialSource": "金融ソース",
        "fresh": "新鮮",
        "freshness": "鮮度",
        "indicators": "インジケーター",
        "lastBrokerTicket": "最終ブローカーチケット",
        "lastEval": "最終評価",
        "na": "N/A",
        "never": "なし",
        "noIndicators": "インジケーターデータなし",
        "orderLifecycle": "注文ライフサイクル",
        "orderTruth": "注文の真実",
        "ordersTotal": "注文総数",
        "pendingBrokerOrders": "保留中ブローカー注文",
        "positionsAge": "ポジション経過時間",
        "positionsFresh": "ポジション鮮度",
        "positionsSource": "ポジションソース",
        "scheduleMagic": "スケジュールマジック",
        "stale": "遅延",
        "strategyMagicOrders": "ストラテジーマジック注文",
        "tickCount": "Tick評価",
        "vmBrokerMismatch": "VM数とブローカー数が異なります",
        "vmOrdersTotal": "VM注文総数",
        "windowBars": "ウィンドウBar数"
      },
      "account": "口座",
      "activeTab": "アクティブな実行",
      "closeAll": "全決済",
      "closeFailed": "決済失敗",
      "config": "設定",
      "confirmClose": "このポジションを決済しますか？",
      "confirmCloseAll": "全ポジションを決済しますか？",
      "confirmStop": "このストラテジーを停止しますか？",
      "defaultVolume": "デフォルトロット",
      "diagnostics": "診断",
      "editParams": "パラメータ編集",
      "editStrategy": "戦略編集",
      "error": "エラー",
      "errors": "エラー",
      "goSchedules": "スケジュールへ",
      "health": "ヘルス",
      "historyTab": "実行履歴",
      "lastSignal": "最新シグナル",
      "logs": "ログ",
      "maxDrawdown": "最大ドローダウン %",
      "maxPositions": "最大ポジション数",
      "mode": "モード",
      "mode_live": "ライブ",
      "mode_paper": "ペーパー",
      "myStrategies": "マイ戦略",
      "noActive": "アクティブなストラテジーはありません",
      "noPositions": "ポジションなし",
      "noRuns": "ストラテジーの実行はありません",
      "parameters": "パラメータ",
      "pnl": "損益",
      "positionClosed": "ポジション決済済み",
      "positions": "ポジション",
      "price": "価格",
      "reason": "理由",
      "riskParams": "リスクパラメータ",
      "runId": "実行ID",
      "runStartFailed": "開始に失敗しました",
      "runStarted": "実行を開始しました",
      "schedulesTab": "スケジュール",
      "signalLog": "シグナルログ",
      "signalType": "タイプ",
      "signals": "シグナル",
      "sl": "SL",
      "stale": "遅延",
      "startedAt": "開始時刻",
      "status": "ステータス",
      "stopAndDisable": "停止・無効化",
      "stopFailed": "停止に失敗しました",
      "stopLossOffset": "ストップロスオフセット",
      "stopSuccess": "ストラテジーが停止しました",
      "stoppedAt": "停止時刻",
      "strategyName": "ストラテジー",
      "streamDisconnected": "接続が中断されました。再接続中…",
      "symbol": "銘柄",
      "takeProfitOffset": "テイクプロフィットオフセット",
      "temporaryRuns": "一時実行",
      "time": "時刻",
      "timeframe": "TF",
      "title": "ライブストラテジーモニター",
      "totalSignals": "総シグナル数",
      "tp": "TP",
      "unknownError": "不明なエラー",
      "volume": "数量",
      "waitingSignals": "シグナルを待機中...",
      "watchSignals": "シグナル監視"
    },
    "templates": {
      "scheduleLaunch": {
        "metrics": {
          "maxDrawdown": "最大ドローダウン",
          "sharpe": "シャープレシオ",
          "winRate": "勝率"
        }
      },
      "actions": {
        "create": "新規ストラテジー",
        "delete": "削除",
        "deploy": "デプロイ"
      },
      "codeModal": {
        "title": "コード"
      },
      "detail": {
        "equityCurve": "エクイティカーブ",
        "noDescription": "説明なし",
        "notFound": "ストラテジーが見つかりません",
        "openInWorkspace": "ワークスペースで開く",
        "overview": "概要",
        "parameters": "パラメータ",
        "profitFactor": "プロフィットファクター",
        "tradeStats": "トレード統計"
      },
      "gallery": {
        "aiGenerate": "AI 生成",
        "deleteFailed": "削除に失敗しました",
        "deploy": "デプロイ",
        "empty": "ストラテジーが見つかりません",
        "filterAll": "すべて",
        "filterMine": "自分の",
        "filterSystem": "システム",
        "fork": "Fork",
        "forkEdit": "フォークして編集",
        "forkFailed": "フォークに失敗しました",
        "forkSuccess": "新しいストラテジーにフォークしました",
        "publish": "公開",
        "searchPlaceholder": "ストラテジーを検索...",
        "shared": "Shared",
        "sortRecent": "最近",
        "sortReturn": "リターン",
        "sortRisk": "リスク",
        "sortUsage": "使用状況",
        "system": "システム",
        "title": "ストラテジー",
        "unpublish": "非公開",
        "unpublishFailed": "非公開化に失敗しました",
        "unpublishSuccess": "非公開にしました"
      },
      "messages": {
        "fetchTemplateListFailed": "ストラテジーの読み込みに失敗しました",
        "publishFailed": "公開に失敗しました",
        "templateDeleted": "削除されました"
      },
      "table": {
        "createdAt": "作成日時",
        "status": "ステータス",
        "useCount": "使用回数",
        "visibility": "公開範囲"
      },
      "visibility": {
        "private": "非公開",
        "public": "公開"
      },
      "chatEdit": "チャット編集",
      "confirmDelete": "このストラテジーを削除しますか？",
      "copyAll": "すべてコピー",
      "deleteConfirm": "このストラテジーを削除しますか？",
      "lines": "行",
      "loadFailed": "テンプレートの読み込みに失敗しました",
      "loadOneFailed": "テンプレートの読み込みに失敗しました",
      "noTemplates": "保存済み��トラテジーテンプレートがありません",
      "rename": "名前変更",
      "saveCurrent": "現在のストラテジーを保存",
      "source": "ソース",
      "sourceCode": "ストラテジーソース",
      "title": "ストラテジーテンプレート"
    },
    "schedules": {
      "actions": {
        "runNow": "今すぐ実行"
      },
      "deleteConfirm": {
        "title": "このスケジュールを削除しますか？"
      },
      "status": {
        "disabled": "無効",
        "enabled": "有効",
        "idle": "アイドル",
        "running": "実行中"
      },
      "table": {
        "schedule": "スケジュール"
      }
    },
    "tuning": {
      "noParams": {
        "desc": "Smart を有効にするには、ストラテジーコードに @param アノテーションを追加してください Tuning. Example: // @param fastPeriod 14 range=5:30:5",
        "title": "調整可能なパラメータが検出されません"
      },
      "searchMethod": {
        "grid": "グリッド",
        "random": "ランダム"
      },
      "disabledHint": "ストラテジーコードとシンボルが必要です。サイドバーからストラテジーを選択するか、先にバックテストを実行してください。",
      "failed": "チューニングに失敗しました",
      "noDimsHint": "下で少なくとも 1 つのパラメータ次元を有効にしてください。",
      "qualityGate": "Gate",
      "strategyName": "ストラテジー",
      "totalTrades": "トレード"
    },
    "workspace": {
      "chartIndicators": {
        "overlay": "オーバーレイ（メインチャート）",
        "subPane": "サブペインインジケーター"
      },
      "sidebar": {
        "backtestHistory": "バックテスト履歴",
        "batchDeleteConfirm": "選択したストラテジーを削除しますか？",
        "batchDeleteRunsConfirm": "選択した実行を削除しますか？",
        "deleteRunConfirm": "このバックテスト実行を削除しますか？",
        "deleteStrategyConfirm": "このストラテジーを削除しますか？",
        "myStrategies": "マイストラテジー",
        "newStrategy": "新規ストラテジー",
        "noRuns": "バックテスト実行はまだありません",
        "noStrategies": "ストラテジーはまだありません",
        "title": "ワークスペース",
        "trades": "トレード",
        "viewAll": "すべて表示"
      },
      "tour": {
        "ai": "AI アシスタント",
        "aiDesc": "AI にストラテジーの生成、最適化、デバッグを依頼できます。適用されたコードはエディタに即時反映されます。",
        "backtest": "バックテスト",
        "backtestDesc": "パラメータを設定してバックテストを実行します。エクイティカーブ、トレード統計、リスク指標を表示できます。",
        "code": "コードエディタ",
        "codeDesc": "Write or paste your MQL strategy code here. You can also import .mq4/.mq5 files from the MQL をインポート tab.",
        "save": "保存して公開",
        "saveDesc": "ストラテジーをテンプレートとして保存、マーケットプレイスで公開、またはライブスケジュールにデプロイします。"
      },
      "importMql": "MQL をインポート"
    },
    "aiChat": {
      "codeLoaded": "コンテキスト内のストラテジーコード",
      "historyTab": "履歴",
      "noContext": "No strategy loaded — describe what you want",
      "strategiesTab": "ストラテジー"
    },
    "ai": {
      "explainHint": "コードを書くとAIの説明が表示されます",
      "reviseHint": "まずコードを書いてから、AIに改善を依頼してください",
      "settingsHint": "AIプロバイダーとモデルを設定"
    },
    "chat": {
      "codeGenerated": "コードが生成されました。下のボタンで戦略レビューとバックテストを実行してください。",
      "entry": "エントリー:",
      "executionPlan": "執行計画",
      "exit": "エグジット:",
      "indicators": "インジケーター:",
      "risk": "リスク:"
    },
    "importEA": {
      "aiTranslate": "AI翻訳",
      "analyze": "ストラテジー構造を分析",
      "apply": "エディターに適用",
      "bridge": "盲点ブリッジ",
      "bridgeBtn": "盲点ブリッジ翻訳",
      "bridgeFailedMsg": "エージェントが全ての盲点を自動でブリッジできません",
      "bridgeFailedTag": "ブリッジ失敗",
      "bridgeHint": "MQL4/MQL5 EAコードを貼り付けると、AIが盲点を自動的にPythonサブセットに翻訳します",
      "bridgeSuccess": "ブリッジ成功",
      "bridging": "AIが盲点をブリッジ中...",
      "codeTooShort": "完全なEA/インジケーターのソースコードを貼り付けてください",
      "confirmImport": "インポートを確認",
      "hint": "MQL4/MQL5コードを貼り付け、「分析」をクリックしてください。",
      "importSuccess": "MQLソースコードがインポートされました。「エディターに適用」をクリックしてエディターに書き込んでください。",
      "importTab": "EAのインポート",
      "migration": "ストラテジーインポート",
      "noBridgeNeeded": "カバレッジ100%、ブリッジ不要",
      "pastePlaceholder": "MQL4/MQL5のEAコードを貼り付けてください...",
      "translate": "Goに翻訳",
      "translating": "AI翻訳中...",
      "tryAI": "AI翻訳を試す",
      "writeTab": "ストラテジーコード"
    },
    "schedule": {
      "maxPositionsPlaceholder": "無制限"
    },
    "validate": {
      "allClear": "すべてのチェックを通過しました — 問題は見つかりませんでした。",
      "autoFixFailed": "自動修正に失敗しました",
      "errors": "エラー",
      "failed": "検証に失敗しました",
      "fixWithAI": "エラーをAI修正に送信",
      "hints": "提案",
      "parameters": "パラメータ",
      "passed": "検証に合格しました — 保存のロックが解除されました。",
      "running": "検証を実行中...",
      "warnings": "警告"
    },
    "version": {
      "colActions": "操作",
      "colDate": "日付",
      "colHash": "ハッシュ",
      "colLang": "言語",
      "colSummary": "変更概要",
      "colVersion": "バージョン",
      "diff": "差分",
      "diffFrom": "元",
      "diffTitle": "差分: v{{from}} → v{{to}}",
      "diffTo": "先",
      "empty": "まだバージョン履歴がありません",
      "history": "バージョン履歴",
      "loadDiffFailed": "差分の読み込みに失敗しました",
      "loadFailed": "バージョンの読み込みに失敗しました",
      "loadVersionFailed": "バージョンの読み込みに失敗しました",
      "rollbackConfirm": "v{{n}} にロールバックしますか？",
      "rollbackFailed": "ロールバックに失敗しました",
      "rollbackSuccess": "バージョン {{n}} にロールバックしました",
      "title": "バージョン履歴",
      "viewTitle": "バージョン {{n}}"
    }
  },
  "accounts": {
    "bind": {
      "fields": {
        "alias": "アカウントエイリアス"
      },
      "messages": {
        "changeCredentials": "認証情報を変更する"
      },
      "placeholders": {
        "alias": "任意のカスタム名"
      }
    },
    "messages": {
      "shareLinkCopied": "共有リンクがクリップボードにコピーされました",
      "shareLinkFailed": "共有リンクの作成に失敗しました"
    },
    "status": {
      "circuit_half_open": "サーキットテスト中",
      "circuit_open": "サーキットオープン"
    }
  },
  "autoTrading": {
    "logs": {
      "columns": {
        "action": "アクション",
        "price": "価格",
        "profit": "損益",
        "symbol": "銘柄",
        "ticket": "单号",
        "time": "時間",
        "volume": "数量"
      },
      "empty": "取引ログはまだありません",
      "title": "最近の取引ログ"
    },
    "messages": {
      "loadFailed": "自動取引データの読み込みに失敗しました",
      "toggleFailed": "切换自动交易失败"
    },
    "settings": {
      "maxDailyLoss": "最大日次損失",
      "maxDailyLossHint": "日次損失がこれを超えた場合、自動で取引を無効化",
      "maxDrawdownPercent": "最大ドローダウン%",
      "maxDrawdownPercentHint": "ドローダウンがこれを超えた場合、自動で取引を無効化",
      "maxLotSize": "最大ロットサイズ",
      "maxLotSizeHint": "1取引あたりの最大ボリューム（ロット）",
      "maxPositions": "最大ポジション数",
      "maxPositionsHint": "最大同時オープンポジション数",
      "maxRiskPercent": "最大リスク%",
      "maxRiskPercentHint": "1取引あたりのリスク許容額（残高の％）",
      "saveFailed": "保存设置失败",
      "saveSuccess": "設定を保存しました",
      "title": "グローバルリスク設定"
    },
    "status": {
      "activeStrategies": "アクティブ戦略",
      "disabled": "自動取引が無効です",
      "enabled": "自動取引が有効です",
      "todayExecutions": "本日の約定",
      "todayProfit": "本日の損益"
    },
    "title": "自動取引"
  },
  "marketplace": {
    "publish": {
      "assetClass": {
        "label": "資産クラス"
      },
      "priceModel": {
        "free": "無料",
        "label": "価格設定",
        "once": "買い切り",
        "subscription": "月額サブスクリプション"
      },
      "riskLevel": {
        "label": "リスクレベル"
      },
      "trialDays": {
        "7": "7 日",
        "14": "14 日",
        "30": "30 日"
      },
      "codeSnippet": "ストラテジープレビュー（公開）",
      "codeSnippetPlaceholder": "任意: ストラテジーのスニペットや概要を共有してください（全員に表示されます）",
      "descriptionLabel": "説明",
      "descriptionPlaceholder": "ストラテジーのロジック、エントリー/エグジットルールを説明してください...",
      "includeBacktestSnapshot": "最新のバックテスト結果を含める",
      "priceAmount": "金額",
      "return": "リターン",
      "tags": "タグ",
      "tagsPlaceholder": "タグを入力してEnterキーを押すと追加されます",
      "title": "マーケットプレイスに公開",
      "titleLabel": "タイトル",
      "titlePlaceholder": "例: ゴールデンクロスストラテジー",
      "trades": "トレード",
      "trialDaysCustom": "カスタム日数",
      "trialDaysLabel": "トライアル期間",
      "trialDaysPlaceholder": "カスタム日数を選択または入力",
      "winRate": "勝率"
    },
    "author": {
      "avgRating": "平均評価",
      "empty": "公開された戦略はまだありません。戦略ライブラリで公開してください。",
      "goToLibrary": "ストラテジーライブラリへ",
      "monthlyRevenue": "月間収益",
      "myStrategies": "公開済み戦略",
      "publishNew": "新規戦略を公開",
      "published": "公開済み",
      "totalRevenue": "総収益"
    },
    "backtest": {
      "capital": "資金",
      "commission": "手数料",
      "completed": "完了",
      "equityCurve": "エクイティカーブ",
      "idle": "パラメータを設定してバックテストを実行してください",
      "leverage": "レバレッジ",
      "maxDrawdown": "最大ドローダウン",
      "protected": "ストラテジーコードは保護されています。バックテストは当社サーバーで実行されます。",
      "run": "バックテスト実行",
      "sharpe": "シャープレシオ",
      "title": "ストラテジーバックテスト",
      "totalReturn": "トータルリターン",
      "totalTrades": "総トレード数",
      "winRate": "勝率"
    },
    "card": {
      "by": "by",
      "free": "無料",
      "owned": "購入日",
      "subscribers": "購読者",
      "winRate": "勝率",
      "yourStrategy": "あなたのストラテジー"
    },
    "detail": {
      "assetClass": "資産クラス",
      "author": "作成者",
      "commentPlaceholder": "コメントを書く...",
      "comments": "コメント",
      "description": "説明",
      "getFree": "無料で入手",
      "rentPrice": "¥{{amount}} / 月",
      "runBacktest": "バックテスト実行",
      "subscribers": "購読者",
      "yourRating": "あなたの評価"
    },
    "live": {
      "loadError": "ライブパフォーマンスデータの読み込みに失敗しました"
    },
    "messages": {
      "commentFailed": "コメントに失敗しました",
      "commentPosted": "コメントを投稿しました",
      "loginFirst": "先にログインしてください",
      "paymentComingSoon": "決済機能は近日公開",
      "publishFailed": "ストラテジーの公開に失敗しました",
      "published": "ストラテジーがマーケットプレイスに公開されました！",
      "rateFailed": "評価に失敗しました",
      "rated": "評価を送信しました",
      "subscribeFailed": "失敗",
      "subscribed": "購入に追加しました"
    },
    "optimization": {
      "decayScore": "ディケイスコア",
      "returnDelta": "リターン差分",
      "sharpeDecline": "シャープレシオ低下",
      "trigger": "トリガー",
      "winRateDecline": "勝率低下"
    },
    "payment": {
      "alreadyPurchased": "この戦略は既に購入済みです。",
      "balanceAfter": "購入後残高",
      "cancel": "キャンセル",
      "confirm": "購入確定",
      "deployGuide": "ストラテジーのデプロイ準備ができました。",
      "depositPrompt": "続行するには入金してください。",
      "goDeploy": "今すぐデプロイ",
      "goToDeposit": "入金",
      "insufficientBalance": "残高不足",
      "oneTimePurchase": "¥{{amount}} 買い切り",
      "price": "価格",
      "purchaseFailed": "購入に失敗しました。もう一度お試しください。",
      "purchaseSuccess": "購入完了！戦略がライブラリに追加されました。",
      "purchasing": "処理中...",
      "strategyName": "戦略",
      "title": "購入確定",
      "walletBalance": "残高"
    },
    "purchases": {
      "empty": "購入履歴はまだありません。マーケットで戦略を見つけましょう。",
      "runBacktest": "バックテスト実行",
      "status": "ステータス",
      "strategy": "戦略"
    },
    "sort": {
      "newest": "新着順",
      "performance": "パフォーマンス順",
      "popular": "人気順",
      "priceAsc": "価格：安い順",
      "priceDesc": "価格：高い順",
      "rating": "評価順",
      "score": "総合スコア"
    },
    "tabs": {
      "author": "作成者センター",
      "marketplace": "マーケット",
      "purchases": "購入履歴",
      "subscriptions": "マイ購読"
    },
    "empty": "公開された戦略はまだありません",
    "filterByClass": "資産クラスで絞り込む",
    "noSubscriptions": "購読はまだありません",
    "searchPlaceholder": "戦略を検索...",
    "subtitle": "コミュニティ戦略を発見、購入、利用",
    "title": "ストラテジーマーケット"
  },
  "notifications": {
    "stream": {
      "autoTrading": {
        "fallback": "自动交易事件触发",
        "title": "自動取引"
      },
      "riskAlert": {
        "fallback": "警报类型: {{alertType}}",
        "title": "リスクアラート"
      },
      "strategyExecution": {
        "completed": "{{symbol}} {{action}} が完了しました",
        "failed": "执行失败: {{error}}",
        "title": "戦略実行"
      },
      "strategySignal": {
        "message": "{{symbol}} が {{signalType}} をトリガーしました",
        "title": "戦略シグナル"
      }
    },
    "actions": {
      "clearAll": "クリア",
      "clearAllConfirm": "すべての通知を削除しますか？",
      "markAllAsRead": "すべて既読"
    },
    "prefs": {
      "newRating": "新しい評価またはコメントを受信しました",
      "newStrategy": "新しいストラテジーが公開されました",
      "performance": "ストラテジーのパフォーマンス異常",
      "priceChange": "ストラテジー価格が変更されました",
      "saveFailed": "設定の保存に失敗しました",
      "subExpiring": "サブスクリプションがまもなく期限切れになります",
      "title": "通知設定"
    },
    "tabs": {
      "all": "すべて ({{count}})",
      "unread": "未读 ({{count}})"
    },
    "types": {
      "risk_alert": "リスクアラート",
      "signal": "シグナル",
      "strategy_execution": "戦略",
      "system": "系统",
      "trade": "取引"
    },
    "all": "すべて",
    "clearAll": "クリア",
    "confirmClearAll": "すべての通知を削除しますか？",
    "empty": "通知はありません",
    "markAllRead": "すべて既読",
    "title": "通知",
    "unread": "未読"
  },
  "schedule": {
    "launch": {
      "noAccount": {
        "bindButton": "MT アカウントを紐付け"
      }
    }
  },
  "sre": {
    "breakers": {
      "columns": {
        "lossPercent": "損失率",
        "state": "状態",
        "strategyId": "ストラテジーID",
        "totalPnl": "総損益",
        "tradeCount": "取引回数",
        "tripReason": "遮断理由",
        "trippedAt": "遮断日時"
      },
      "confirmReset": "このブレーカーをリセットしますか？",
      "description": "戦略ブレーカーの状態概要 — 異常な損失を自動検出し、トリップします。",
      "noBreakers": "登録されたブレーカーはありません",
      "stateClosed": "正常",
      "stateHalfOpen": "半開（確認中）",
      "stateOpen": "遮断",
      "title": "戦略ブレーカー"
    },
    "canary": {
      "columns": {
        "accounts": "カナリアアカウント",
        "days": "日数",
        "startAt": "開始日時",
        "status": "ステータス",
        "strategyId": "戦略ID",
        "versionTag": "バージョンタグ"
      },
      "accountIdsLabel": "カナリアアカウントID（カンマまたは改行で区切り）",
      "accountIdsPlaceholder": "account-1, account-2",
      "canarying": "カナリア中",
      "confirmDelete": "このカナリア設定を削除しますか？",
      "description": "新しい戦略バージョンは、数個のアカウントで N 日間実行された後、全体に昇格されます。",
      "durationDays": "カナリア日数",
      "newCanary": "新規カナリア",
      "newCanaryTitle": "新規カナリア",
      "noCanaries": "カナリア設定がありません",
      "promoted": "昇格済み",
      "title": "カナリア設定"
    },
    "killSwitch": {
      "title": "キルスイッチ",
      "undoWindow": "取り消し期間: 残り {{minutes}}分 {{seconds}}秒",
      "confirmEngage": "作動確認",
      "confirmTitle": "キルスイッチ作動 — 確認",
      "confirmWarning": "これにより、保留中や送信済みの注文を含むすべてのアカウントの取引活動が即座に停止されます。理由を入力し、確認のため「KILL」と入力してください。",
      "description": "ワンクリックで全取引を停止 — KILL 確認が必要。5 分以内に取り消し可能です。",
      "disarmed": "キルスイッチ解除済み — 取引は正常です",
      "disengage": "キルスイッチ解除",
      "engage": "キルスイッチ作動",
      "engaged": "キルスイッチ作動中 — 全取引が停止されています",
      "engagedAt": "作動日時",
      "operator": "操作者",
      "reason": "理由",
      "reasonLabel": "理由（必須）",
      "reasonPlaceholder": "例：異常な市場変動を検出し、全取引を緊急停止",
      "status": "ステータス",
      "typeKill": "確認のために「KILL」と入力",
      "typeKillPlaceholder": "KILL と入力（大文字）",
      "undo": "キルスイッチ取消"
    }
  },
  "wallet": {
    "deposit": {
      "table": {
        "amount": "USDT 金額",
        "amountUsd": "USD クレジット",
        "confirmations": "確認数",
        "status": "ステータス",
        "time": "時間",
        "txHash": "TXハッシュ"
      },
      "address": "受取アドレス",
      "addressCopied": "アドレスをコピーしました",
      "amountLabel": "USDT 金額",
      "button": "新規入金",
      "copy": "コピー",
      "exchangeRate": "為替レート",
      "failed": "入金リクエストの送信に失敗しました。",
      "history": "入金履歴",
      "modalTitle": "入金リクエストを送信",
      "network": "ネットワーク",
      "notConfigured": "USDT入金はまだ設定されていません。サポートにお問い合わせください。",
      "notice": "指定されたネットワーク経由でのみUSDTを送信してください。他のトークンの送信や異なるネットワークの使用は永久に失われる可能性があります。",
      "submit": "送信",
      "success": "入金リクエストが送信されました。入金は自動的に確認されます。",
      "title": "入金",
      "txHashLabel": "トランザクションハッシュ（任意）",
      "willCredit": "クレジット予定"
    },
    "passkey": {
      "add": "パスキー追加",
      "confirmRemove": "このパスキーを削除しますか？",
      "createdAt": "作成日時",
      "credentialId": "クレデンシャル ID",
      "name": "名前",
      "namePlaceholder": "例：マイ YubiKey",
      "register": "登録",
      "registerFailed": "登録に失敗しました",
      "registerHint": "パスキーの名前を入力し、登録をクリックして WebAuthn フローを開始します。",
      "registered": "パスキーが登録されました",
      "removed": "パスキーが削除されました",
      "signCount": "署名回数",
      "title": "パスキー管理"
    },
    "table": {
      "amount": "金額",
      "balanceAfter": "調整後残高",
      "description": "説明",
      "time": "時間",
      "type": "種類"
    },
    "txType": {
      "adjustment": "残高調整",
      "deposit": "入金",
      "fee": "手数料",
      "reversal": "取消",
      "withdrawal": "出金"
    },
    "whitelist": {
      "add": "アドレス追加",
      "added": "ホワイトリストアドレスが追加されました",
      "address": "アドレス",
      "addressLabel": "TRC20 アドレス",
      "addressRequired": "アドレスを入力してください",
      "confirmRemove": "このホワイトリストアドレスを削除しますか？",
      "confirmedAt": "確認日時",
      "label": "ラベル",
      "labelLabel": "ラベル（任意）",
      "labelPlaceholder": "例：私の Binance ウォレット",
      "removed": "ホワイトリストアドレスが削除されました",
      "status": "ステータス",
      "title": "ホワイトリスト管理"
    },
    "withdraw": {
      "amount": "金額",
      "amountLabel": "出金金額 (USDT)",
      "amountRequired": "金額を入力してください",
      "available": "利用可能残高",
      "cancelled": "出金がキャンセルされました",
      "confirmCancel": "この出金をキャンセルしますか？",
      "destAddress": "送金先アドレス",
      "destLabel": "送金先 TRC20 アドレス",
      "destRequired": "送金先アドレスを入力してください",
      "failed": "出金に失敗しました",
      "new": "新規出金",
      "noBalance": "出金可能な残高がありません",
      "status": "ステータス",
      "submit": "送信",
      "success": "出金が送信されました",
      "time": "時間",
      "title": "出金",
      "txHash": "トランザクションハッシュ",
      "warning": "出金にはパスキー認証が必要です。送金先アドレスが正しいことを確認してください — ブロックチェーン取引は取り消せません。",
      "whitelist": "ホワイトリスト（クリックで入力）"
    },
    "accountNumber": "口座番号",
    "balance": "残高",
    "currency": "通貨",
    "frozen": "凍結",
    "frozenBalance": "凍結",
    "history": "履歴",
    "title": "マイウォレット",
    "transactions": "取引履歴"
  },
  "agent": {
    "analysis": {
      "consistency": "一貫性",
      "detailed": "詳細分析",
      "drawdown": "DD",
      "observations": "主要な観察",
      "overfitting": "過学習リスク",
      "risk_adj": "リスク調整リターン",
      "sharpe": "シャープレシオ",
      "suggestions": "改善提案",
      "title": "バックテスト分析",
      "winrate": "勝率"
    },
    "profile": {
      "blind_spots": "盲点",
      "coverage": "カバレッジ",
      "entry": "エントリー",
      "exit": "決済",
      "indicators": "インジケーター",
      "regime": "市場レジーム",
      "risk": "リスク管理",
      "strengths": "強み",
      "timeframe": "時間足",
      "title": "ストラテジープロフィール",
      "weaknesses": "弱み"
    },
    "semantic_diff": {
      "effect": "影響",
      "title": "ストラテジーの変更点"
    }
  },
  "auth": {
    "fields": {
      "confirmPassword": "确认密码",
      "email": "メール",
      "login": "メールアドレス / 口座番号",
      "password": "パスワード"
    },
    "forgotPassword": {
      "adminHint": "パスワードをリセットするには、管理者またはサポートにお問い合わせください。",
      "adminTab": "管理者",
      "backToLogin": "返回登录",
      "emailSent": "メールアドレスが存在する場合、リセットリンクが送信されました。",
      "emailTab": "メール",
      "hint": "管理者またはサポートに連絡してパスワードをリセットしてください。",
      "mtFailed": "MT 認証情報の検証に失敗しました。",
      "mtHint": "紐付け済みの MT アカウント認証情報を入力して身元を確認してください。サーバーとプラットフォームは自動検出されます。",
      "mtLogin": "MT アカウント番号",
      "mtLoginPlaceholder": "例: 12345678",
      "mtPassword": "MT パスワード",
      "mtPasswordPlaceholder": "MT 取引パスワード",
      "mtTab": "MT 検証",
      "mtVerified": "身元が確認されました。パスワードリセットへ移動します。",
      "sendResetLink": "リセットリンクを送信",
      "title": "パスワードリセット",
      "verifyAndReset": "確認してパスワードをリセット"
    },
    "login": {
      "forgotPassword": "パスワードをお忘れですか？",
      "login": "ログイン",
      "noAccount": "没有账户？",
      "registerNow": "新規登録",
      "rememberMe": "ログイン状態を保持",
      "signingIn": "ログイン中...",
      "subtitle": "本サービスはテストであり責任を負いません"
    },
    "messages": {
      "fetchMeFailed": "加载用户信息失败",
      "loginFailed": "ログインに失敗しました。メールアドレスとパスワードを確認してください。",
      "loginSuccess": "ログインしました",
      "logoutSuccess": "ログアウトしました",
      "registerFailed": "登録に失敗しました。しばらくしてから再試行してください。",
      "registerSuccess": "登録が完了しました。ログインしてください。"
    },
    "register": {
      "haveAccount": "すでにアカウントをお持ちですか？",
      "loginNow": "ログイン",
      "register": "登録",
      "signingUp": "登録中...",
      "subtitle": "新規アカウント作成"
    },
    "resetPassword": {
      "confirmPassword": "パスワードの確認",
      "confirmRequired": "パスワードを確認してください",
      "failed": "パスワードのリセットに失敗しました。",
      "invalidToken": "リセットトークンが無効または存在しません。",
      "mismatch": "パスワードが一致しません。",
      "newPassword": "新しいパスワード",
      "submit": "パスワードをリセット",
      "success": "パスワードがリセットされました。新しいパスワードでログインしてください。",
      "title": "新しいパスワードを設定"
    },
    "validation": {
      "confirmPasswordRequired": "パスワードを確認してください",
      "emailInvalid": "有効なメールアドレスを入力してください",
      "emailRequired": "メールアドレスを入力してください",
      "loginRequired": "メールアドレスまたは口座番号を入力してください",
      "passwordMin8": "パスワードは8文字以上必要です",
      "passwordMismatch": "パスワードが一致しません",
      "passwordRequired": "パスワードを入力してください"
    }
  },
  "common": {
    "months": {
      "jan": "1月",
      "jul": "7月"
    },
    "time": {
      "day": "{{n}}天",
      "hour": "{{n}}时",
      "lessThanMinute": "<1分钟",
      "minute": "{{n}}分"
    },
    "action": "操作",
    "active": "アクティブ",
    "back": "戻る",
    "cancel": "キャンセル",
    "clear": "清除",
    "close": "決済",
    "comingSoon": "即将上线",
    "confirm": "確定",
    "copied": "コピーしました",
    "copy": "コピー",
    "copyFailed": "コピーに失敗しました",
    "create": "新規",
    "created": "作成しました",
    "currentPosition": "📊 現在のポジション",
    "currentPrice": "現在価格",
    "delete": "削除",
    "deleteFailed": "削除に失敗しました",
    "deleteSelected": "選択した{{count}}件を削除",
    "deleted": "削除しました",
    "disable": "無効化",
    "disabled": "已禁用",
    "duplicateName": "名前が重複しています",
    "edit": "編集",
    "enable": "有効化",
    "enabled": "有効化しました",
    "error": "エラー",
    "failed": "失敗",
    "false": "いいえ",
    "gotIt": "了解",
    "hideDetails": "詳細を隠す",
    "inactive": "停用",
    "indicatorSettings": "{{name}} 設定",
    "lineColor": "ライン色",
    "loading": "読み込み中...",
    "loadingFailed": "読み込みに失敗しました",
    "message": "メッセージ",
    "next": "次へ",
    "no": "否",
    "noData": "データがありません",
    "noOpenPositionsForSymbol": "{{symbol}} のポジションはありません",
    "none": "なし",
    "off": "オフ",
    "ok": "OK",
    "on": "オン",
    "openPrice": "建値",
    "operationFailed": "操作失败",
    "pageError": "ページエラー",
    "pageUnderDevelopment": "此页面开发中",
    "pleaseWait": "しばらくお待ちください...",
    "previous": "戻る",
    "refresh": "更新",
    "remove": "移除",
    "required": "必須",
    "reset": "リセット",
    "retry": "リトライ",
    "save": "保存",
    "saveFailed": "保存に失敗しました",
    "saveSuccess": "保存成功",
    "saved": "保存済み",
    "saving": "保存中...",
    "searching": "検索中...",
    "selectSymbolToViewChart": "銘柄を選択してチャートを表示",
    "selected": "選択済み",
    "send": "送信",
    "showDetails": "詳細を表示",
    "status": "ステータス",
    "step1Label": "ブローカー",
    "step2Label": "認証情報",
    "step3Label": "確認",
    "success": "成功",
    "total": "合計 {{total}}",
    "totalItems": "共 {{count}} 项",
    "translate": "翻訳",
    "true": "はい",
    "unexpectedError": "予期しないエラーが発生しました",
    "unit": "単位",
    "unknown": "未知",
    "unknownError": "不明なエラー",
    "unsaved": "未保存",
    "updated": "更新しました",
    "viewOriginal": "原文を見る",
    "viewTranslation": "翻訳を見る",
    "yes": "是",
    "you": "你"
  },
  "dashboard": {
    "quickActions": {
      "aiStrategy": "AI戦略"
    },
    "noAccountsDesc": "監視と取引を開始するには、最初の MT4/MT5 アカウントを紐付けてください。"
  },
  "errors": {
    "ai": {
      "api_key_required": "API Key は必須です",
      "base_url_required": "Base URL は必須です",
      "base_url_scheme_invalid": "Base URL は http:// または https:// で始まる必要があります",
      "base_url_should_not_end_with_chat_completions": "Base URL は /chat/completions で終わらないようにしてください",
      "config_service_not_initialized": "AI 設定サービスが初期化されていません",
      "config_valid": "AI 設定は有効です",
      "failed_to_create_request": "リクエストの作成に失敗しました",
      "forbidden_quota": "配额超限",
      "free_tier_exhausted": "AI の無料枠が上限に達しました。プロバイダー管理画面で「無料枠のみ使用」を無効化するか、有料キーに切り替えてください。",
      "insufficient_balance": "AIウォレットの残高が不足しています。続行する前にチャージしてください。",
      "insufficient_balance_title": "残高不足",
      "invalid_base_url": "Base URL が無効です",
      "invalid_provider": "無効なプロバイダです",
      "no_trade_data_available": "利用可能な取引データがありません",
      "not_configured": "AI が設定されていません。先に AI 設定で有効化・設定してください。",
      "probe_ok": "OK",
      "probe_ok_no_models": "OK（model が返されませんでした）",
      "provider_required": "プロバイダを選択してください",
      "provider_returned_empty_message": "AI プロバイダが空のメッセージを返しました",
      "rate_limited": "AI サービスがレート制限/クォータ不足（429/資源枯渇）。しばらく待つか、利用可能な API Key/model に切り替えてください。",
      "request_failed": "API リクエストに失敗しました"
    },
    "connection_failed": {
      "content": "无法连接到服务器，请检查网络后重试。",
      "title": "接続に失敗しました"
    },
    "access_denied": "アクセスが拒否されました",
    "account_connected": "接続しました",
    "account_connection_failed": "取引サーバーへの接続に失敗しました",
    "account_not_found": "口座が見つかりません",
    "auto_trading_disabled": "自動売買を無効にしました",
    "auto_trading_enabled": "自動売買を有効にしました",
    "email_already_registered": "このメールアドレスは既に登録されています",
    "invalid_credentials": "認証情報が正しくありません",
    "not_authenticated": "認証されていません",
    "schedule_service_not_available": "スケジュールサービスは利用できません",
    "translate_failed": "翻訳に失敗しました",
    "user_not_found": "ユーザーが見つかりません"
  },
  "importAnalysis": {
    "execution": {
      "onBar": "バー終了時イベント駆動",
      "onInitGrid": "グリッド初期化",
      "onTick": "ティック駆動"
    },
    "sizing": {
      "fixed": "固定ロット",
      "martingale": "マーチンゲール",
      "percentBalance": "残高に対する割合"
    },
    "analyzing": "戦略構造を分析中...",
    "cannotImport": "自動インポート不可",
    "coverageTitle": "インポートカバレッジ",
    "emptyAnalysisDesc": "ストラテジーロジックが認識されませんでした。ソースコードが不完全か、別の言語が使用されている可能性があります。",
    "goodCoverage": "インポートカバレッジは良好です",
    "goodCoverageDesc": "戦略のメインロジックを認識。安全にインポートできます。使用前にパラメータリストを確認してください。",
    "guiNoiseDesc": "以下の盲点はチャート表示/ボタン機能で、サーバー側の実行ではスキップされ、取引結果に影響しません。安全にインポートできます。",
    "handling": "ハンドリング",
    "incompleteCoverage": "取引ロジックのカバレッジが不完全",
    "location": "位置",
    "noBlindSpots": "確認不要のロジック",
    "noBlindSpotsDesc": "すべてのストラテジーロジックが自動認識されました。安全にインポートできます。",
    "tradeLogicComplete": "取引ロジックを完全に認識しました",
    "userActionRequired": "操作が必要です"
  },
  "logs": {
    "result": {
      "pass": "合格",
      "reject": "却下"
    },
    "triggerSource": {
      "manual": "手動",
      "recovery": "リカバリー",
      "strategy": "戦略"
    }
  },
  "onboarding": {
    "step1": {
      "action": "アカウント連携",
      "desc": "MT4/MT5 取引アカウントを連携して開始。",
      "title": "アカウントを連携"
    },
    "step2": {
      "action": "ワークスペースを開く",
      "desc": "AI を使って自然言語から取引戦略を生成。",
      "title": "最初の戦略を作成"
    },
    "step3": {
      "action": "プランを見る",
      "desc": "より多くの AI トークン、戦略、ライブ取引をアンロック。",
      "title": "プランをアップグレード"
    },
    "dismiss": "了解、閉じる",
    "subtitle": "3つの簡単なステップで始めましょう"
  },
  "subscription": {
    "feature": {
      "aiTokens": "{{count}} AIトークン/月",
      "backtests": "{{count}} バックテスト/日",
      "liveStrategies": "{{count}} ライブ戦略",
      "strategies": "{{count}} 戦略",
      "symbols": "{{count}} シンボル/戦略",
      "unlimitedAccounts": "無制限の MT アカウント"
    },
    "accountBroker": "ブローカー",
    "accountLogin": "ログイン",
    "accountServer": "サーバー",
    "accountStatus": "ステータス",
    "accountType": "タイプ",
    "activeStrategies": "アクティブ戦略",
    "aiTokens": "AIトークン",
    "aiTokensRemaining": "残り AI トークン",
    "autoRenew": "自動更新",
    "billingCycle": "請求",
    "boundAccountsCount": "紐付け済みアカウント",
    "boundAccountsTitle": "紐付け済み MT アカウント",
    "boundAt": "紐付け日時",
    "cancelAutoRenew": "自動更新をキャンセル",
    "cancelFailed": "キャンセルに失敗しました。再試行してください。",
    "cancelSuccess": "自動更新がキャンセルされました。現在の期間終了までサブスクリプションは有効です。",
    "changeFailed": "プラン変更に失敗しました。再試行してください。",
    "changePlanTitle": "プラン変更",
    "changeSuccess": "プラン変更成功！",
    "chargeNotice": "有料プランはウォレットから引き落としされます。無料プランは課金されません。",
    "charged": "引き落とし: {{amount}}, 残高: {{balance}}",
    "choosePlan": "プランを選択",
    "currentPlan": "現在のプラン",
    "freeForever": "永久無料",
    "insufficientBalance": "ウォレット残高が不足しています。先にチャージしてください。",
    "month": "月",
    "monthly": "月額",
    "noBoundAccounts": "まだ紐付け済みアカウントがありません。ストラテジーをスケジュールすると自動的に紐付きます。",
    "noPlans": "利用可能なプランがありません",
    "period": "現在の期間",
    "runtimeMinutes": "実行時間（分）",
    "selectBillingCycle": "請求サイクル",
    "subscribeFailed": "サブスクリプションに失敗しました。再試行してください。",
    "subscribeSuccess": "サブスクリプションが有効になりました！",
    "subscribeTitle": "プランに登録",
    "title": "サブスクリプションプラン",
    "unbind": "紐付け解除",
    "unbindConfirm": "このアカウントの紐付けを解除しますか？有効なスケジュールは停止されます。",
    "unbindFailed": "アカウントの紐付け解除に失敗しました。",
    "unbindSuccess": "アカウントの紐付けを解除しました。",
    "usageTitle": "今月の使用量",
    "walletBalance": "ウォレット残高",
    "year": "年",
    "yearly": "年額"
  },
  "symbolDetection": {
    "tradeMode": {
      "disabled": "已禁用",
      "longOnly": "仅做多",
      "longShort": "多空双向",
      "shortOnly": "仅做空",
      "unknown": "未知"
    },
    "label": "検出された銘柄",
    "loading": "解析中...",
    "noSymbols": "取引銘柄が検出されませんでした。具体的な銘柄名を含めてみてください（例：「Bitcoin」「EURUSD」「Gold」）",
    "resolvedTooltip": "ブローカー：{{broker}} | モード：{{mode}}",
    "unresolvedTooltip": "取引口座が未バインドのため、解決できません"
  },
  "analytics": {
    "pnl": "損益:"
  },
  "app": {
    "name": "AlphaForge"
  },
  "landing": {
    "brokersDesc": "IC Markets、Pepperstone、XM、Exness、OANDA、FXTM、FBS、OctaFX、HotForex、Alpari、RoboForex など。既存のブローカーアカウントは数秒で接続できます。",
    "brokersLink": "対応ブローカーをすべて見る",
    "brokersTitle": "30 以上の MT4/MT5 ブローカーに対応"
  },
  "language": {
    "english": "English",
    "japanese": "日本語",
    "simplifiedChinese": "簡体中国語",
    "traditionalChinese": "繁体中国語",
    "vietnamese": "Tiếng Việt"
  },
  "market": {
    "allSymbols": "全銘柄",
    "ask": "売値",
    "bid": "買値",
    "common": "共通",
    "emptyWatchlist": "暂无自选",
    "loadingSymbols": "読み込み中...",
    "mid": "仲値",
    "mtSessionLost": "⚠ MT セッション切断 — 再接続中…",
    "noSymbolSelected": "銘柄を選択してマーケットデータを表示",
    "noSymbolsFound": "銘柄が見つかりません",
    "popularSymbols": "人気銘柄",
    "searchPlaceholder": "銘柄を検索（例: EURUSD, XAUUSD）",
    "searchSymbol": "搜索品种...",
    "selectAccount": "取引口座を選択",
    "selectSymbol": "銘柄を選択",
    "spread": "スプレッド",
    "watchlist": "ウォッチリスト"
  },
  "menu": {
    "accounts": "アカウント",
    "aiAssistant": "AIアシスタント",
    "algoDashboard": "アルゴダッシュボード",
    "analytics": "分析",
    "assetAnalysis": "AI分析",
    "assets": "戦略資産",
    "autoTrading": "自動取引",
    "dashboard": "ダッシュボード",
    "devGroup": "戦略開発",
    "experiments": "戦略実験",
    "indicatorCatalog": "インジケーターカタログ",
    "logs": "システムログ",
    "market": "マーケット",
    "marketRegime": "マーケットレジーム",
    "marketTools": "マーケット分析ツール",
    "marketplace": "マーケットプレイス",
    "opsGroup": "戦略運用",
    "schedules": "戦略スケジュール",
    "strategies": "戦略管理",
    "strategy": "戦略",
    "strategyLibrary": "戦略ライブラリ",
    "strategyLive": "ライブ監視",
    "strategyWorkspace": "戦略ワークスペース",
    "subscription": "サブスクリプション",
    "trading": "取引",
    "wallet": "ウォレット"
  },
  "monitoring": {
    "acquired": "取得済み",
    "avgGapSec": "平均ギャップ (秒)",
    "bidGtAsk": "Bid>Ask",
    "consumerLag": "コンシューマーラグ",
    "database": "データベース",
    "dbPool": "DB接続プール",
    "deadAccounts": "停止アカウント",
    "disconnected": "切断",
    "diskUsage": "ディスク使用量",
    "dlq": "デッドレターキュー (DLQ)",
    "droppedBars": "ドロップされたバー",
    "droppedSignals": "ドロップされたシグナル",
    "gcCount": "GC回数",
    "gcPauseAvg": "GC停止平均",
    "goRuntime": "Goランタイム",
    "goroutines": "Goroutine",
    "healthy": "OK",
    "heapMemory": "ヒープメモリ",
    "idle": "アイドル",
    "lastUpdate": "最終更新",
    "maxGapSec": "最大ギャップ (秒)",
    "mdGateway": "MDゲートウェイ",
    "nonPositive": "非正",
    "parseErrors": "解析エラー",
    "pushInterval": "プッシュ間隔: 5秒",
    "serviceHealth": "サービス健全性",
    "spillFiles": "スピルファイル",
    "sseConnected": "SSE接続済み",
    "stackUsage": "スタック使用量",
    "staleAccounts": "滞留アカウント",
    "streamError": "ストリームエラー",
    "title": "システム監視",
    "totalConns": "合計",
    "unknown": "不明",
    "uptime": "稼働時間",
    "uptimeDays": "{{d}}日 {{h}}時間",
    "uptimeHours": "{{h}}時間 {{m}}分",
    "uptimeMinutes": "{{m}}分 {{s}}秒",
    "uptimeSeconds": "{{s}}秒",
    "waitingData": "データ待機中..."
  },
  "profile": {
    "lastLogin": "最終ログイン",
    "nickname": "ニックネーム",
    "registered": "已注册",
    "role": "役割",
    "status": "ステータス",
    "title": "プロフィール"
  },
  "share": {
    "actions": "操作",
    "createNew": "新しい共有リンクを作成",
    "createdAt": "作成しました",
    "deleteConfirm": "删除此分享链接？",
    "empty": "共有リンクはまだありません",
    "expires": "有効期限",
    "positions": "持仓",
    "showPositions": "显示持仓",
    "title": "共有管理",
    "token": "共有リンク",
    "userId": "ユーザー",
    "views": "閲覧数"
  },
  "sharePage": {
    "avgHolding": "平均保有時間",
    "avgLoss": "平均損失",
    "avgWin": "平均利益",
    "bestTrade": "ベストトレード",
    "bySymbol": "銘柄別成績",
    "closeTime": "決済",
    "count": "取引数",
    "countUnit": "件",
    "disclaimer": "過去の実績は将来の成果を保証するものではありません。",
    "equityCurve": "資産曲線",
    "expired": "この共有リンクは期限切れです",
    "footer": "AlphaForge により生成",
    "language": "言語",
    "loadFailed": "共有データの読み込みに失敗しました",
    "losingTrades": "負けトレード数",
    "maxDrawdown": "最大ドローダウン",
    "netProfit": "純損益",
    "noPositions": "暂无持仓",
    "noTrades": "取引履歴がありません",
    "notFound": "見つかりません",
    "openPrice": "开仓价",
    "positions": "当前持仓",
    "positionsLocked": "创建者未开放持仓查看",
    "profit": "損益",
    "profitFactor": "プロフィットファクター",
    "sharpeRatio": "シャープレシオ",
    "side": "売買",
    "subtitle": "実際の取引成績",
    "symbol": "銘柄",
    "title": "取引パフォーマンス",
    "totalReturn": "純損益",
    "totalTrades": "総取引数",
    "totalVolume": "総取引量",
    "tradeRecords": "取引履歴",
    "volume": "数量",
    "winRate": "勝率",
    "winningTrades": "勝ちトレード数",
    "worstTrade": "ワーストトレード"
  },
  "theme": {
    "switchToDark": "ダークモードに切替",
    "switchToLight": "ライトモードに切替"
  },
  "topbar": {
    "logout": "ログアウト",
    "profile": "プロフィール",
    "settings": "設定",
    "switchToAdmin": "管理画面へ切替",
    "systemOk": "システムは正常に稼働中",
    "user": "ユーザー"
  }
} as const;
export default Base;
