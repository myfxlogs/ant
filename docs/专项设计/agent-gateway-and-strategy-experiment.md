...
具体实施依据拆分到专项文档。本文档保留总纲、能力边界、阶段路线与禁止事项；proto、数据库字段、RPC 草案、验收标准与迁移步骤以专项文档为准。
...
| 专项文档 | 职责 |
|----------|------|
| `docs/domains/Agent-Gateway-专项设计.md` | Agent token、scope、allowlist、限流、审计、幂等与只读 facade 边界。 |
| `docs/domains/通用Job与事件流专项设计.md` | 长任务提交、Job 状态机、事件流、Connect stream / SSE 与幂等边界。 |
| `docs/专项设计/策略实验与评分专项设计.md` | 参数空间、候选生成、批量回测、多因子评分、LLM 多轮优化与前端职责边界。 |
| `docs/专项设计/策略资产库专项设计.md` | 内部策略资产库、发布审核、克隆、版本、同步提示与 Agent/MCP 权限边界。 |
| `docs/domains/交易执行适配器专项设计.md` | MT4/MT5 ExecutionAdapter 行为等价治理、错误归一、成交回报与审计 metadata。 |
| `docs/domains/MCP工具面专项设计.md` | MCP 工具清单、scope 映射、Agent facade 映射、安全审计与部署形态。 |
| `docs/domains/市场状态与K线缓存专项设计.md` | Market Regime、K 线缓存、只读数据源工厂与回测 metadata 边界。 |

后续涉及新增 `.proto`、ConnectRPC service、数据库表、前端页面或策略服务计算时，应先引用或更新对应专项文档，再进入代码实现。
