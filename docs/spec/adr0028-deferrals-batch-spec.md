...
> **Phase**：mature（防线 A/B 落地 + 有内测真实用户后；需人导 trace）。
...
| Part | Phase | 何时做 | 依赖 |
|------|-------|--------|------|
| **B 用户侧恢复前端** | near-term | 现在可做（防线输出已就绪） | 无（纯前端 + 复用 agent bridge）|
| **A 判决型误报处置** | post-launch | 有真实误报数据后 | Part B 的前端反馈入口可复用 |
| **C MT golden trace** | mature | 内测用户后 | **需人导 MT trace**（一次性）|

> 建议：**B 先做**（提升防线可用性，用户能看懂诊断 + AI 修复闭环）；A 等 post-launch 误报数据；C 等内测 + 人导 trace。三者独立，不必同时。
