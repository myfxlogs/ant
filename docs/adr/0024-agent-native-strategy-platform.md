...
### Phase 4: 知识库 + 策略进化 (~3.5 周) — 🤖 见 ADR-0025 实施方案

**目标：** 经验积累 + 实盘监控 + 自动优化 + Agent 交互体验

> **本 Phase 的详细设计由 ADR-0025 定义。** 以下为概览，具体 schema、优先级、交付物清单见 0025。

| 工作项 | 周期 | 对应 ADR-0025 |
|---|---|---|
| pgvector 知识库 schema + 语义检索 + 结构指纹 | 3 天 | §4.2 |
| 三层记忆系统（全局领域知识 + 用户模板 + Agent 自动经验）+ MEMORY.md 索引 + 会话启动注入 | 3 天 | §4.1-§4.5 |
| 领域知识注入 + Admin 管理端领域知识页 | 2 天 | §7 |
| 策略回溯 Agent（聚焦版 Nudge Engine） | 5 天 | §6 |
| SSE 退化告警 + 前端通知 | 3 天 | §8 `DegradationAlert` 钩子 |
| Plan Mode 前端交互卡片 | 3 天 | §3 |
| 端到端测试 | 4 天 | — |

**产出：** 自我学习进化能力 + 非程序员用户可用的策略设计交互

### 总周期：13-14 周（Phase 4 工期缩短 ~0.5 周，总周期基本不变）
