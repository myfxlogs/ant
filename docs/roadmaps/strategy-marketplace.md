...
|--------|------|----------|----------|------------------------|
| 散户交易者 | 千万级 | 弱（不写代码） | 低→**中（买策略愿意付费）** | ✅ 策略市场目标买方 |
| MQL 脚本开发者 | 十万级 | 中 | 低 ($30-500/EA) | ⚠️ 过度设计 |
| **专业 MQL 量化开发者** | 万级 | **强** | 中高 | ✅ **核心策略提供者** |
| 跨市场量化团队 | 千级 | 强 | 高 | ✅ **最佳匹配** |
| 机构量化 | 极小 | 弱（不用 MQL） | 高但不匹配 | ❌ 不会离开 Python 生态 |

**策略市场同时覆盖了散户（买方）和专业 MQL 开发者（卖方），形成双边网络效应。** 这是纯开发者工具模型无法触达的市场面。
...
| 风险 | 严重度 | 应对 |
|------|--------|------|
| 回测美化（实盘表现远差于回测） | 🔴 | 标准化回测+实盘跟踪记录+第三方验证（Phase 1.1） |
| 策略亏损法律责任 | 🔴 | 明确免责声明+风险提示+"过往业绩不代表未来表现"（Phase 1.4） |
| 冷启动（无买方↔无卖方） | 🟡 | AI 生成第一批策略（Phase 2），先让买方有东西可买，再吸引提供者 |
| 信任鸿沟（用户不信任 AI 生成的策略） | 🟡 | 实盘业绩公开+提供者验证+回测质量门槛（Phase 1） |
