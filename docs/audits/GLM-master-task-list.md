...
### P1-6 · 多策略共账户（Pro 档容量冲突）✅ 已完成

| # | 内容 | 状态 |
|----|------|------|
| P1-6a | **商业 bug**：`session_registry.go:116` 一账户一会话，Pro 档卖"5 账户/20 实盘策略"(`strategy-marketplace.md:172`) → 付费用户最多跑 5 个实盘策略 | ✅ 已解决（决策 A/B 落地，见 P1-6b）|
| P1-6b | 多策略共账户归因闭环（step⑥）：trade_records.schedule_id 按 magic 回填 | ✅ **①-⑥ 全部完成**（2026-08-08 审计方验收）：①-⑤ commit `e47ea7bb`（magic 打标 / close_all 隔离 / 多 session；风控 account 级 = 决策 B）；**⑥ commit `00e5ccc1`**（`ResolveScheduleIDByMagic` account-scoped + 共享 helper 回填 schedule_id）。残留（低）：DB 级集成测待补（仅 unit+stub），登记 `tech-debt-registry.md` ARCH-4 |
