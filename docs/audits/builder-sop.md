...
每次施工必须同时满足（CLAUDE.md「因困难而妥协最优解视为违规」的展开）：

1. **架构最优解**：当前约束下最合理的架构。不堆砌、不过度设计、不照搬别处模式。
2. **实现最优解**：最简最可靠的实现路径。不绕弯、不重复造轮子（先 `cap.sh` 查）。
3. **第一性原理**：从根本需求推导，而非"别人这么写 / 上次这么写 / 框架默认"。
4. **干净无冗余**：无死代码、无 unused import/变量、无 commented-out、无残留调试代码。
5. **无 BUG**：逻辑正确 + 边界全覆盖（空/nil/极值/负数/并发/重试/精度 Decimal）。
6. **无技术债**：无 TODO/FIXME/HACK、无"暂时这样后面再改"。**要么做对要么不做**——快捷方式（回退代替重新生成 / 标记 legacy / 沉默代替修复）= 违规。
7. **完全合规**：CLAUDE.md 全部约束——proto+ConnectRPC+SSE、decimal（禁 float64 价格）、push-first（禁轮询）、禁 JSON/WS/REST/nolint/ts-ignore、file-lines 红线（Go>450/TS>375）、确定性（禁 `time.Now` 测试 / 裸 map 迭代）。
8. **YAGNI + 不偷懒**：不提前设计用不到的（YAGNI），但该有的防御/抽象不能省（不偷懒）。

**自检**：完工后对着这 8 条 + 审计方验收 5 维（意图理解 / 可演进性 / 测试质量 / 防御性 / 克制）过一遍。任何一条不达标，回去改，**不要带着债提交**。
...
> **三层无损接手**：`tech-debt-registry.md` + `handover-audit-plan.md` 变更日志 + `memory/` 三层，是"任何一方休息、另一方读三层就能完整恢复上下文"的保证。所以回填铁律是 **标 ✅ 保留，永不删除**（详见 2.6）——删一条 = 接手方少一块拼图 = 有损接手。
...
### 2.6 标记 ✅ 不删除（无损接手铁律 — 三层通用）

完工项**标 ✅ 保留行，永不删除**。适用于三层所有"行式"记录：

- **`tech-debt-registry.md`**：`🟦open → ✅done`（标日期），行保留，追加根因/修复/对抗证明/测试结果（见 2.1）。✅ 行 = 已验事实 + 决策轨迹，删了无据可查。
- **memory（`open-items-registry.md`）**：完工项标 ✅ + 指向 docs（如 `✅ ARCH-4 全✅，详见 registry ARCH-4`），**不删行**。memory 是指针，精简靠"✅ + 指向"，不靠删。
- **`handover-audit-plan.md` 变更日志**：本就 append-only（只追加新行，不改/删旧行）。

**为什么**：审计方（Claude Code）/ 施工方（Windsurf）/ 任意后续 agent 三方靠这三层**无损接手**——任何一方休息，另一方读完三层就知道"做了什么 / 为什么 / 验过没"。删一条完工记录 = 下一个人不知道这事已做且已验 = 重复劳动或误判。**删了还以为没做，比没做更糟。**

**完工时正确动作**：状态列 `→ ✅done(日期)` + 追加证据；子项过时则划删除线标 ✅（`~~旧描述~~✅`），**行留下**。需要"瘦身"时缩文字 + 加指向，绝不删整行。**🆕 2026-08-11 修订（用户批准，省 token）**：✅done 明细行允许归档 git（文件只留状态 + 近 3 条 changelog + "靠 git 追溯"指针，指针必须写明）；open/返工中条目 + 根因 + 对抗测试记录 + changelog 追加仍必留文件内。
