...
| I18N-MIXED-2 | zh-tw/ja/vi locale 同类未译缺口（I18N-MIXED-1 设计实查时发现）：zh-tw base 212 条多词英文未译（比 zh-cn 更重）、ja/vi 同类 + vi/zh-tw 另有缺字段（`i18n-check --strict` 全量 1519 errors 主要来源）。业主指令"信息以中文显示"优先 zh-cn/zh-tw；vi/ja 排期待裁定。修复路径同 I18N-MIXED-1（DICT 表需 per-locale 重建——translate 脚本仅 zh-cn）。 | 🟦open（2026-09-19 登记）|
