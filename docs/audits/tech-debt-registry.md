...
| MQL-LOOP-4 | **P2-T4/T5 扩展（useAIFix 扩到 coverage fatal + T5 实盘门控）——2026-09-19 Devin CLI 独立复审：实现早已在库，"P2 暂缓"系登记时未察觉 commit `0ef2638b`（2026-08-08 MQL Pipeline Completion 5/5）已落地所致的条目漂移**。**T5 实盘门控实证**：`CheckLiveCoverage`（`internal/marketplace/quality.go:343` 双路——strategy_id 存在→最新 SUCCEEDED 回测 `checkUnreliableCoverage` fatal 盲区+`checkDegradedStatus` 不变量违例；无 ID→`checkSourceCoverage` 经 `CompileMQLWithCoverage` 查 fatal severity）经 `SetCoverageChecker(d.mktplaceSvc)`（`cmd/server/handlers_strategy.go:127`）注入，`preflightLiveChecks`（`live_runner.go:118-123`）在 RunLiveStrategy 共享 chokepoint 执行——StartStrategy/launchEventSession/dispatch 三路收敛不可绕过，modeLive-only，paper/nil-checker 跳过。**T4 useAIFix 实证**：`useAIFix.tsx` 已处理 fatal severity 盲区（`buildBlindSpotInstruction` fatal 过滤→Critical 区），作者审 diff Modal（不可跳过作者审查），`updateCode`→`onRerunBacktest` 自动重回测——spec 流程 revise→审diff→保存→重回测全环。**同 commit 未登记兄弟项全部实证在库**：CI golden regression（`golden_regression_test.go` 3 EA+`AdversarialDeleteConstant`）、K3 demand signal（migration 270+`RecordDemandSignal` 接线 quality.go:175+`demand_test.go`）、coverage display（coverageScore 于 ImportEAPanel/CodeEditorArea）、C1 compound interest（`c1_compound_interest_test.go`）。**独立对抗验证（2026-09-19）**：原 `RejectsLive` 为弱 pin——摘门（`&& false`）后仍 PASS（下游 `no BarSource configured` 错误满足 `err!=nil`，判别失效）；审计方补强断言 `contains("fatal coverage")`→同 mutation 精确 RED（`expected 'fatal coverage' rejection, got unrelated error`）→restore→4/4 GREEN。测试独立重跑：T5×4/CI golden×4/KB×4 全绿。 | ✅done（Devin CLI 验收 2026-09-19——实现+弱 pin 补强+mutation 实证）|
...
| POST-2 | 性能/容量压测（下单/回测/SSE）。**2026-09-19 设计实查**：本机即生产（compose 全栈+实盘账户）→全量压测不可行；容量模型代码实查：回测 3-worker+SKIP LOCKED+lease 有界、per-user 配额有界、**SSE 扇出无界**（待探针量化）、下单真路径不可测于本机。派工探针批（in-process 零触生产）：VM 并发退化曲线+SSE 扇出成本+paper 下单延迟+容量基线文档。**🟦open（施工完成，待独立复审 2026-09-19）**：S1 `BenchmarkVMExec_Concurrency{N=1,4,8,16}`（`vm_bench_test.go`，独立 VM 实例共享只读 bytecode，4vCPU 上 N=4 吞吐 1.83×单核、N≥8 平台期 ~30k ticks/s≈2.1×，3-worker 排队量化：N=16 并发 1000-tick 回测 ~0.53s vs 单独 68ms）；S2 `TestSSEFanoutCostCurve`（`internal/paper/sse_fanout_probe_test.go`，真 PaperEngine+httptest 复刻 WatchPaperAccount 循环，N=500 投递 p99 19.9ms、每流成本恒定 ~2 server goroutine+~45-50KB[生产+keepalive 1 goroutine]）；S3 `TestPlacePaperOrderLatency_Concurrency`+`BenchmarkPlacePaperOrder`（`paper_order_probe_test.go`，串行 6.2µs/单≈16 万单/s，并发封顶 ~34 万单/s，瓶颈在下游 DB 非引擎）；S4 `docs/benchmarks/post2-capacity-baseline-2026-09.md`（三轴数字+容量模型表+缺口+staging 7 项）。**对设计表修正**：SSE 并非无 limiter 而是形态错位——`SSEStreamLimitMiddleware(5)` 已接线（`main.go:276`）但 `isSSERequest` 只匹配 `text/event-stream`，前端主形态 ConnectRPC binary（`application/connect+proto`，`frontend/src/client/connect.ts:97`）不过 limiter→净效果无界，修法=limiter 纳入 ConnectRPC 形态；另发现 broadcast chan(8) 突发 drop-slow-consumer 为快照语义自洽（非债）。门禁：build/vet/test/bench/check-lines(0 errors)/diff-check 全绿+race×3（paper 9.6s 绿）。边界遵守：零触容器/生产端口/mtapi/DB。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——施工 2408d34c：S1/S2/S3 探针独立重跑全绿+race 绿+check-lines 0 errors+diff --check 净；S2 探针复刻真 WatchPaperAccount 循环+真 PaperEngine 原语、口径注释诚实、判别断言存在；**设计表修正独立核实成立**：sse_limiter.go:65-72 isSSERequest 仅匹配 text/event-stream，前端零 EventSource/纯 ConnectRPC binary（connect.ts:97 useBinaryFormat）→limiter 在主形态上从不触发=净无界，修法=纳入 ConnectRPC 形态；broadcast chan(8) drop-slow-consumer 经核实为全量快照语义自洽非债。残余=staging 7 项清单已落文档；缺口登记 G-POST2-1/G-POST2-2）|
| FEAT-3 | 受保护回测对齐 | 🟦open（roadmap）|
| VM-LIVE-PARITY-1 | **VM 实盘 parity 验证（业主提议真实账号测试，2026-09-19 设计实查完成）**：代码层 fail-closed 已全验，剩余未证轴=真实经纪商行为。假设清单：P1 `OrderResult.Price/Volume` 回显请求值非实际 fill（broker.go:51-55，executor 只回 ticket）；P2 deviation 静默丢弃（PlaceOrder 签名无参）；P3 符号常量硬编码 venue 值（freeze=0/trade_mode=FULL/exemode=MARKET/order_mode=63——vm_builtin_account.go:180-194）；P4 StopsLevel 等 venue 回填正确性未在真 broker 验；P5 无 minlot/lotstep 归一化层；P6 stops/freeze 前置校验未见；P7 tick 断连/点差行为。测试协议 docs/audits/vm-live-parity-protocol.md（T1-T7 可证伪断言，0.01 手 EURUSD 白名单 max5 单）。**待账号**：demo 优先，需 broker 服务器名+MT4/5+demo/live+secrets 通道凭据（聊天暴露的密码须先重置）。 | ✅done-测试执行（2026-09-19 Devin CLI 实盘对账完成——Exness-Trial demo `904d14e6` BTCUSDm 3 单+2 拒绝用例，P&L -0.94 USD 全平）：**P1 实锤双轴**（请求 price=0→fill 81262.24、volume 0.015→归一化 0.02，VM OrderSend 回显请求值非 broker 事实）；**P4 实锤瘦数据**（point/lotStep/stopLevel/maxLot/tradeMode 全 0 而 lotStep 实存 0.01）；拒绝路径 fail-closed 实证（131 Invalid volume 如实上浮）；SL 全链执行实证（81253.63→[sl] 81238.16）；comment 在途丢失附带发现。报告 docs/audits/vm-live-parity-results-2026-09.md；修复项另立 F1/F2/F3）|
| G-POST2-1 | **ConnectRPC server-streaming 无连接上限（P2 容量/滥用面，POST-2 探针批发现）**：`SSEStreamLimitMiddleware(5)`（`main.go:276`）已接线但 `isSSERequest`（`sse_limiter.go:65-72`）只匹配 `text/event-stream`；前端全量走 ConnectRPC binary（`application/connect+proto`）→ WatchPaperAccount/SubscribeJob/ChatStream/SubscribeMetrics 等全部 server-streaming RPC 不过 limiter。N tabs×M users 线性堆积 ~2 goroutine+~45-50KB/流（实测），1 万流≈2 万 goroutine+~0.5GB。修法=limiter 纳入 ConnectRPC 形态（或对 server-streaming RPC 加等价闸），非新增限制体系。 | 🟦open（设计实查完成→派工 builder-handoff-gpost2-stream-limit.md——**实查新发现第二层缺陷**：sseOwnerKey 取 context 值恒空[auth interceptor 在 middleware 后跑]→key 恒 anon→真 SSE 请求实为全进程共享 5 流；修法=Connect StreamLimitInterceptor WrapStreamingHandler[auth 后跑 GetUserID 可用]+withSency 链尾收口零调用点 churn+metric GaugeVec/Counter[关 G-POST2-2]+sseOwnerKey 改读 header[X-Real-IP 优先，ratelimit.go:117 先例]。**⚠️待独立复审（施工完成 2026-09-19）**：S1 `internal/interceptor/stream_limit.go` `NewStreamLimitInterceptor(userCap=10, ipCap=30)` 三方法形态[WrapUnary/StreamingClient 直通，StreamingHandler acquire→handler→defer release]，键序 user→ip→anon 带前缀防撞，拒绝 `CodeResourceExhausted`；S2 `ant_stream_active_streams{key_type}` GaugeVec+`ant_stream_limit_rejections_total{key_type}` Counter（promauto，同文件）；S3 withSency 链尾追加+包级 var nil 安全（handlers.go，registerHandlers 入口构造，55 调用点零改动，跑在 auth 后）；S4 sseOwnerKey 复用 `extractClientIPFromHeader` 改读 header+middleware 注释翻正。T1 cap=2 阻塞 fake 第 3 并发确定性拒绝+释放后恢复/T2 三键类隔离+metric 断言（testutil）/T3 mutation 删 acquire 检查→双测试 RED（0.016s，"admitted over cap"）→restore→GREEN；探针调用 select 双事件裁决[被拒 err|准入 entry 信号]消调度竞态（首轮 mutation 曾致挂死，已修为确定性收敛）。门禁 build/vet/test/race/check-lines(0 errors)/diff-check 全绿。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——施工 295d93f8：S1-S4 逐格相符[key 前缀 u:/i: 防撞/CodeResourceExhausted/withSency 链尾 nil 安全收口 55 调用点零改动/sseOwnerKey 改读 header 恢复 per-IP 本意]；测试判别面复核成立[channel 同步+entered|err 双事件裁决消调度竞态+三桶隔离+metric testutil 断言]；**T3 独立 mutation 复验**：acquire 恒准入→双测试 0.013s 精确 RED[admitted over cap]→restore GREEN；build/vet/race 20/check-lines 0 errors/diff-check 全绿）|
| VM-LIVE-PARITY-F1 | **实盘 OrderSend 结果回显请求值非 broker 事实（P2，实盘对账实锤）**：`OrderResult{Price: req.Price, Volume: req.Volume}`（strategy/runner/broker.go:51-55）——executor 只回 ticket，实际 fill 价/量不回读。实盘证据：请求 price=0→fill 81262.24、volume 0.015→broker 归一化 0.02，VM 返回值=请求回显≠broker 事实。修法：fill 后回读（position/OrderHistory 对账填实际值）或如实标注 requested 语义；deviation 形参丢弃同链处理（接线或声明）。回测无此问题（模型价=请求价）。 **2026-09-19 设计实查完成**（设计 `docs/audits/design-vm-live-parity-f1f3.md`）：定案=`OrderSendReply.GetResult()` 同步响应本带完整 Order 被 adapter 丢弃——`mthub.OrderExecutor.PlaceOrder` 改 `(*OrderRecord, error)` 填 broker 回执事实+State 显式推导；`runner.OrderExecutor` 改 `(sdk.OrderResult, error)`+deviation 形参（brokerImpl 透传化，造假点物理消除）；deviation 四层丢链全接（signal 转换器→submitOrder→mthub req→adapter Slippage mt4/mt5）；sentinel IntVal(1) 不动残余 R1。**⚠️待独立复审（施工完成 2026-09-19）**：S1 接口改 `(*OrderRecord,error)`+OrderRequest.Deviation（order_types.go）；mt4/mt5 adapter 回执全映射[mt4 补 Comment/Slippage，mt5 补 Slippage(uint64 钳负)]+State 显式推导[market→Open 防 Pending 零值撒谎，两侧同型 op-based；mt5 Order 无 magic getter→留零]；S2 runner.OrderExecutor 改 `(sdk.OrderResult,error)`+deviation 形参、brokerImpl 纯透传（回显点物理删除）；S3 proto comment=18+vmSignalToProto/submitOrder 接通（Magic/OppositeTicket 刻意不映射=系统覆盖/R4）；sdk 两 Comment 字段注释 ANSI 不保真。T1/T5/T7/T8 绿+M1(brokerImpl 复辟回显→T5 RED)/M3(mt4 删 Comment→T1 RED) 双先红后绿。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——施工 d39afc62 逐格核实：adapter 回执 verbatim 映射+State 显式推导防零值撒谎、brokerImpl 透传化、deviation 四层接通、Magic/OppositeTicket 刻意不映射判别测试双向钉住[999/555 不得出现]；**M1 独立 mutation 复验**：复辟 req 回显→T5 精确 RED[Price=0 want 81262.24/Volume=0.015 want 0.02]→restore GREEN；T8 真集成[真 MtHubService+fake executor 捕获 req 穿 coordinateMutation 全链+barrier confirmed 收尾]；build/vet/1091 测试/race 418/check-lines 0 errors 全绿）|
| VM-LIVE-PARITY-F2 | **SymbolParams 瘦数据链（P3，实盘对账实锤）**：mtapi 上游对 BTCUSDm 不返回 point/lotStep/stopLevel/maxLot/tradeMode（实测全 0，lotStep 实存 0.01 经归一化证明）→VM 实盘 `MarketInfo(MODE_POINT)`=0，按 point 算 SL 距离的策略拿零值。修法：point 从 digits 推导 fallback（10^-digits）并标注来源；其余字段缺省应如实"未知"非 0；或经 SymbolInfoEx（mt4.pb.go:3487 全字段）补链。 **2026-09-19 设计实查完成**（设计 `docs/audits/design-vm-live-parity-f1f3.md`）：mt4 `si.GetEx()` 从未读（Ex 有 Point/ContractSize/StopsLevel/Trade/TickValue/FreezeLevel 等 54 getter）→flat→Ex 逐字段回退；**附带两语义错**：mt4 `TradeMode=gp.GetExecution()` 枚举错配（正确=ex.Trade）、mt5 `PointValue=si.GetTickValue()` 字段错配（正确=si.GetPoints()）；SymbolParam 扩 TickValue/TickSize/SwapLong/SwapShort/FreezeLevel；live VM 传播缺 lot/tick/swap 字段→proto LiveStrategyContext+TickContext 扩字段+backfill+UpdateSymbolInfo 聚合；point 推导放 VM 边界 helper。**⚠️待独立复审（施工完成 2026-09-19）**：S4 SymbolParam 扩五字段+TradeMode canonical 注释；mt4 FetchSymbolParams 逐字段 flat→Ex 回退+TradeMode=ex.GetTrade()（Execution 错配删除）；mt5 PointValue=si.GetPoints()（零→GetTickSize）+TickValue/TickSize 归位+mt5 pb 无 freeze 字段（grep 零命中）→留 0 注释=未知；S5 proto LSC 34-40/Tick 18-24 七字段+双 backfill+pointOrDerived（唯一允许推导，VM 边界）+UpdateSymbolInfo 聚合 LiveSymbolInfo+brokerImpl harness 填全字段（backfill 块抽 live_symbol_backfill.go 保 check-lines 预算）。T2/T3/T4/T6 绿+M2(删 ex 回退→T2/T3 RED) 先红后绿。F1/F2/F3 门禁：build/vet/五包 test/race 全绿+check-lines 0 errors（live_context.go 回到存量 warn 线内）+diff-check 净。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——施工 d39afc62 核实+**复审抓出 S5 缺口审计侧修补**：builder 存了 liveLot\*/liveTick\*/liveSwap\* 7 ctx 字段但 `brokerImpl.SymbolInfo` harness 分支未填=死存储（VM 实盘 lot/tick/swap 仍全 0）——审计侧补 7 字段映射+新增 `TestBrokerImpl_SymbolInfo_HarnessFullFacts` 钉透传，mutation 复验[VolumeMin→Zero→RED]→GREEN；**M2 独立 mutation 复验**：TradeMode 复辟 gp.GetExecution→T3 精确 RED[9 want 4]；mt5 PointValue=GetPoints/TickValue 归位判别测试成立[0.01 vs 1.5 异值]；pointOrDerived 唯一推导边界正确[VM 边界非 adapter]）|
| VM-LIVE-PARITY-F3 | **comment 字段在途丢失+UTF-8 损耗（P4，实盘对账实锤）**：PARITY-* comment 到 broker 记录全空（magic 正常透传）；既有持仓 comment 显示 `\ufffd` 乱码——adapter→mtapi→MT4 comment 编码链有 ANSI/UTF-8 边界损耗。 **2026-09-19 设计实查完成**（设计 `docs/audits/design-vm-live-parity-f1f3.md`）：丢链三层实证——`antv1.StrategySignal` 无 comment 字段（proto 加 field 18）→`vmSignalToProto` 不映射→`submitOrder` 不设 req.Comment→mt4 adapter 不设 pb.Comment（mt5 已传）；读侧 `o.GetComment()` 透传正确，乱码=MT4 ANSI 上游事实非我方损坏；策略=原样透传+字段注释如实标注 ANSI 不保真。**⚠️待独立复审（施工完成 2026-09-19）**：proto `StrategySignal.comment = 18`（buf generate 2 pb.go+2 pb.ts 随提交）→vmSignalToProto 补 Comment 映射→submitOrder 补 req.Comment→mt4 adapter 补 pb.Comment（S1 合步）；读侧不动（透传=事实）；不做 UTF-8 校验。T7/T8 绿+M3 先红后绿；随 F1/F2 门禁全绿。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——施工 d39afc62：proto comment=18+pb.go/pb.ts 双侧重生成随提交；vmSignalToProto/submitOrder/mt4 adapter 三层接通逐格核实；**M3 独立 mutation 复验**：删 `Comment: req.Comment`→T1 精确 RED["" want PARITY-X]→restore GREEN；读侧 passthrough 不动正确）|
| VM-LIVE-VENUE-1 | **VM venue 交易常量硬编码（F1/F2/F3 残余 R2，P2）**：`SymbolInfoInteger` TRADE_MODE 恒 4(FULL)/FREEZE_LEVEL 恒 0/EXEMODE 恒 2(MARKET)+`MarketInfo` FREEZELEVEL 恒 0/TRADEALLOWED 用账户旗标（vm_builtin_account.go:181-194/:255-276）——`SymbolParam.TradeMode/FreezeLevel` 已采集（F2）但断在 proto 边界，策略拿假 venue 事实下单（freeze 区 modify 裸撞 broker 拒绝）。**2026-09-19 设计实查完成**（设计 `docs/audits/design-vm-venue-const-r2.md`+派工 `builder-handoff-vm-venue-const-r2.md`）：-1=unknown 哨兵全链（枚举 0=disabled/无冻结是真值不可混淆）；mt4 Ex==nil→-1×3（含新采 TradeExemode=ex.GetExemode()）、mt5 Freeze/Exemode=-1（pb 无字段）；proto LSC 41-43/Tick 25-27 扩 int32+backfill 逐字透传（param==nil→-1）+runner ctx/harness 直填+sdk.SymbolInfo 扩字段；SimBroker 回测模型常量保留（4/0/2）；MODE_TRADEALLOWED 改 symbol∧账户双轴（mode∈{1,2,4}∧IsTradeAllowed）。不做 CALC_MODE/ORDER_MODE（无 venue 源）。**2026-09-19 独立复审验收**：builder commit `dfd9eccd` 范围核符 S1-S6；build/vet/2079 测试/check-lines 0 errors/diff-check 全绿。三项独立 mutation 实证——M1 mt4 Ex==nil 哨兵删→RED（`got TradeMode=0 FreezeLevel=0 TradeExemode=0`）；M2 `SymbolInfoInteger` prop26 复辟常量 0→RED（`want 5`/`want -1` 双格判真值与哨兵）；M3 TRADEALLOWED 复辟纯账户旗标→RED（unknown/disabled/close_only 三格 `want 0 got 1`）；全部恢复 GREEN、工作树无残留。mt5/orders.go 450/300 贴预警线裁决为存量债另立 `CODE-SIZE-MT5-ORDERS-1`（builder D-012 建议采纳，一任务一范围不扩批）。未部署——待后续统一部署+实盘 venue 字段复验。| ✅done（2026-09-19 复审验收）|
| CODE-SIZE-MT5-ORDERS-1 | **mt5/orders.go 行数贴预警线（P3 存量债，VM-LIVE-VENUE-1 复审裁决单独立项）**：HEAD 施工前已 449/300 🟡（1.497×），R2 +4 行顶过 1.5× ERROR 线后 builder 压缩注释回 450=正好 1.5× 贴线——下次该文件任何净增行即 🔴 阻断。修法：按语义边界抽函数（参照 VM-CODE-HYGIENE-1 先例：FetchSymbolParams/PlaceOrder/历史查询拆 orders_*.go 子文件）。**2026-09-19 修复**：`FetchSymbolParams`/`FetchPriceHistory`/`FetchAllSymbols` 三函数 verbatim 移至新文件 `symbol_params.go`（符号元数据查询非订单语义）——orders.go 450→348 🟢、symbol_params.go 115。**零行为变更实证**：diff 逐字节比对搬迁块一致；mt5 包 131 测试+build/vet/check-lines 0 errors 全绿。 | ✅done（2026-09-19 Devin CLI 直接施工+验收——verbatim move 实证替代 mutation）|
| TRADE-RECORDS-DUP-1 | **trade_records 重复行 879 对（P2，2026-09-19 部署期 migration 278 实锤）**：live 路径（CST 编码）与 history-import 路径（UTC）对同一真实平仓各写一行——同 account_id+ticket+真实 close 时刻（CST 行 -8h 后 = UTC 行 close_time）。278 规范化时撞 `uk_trade_record_ticket` 唯一键。分布：778 对双 NULL-hash（链前时代行）/80 对 CST-NULL+UTC-已 hash/21 对双已 hash。处理裁定：迁移只修 10,623 非冲突行，879 对整体留残（删行动审计台账+已 hash 行删必断 follower prev_hash 链）。去重需专项设计：778/80 对可删 CST 侧（NULL-hash 链外，旗标迁 follower 代价小）；21 对双 hash 行**不可删不可改**（prev_hash 级联）——候选=tombstone/标注列或 VerifyChain 已知重复豁免。数据样本：profit/价格/SL/TP/comment 双侧等价，UTC 侧 magic 更全。**2026-09-19 设计实查完成**（设计 `docs/audits/design-trade-records-dedup.md`+派工 `builder-handoff-trade-records-dedup.md`）：类分布实测修正为 778 双 NULL/**101 双已 hash**（无混合类）；**同族新伤实证**——①epoch-零幻影行 **3,024**（close_time='1970-01-01'，全已 hash，8 账户，**仍在出血** 09-18+555/09-19+55；根因 `SyncAccountHistory`/`SyncOrderHistory` 两 verbatim 写入点无 State/IsZero 过滤，mt4/mt5 adapter `State=Open` 标注正确但带 epoch CloseTime）；②BALANCE 出入金 160 行混入（消费方已过滤=保留+今后止血）；挂单类 36 行核为合法成交挂单保留。方案：migration 281 dedup_log 全列快照归档+严格判据删 CST 侧 879（实测新断点 0+20）+删 epoch 3,024（断点 2,024）+两写入点守卫（State!=Closed||IsZero||Balance/Credit→skip）+VerifyChain dedup_log 豁免（prev_hash∈logged→非 break）。down 全复原（OVERRIDING SYSTEM VALUE 回插 seq）。**2026-09-19 独立复审验收**：builder commit `7fce4558` 范围核符 S1-S5——`SyncableClosedTrade` 单源守卫（State!=Closed∥CloseTime.IsZero∥Balance/Credit→skip）两 verbatim 站点接线、VerifyChain `deleted_link` 信息级豁免（42P01 缺表回退严格态）、down OVERRIDING 回插+DROP log。**生产数据克隆独立实证**：ant_dedup_review 克隆 20,459 行——up 执行 DELETE 879+3,024 精确命中、log=3,903（879 kept_id/3,024 NULL）、谓词幂等重跑 0/0、down 后 pg_dump 全表 diff 与产库逐字节相等（仅 \restrict 会话令牌差异）；T5/T6 真迁移文件实测 PASS、T3/T4 豁免/判别 PASS。**三项独立 mutation**：M-A 删 State 分支→T1 RED（open_state_epoch_close+pending_limit =true want false）；M-B 禁豁免分支→T3 RED（归档删除浮为 chain_break seq=-900000）；M-C 弱化签名去 open_time 判据→合成巧合对计数 879→880（严格签名不吞合法双峰平仓，open_time+8h 承重实证）。全部恢复 GREEN。未部署——部署后产库实清+写入止血生效。副本注记：trade_records 无 user_id FK（migration 150 意图未落实，仅 NOT NULL——另债观察不阻本批）。 | ✅done（2026-09-19 Devin CLI 独立复审验收——产数据克隆实证+三 mutation）|
| VERIFY-CHAIN-SEMANTIC-1 | **VerifyChain 写/验语义错位基线噪音（P3，TRADE-RECORDS-DUP-1 设计实查发现）**：写路径 `prev_hash`=**全局尾**（`ORDER BY seq DESC LIMIT 1` 跨账户），VerifyChain 按**同账户**前行校验——生产实测 14,392 已 hash 行中 2,113 行 prev_hash 指向异账户行=基线 chain_break 噪音（非篡改）。修法：VerifyChain 改全局 union 视角（trade_records∪dedup_log 按 seq 重建原链）或显式声明 per-account 近似语义并容忍首行。VerifyChain 当前生产零调用（仅 integration test），影响=审计能力可信度。**审计扩面（2026-09-20 对抗审计）**：根因 B 浮出——写侧 hash 覆盖 `NewFromFloat` 最短浮点串（含 ulp 噪声，实测 seq=13601 `open_price="4324.1990000000005"` 命中），NUMERIC 列不可重建；产库 union 14,409 行实测 `Decimal.String()` 编码仅 9,804 可重算、**4,605 永久不可确认**、`::text` ~0%。设计 v2（`design-verify-chain-semantic.md`）：union 验链 + 写侧 hash 规范化（RETURNING `::text`）+ 双编码重算（存量 9,804 可确认/4,605 如实标歧义）。**复审（2026-09-20 Devin CLI）**：施工 `e5290433` 复审抓出**根因 C**——写侧尾读仅 trade_records 而 union 真尾在归档（产库 live 尾 seq=102029/归档尾 102092），新 append 永久假 chain_break；审计侧修补=尾读 union 化+`to_regclass` 探测回退（tx 内 42P01→25P02 毒化不可捕获）。产克隆 `ant_verify_review`（20,476 行）实测：T1-T9+存量 hash 测试 **16 全绿**（chain_break=0/hash_mismatch=4,605 精确基线）；独立 mutation×4 全 RED→恢复 GREEN——**M1** live-only 走查→基线 chain_break=1,325 复活；**M2** 删规范化通道→mismatch 4,605→14,409；**M4** hash 回退内存串→T8 噪声行失配；**M6** 尾读回退 live-only→T2/T3/T4/T5/T9 精确复红。另修存量 hash 测试 FK 父行缺失（环境性）。 | ✅done（Devin CLI 复审验收+部署 2026-09-20 `c74d8d3`，含根因 C 审计侧修补——旧二进制持续遗断链行属出血型缺陷即时止血；部署后暂无新 append，union 尾=arch seq 102092 待有机流量续接实证）|
| KB-SEED-DRIFT-1 | **kb_compat_fact 种子漂移——陈旧枚举编号全局压过内建表（P1，R2 实盘探针实证 2026-09-20）**：`kb_compat_fact` 是 `interp.MQLConstants` 一次性快照（08-08 seed），`ON CONFLICT DO NOTHING`+仅空表才 seed → 09-18 VM-ENUM-NUMBERING-1 重编号（符号/市场信息枚举对齐真 MQL5 序号）永不传播；`LookupMQLConstant` KB-first 使 DB 陈旧值**全局压过修正表**——进程内所有编译（live+backtest）解析旧 prop 编号。产库实测 43 常量错位：SYMBOL_TRADE_MODE 16（应 22）→live 探针实证 `SymbolInfoInteger` fatal `unsupported prop 16` 每 tick 崩；SYMBOL_DIGITS 12（应 17）同 fatal；MODE_FREEZELEVEL 16（应 32）→**静默错路由**到 TICKVALUE 返错值；ACCOUNT_MARGIN_* 四值置换。另有 8 个已从注册表删除的名字被陈旧行复活（MODE_MARGINCALCMODE/SYMBOL_SWAP_ROLLOVER3DAYS 等）。修法=Seed 升级为 reconcile：seed 行镜像内建表（`ON CONFLICT DO UPDATE WHERE source='seed'` 纠偏 + `pruneSeedRows` 删已移除名），`Start()` 每次 boot 无条件执行→全环境自愈；`manual`/`auto-verified` 行保留（ops override 特性不动）。改动：knowledgebase/seed.go upsert+prune、service.go Start 顺序、新 seed_reconcile_integration_test.go 三格（漂移纠偏/剪除/手工豁免）。mutation：M1 upsert 回退 DO NOTHING→漂移值存活 RED；M2 删 prune→陈旧行残留 RED。产库已由测试自愈（43 值纠正+剪除+`MY_C1_TEST_CONST` 测试垃圾清除）。 | ✅done（2026-09-20 Devin CLI 审计侧修复+实证——live 探针 R2 抓出、3 集成测试绿、M1/M2 mutation RED→恢复 GREEN）|
| TUNING-OVERFIT-2 | OOS-at-publish 惰性闸（`quality.go:302` 条件性惰性，优化快照未填 OOS 字段）。**2026-09-19 设计实查**：门已配 0.5 生效中（migration 213/260）但 5 处 `BacktestSnapshot{}` 生产点零填充 Oos 字段（`AgentBacktestResult` 无 OOS）→ 门恒空转=静默 fail-open；完整闭环=publish OOS 回测（feature 另立），LOWPRI-SWEEP-1 S6 先 fail-visible 化。**🟦open（施工完成，待独立复审）**：quality.go OOS 块抽 evaluateOosGate 方法（gate 已配但无 OOS 数据→zap warn fail-visible；armed/有数据/降级违反/未武装四态测试+T6 observer 判别 RED 实证）；完整闭环（publish 时 OOS 回测）另立 feature。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——施工 51b01d87：evaluateOosGate 抽取+armed-缺数据 zap warn fail-visible；**T6 独立 mutation 复验**：Warn→Debug→observer 断言精确 RED→restore GREEN；四态测试全绿[armed缺数据warn/有数据静默/降级违反/未武装静默]。完整闭环=publish OOS 回测 feature——另立登记）|
| VM-LIVE-SYNC-DISPATCH-1（R1） | **signal-mode `OrderSend` 返回假票号 `IntVal(1)` + 同事件信号丢单 + `cancel_all` 空转（P1 语义类，VM-LIVE-PARITY-1 残余 R1）**：原链 `builtinOrderSend→vm.signal→IntVal(1)→事件返回→dispatchResponse→coordinateMutation`——真票号拿到时 MQL 表达式早已返回，策略存 ticket 做 OrderSelect/OrderClose 必然失配；`vm.signal` 单槽 last-wins 使同事件第二单**静默丢单**；`cancel_all` 在 dispatch switch 无 case 落 default 空转。修法（设计 `design-vm-live-sync-dispatch-r1.md` v2）：VM 增 `syncDispatch` 回调——builtin 在事件循环 goroutine 内同步走 `coordinateMutation`（同 barrier/熔断/读回验证/审计全复用），live 返回 broker 真票号、paper 保留哨兵；15 处 signalMode 分支统一 `emitSignal`（err→`-1`/`false`+`lastError=146`，零 fatal）；`dispatchSignalSync` confirmed 后把 broker 事实注入 runner live state（`ApplyConfirmedPosition`/`RemoveConfirmed*`/`ApplyConfirmedPendingOrder`/`ApplyConfirmedModify`，`ctx.mu` 保护）恢复 `OrderSend→OrderSelect` 同事件语义；`dispatchResponse` 增 `alreadyDispatched`（`sessionSyncDispatched` 实测标记）防双发；接线=`wireSyncDispatch` 在 handleBar×2/handleTick/handleTrade 每事件前安装；潜伏缺陷 B 修复=`dispatchCancelAll`（OpenedOrders 非市价过滤+ARCH-4 magic 过滤+逐票 coordinator+unknown 停批）。**复审抓出缺口修补**：`mutationResult.affectedTickets`——close_all/cancel_all 批内单条已确认票号回传（含 unknown 早退部分批次），runner 侧逐票移除防幽灵仓位。改动面：vm.go/vm_builtin_trade.go/vm_builtin_trade_signals.go/interp_runner.go/runner.go/vm_live_session.go/mutation_coordinator.go/live_dispatch.go/live_sync_dispatch.go(新)/live_context.go/live_runner_events.go+两新测试文件。 | ✅done（2026-09-20 Devin CLI 施工+独立复审验收——VM 层 5 测+server 侧 6 测全绿；strategy 包 442 绿、mql2go 绿、全仓回归零新增失败[3 个 internal/service 失败系硬编码 DSN localhost:5432 环境性存量、文件未触碰]；mutation×4 全 RED→恢复 GREEN：**M1** builtin 回退 `IntVal(1)`→T1 红；**M2** 删 `alreadyDispatched` 跳过分支→T8 红（placeCount=2 双发实证）；**M3** 删 `applyConfirmedToRunner`→T5 红（confirmed 仓位丢失）；**M4** 删 affectedTickets 移除循环→T9 红（幽灵仓位残留）。残留登记：paper 哨兵 ticket=1[无 broker 概念]、resp.Signals 单槽 last-wins[shadow 审计仅末单，DB 审计经 recordSignalIntent 逐单全覆盖]、R4 CloseBy live dispatch 未做。**已部署 `5db60c7f`**——backend healthy，日志无 panic/派发错误）。**实盘端到端探针验证（2026-09-20，demo `904d14e6` login 95262066 MT4 Exness-Trial，run `8bfe7ef0`）**：单 OnTick 内 OrderSend→OrderSelect→OrderClose 全链实测——`send_ret=394076399`（broker 真票号非哨兵）、`select=true openPrice=80365.17 lots=0.01 type=0`（同事件注入仓位带 broker 事实）、`close=true`、`select_after_close=false`（确认移除无幽灵仓位）；coordinator 生命周期 submitting→submitted→confirmed 在事件内同步完成（~1.8s）；探针停后 positions=0|
| LIVE-CLOSEALL-PENDING-1 | **`close_all` 对挂单也发 `CloseOrder`——确定性被拒且确认等待毒化批次（P2，R1 复审发现）**：`OpenedOrders`（MT4 order pool 含挂单）中 magic/symbol 匹配的挂单也进 `coordinateMutation` 发 `CloseOrder`——broker 确定性拒绝，更重的缺陷是其确认等待超时使整批 `outcome_unknown`/barrier locked（市价单平仓被挂单连带阻断）。修法=镜像 `dispatchCancelAll` 反向过滤：循环首行 `o.OrderType != mthub.OrderMarket` 则 skip（OrderLimit/OrderStop/OrderStopLimit/OrderBalance/OrderCredit 全跳过——close_all 只碰市价仓位，挂单归 cancel_all/DeleteOrder）。顺带拆文件：`live_dispatch.go` 460 行超红线（他 agent 回填回超）→ per-action dispatcher×6+`submitOrder` 迁 `live_dispatch_actions.go`（209+264 行）。 | ✅done（2026-09-20 Devin CLI 施工+验收——T10 `TestSyncDispatch_CloseAll_SkipsPendingOrders` 先红[未修时 pending 的 CloseOrder 确认等待→outcome_unknown 整批失败，比噪音更重]→修复绿；**M5 mutation** 删过滤行→T10 精确复红→恢复绿；strategy 包 443 绿、race×SyncDispatch 7 绿、check-lines 0 errors[拆分同时清偿存量超限]、build/vet/diff-check 净；**已部署 `799fae03`** backend healthy）|
| LIVE-ORDERREC-SIDE-TYPE-1 | **MT4/MT5 `PlaceOrder` 回执丢 `Side`+`OrderType`——注入 VM 的挂单/卖单被读错类型方向（P1 语义类，R1 实盘深探抓出）**：两 adapter 的 PlaceOrder 构造 `OrderRecord` 只填 Ticket/Price/Volume/SL/TP/Comment/Magic，漏 `Side`/`OrderType`（零值→读成 SideBuy+OrderMarket）；`dispatchSignalSync→applyConfirmedToRunner→orderRecordToPending` 注入后，同事件 `OrderSelect` 命中挂单但 `OrderType()` 返 0（OP_BUY）而非 2（OP_BUYLIMIT）——实盘探针实测 `pend_select type=0`。卖方向单更糟：`Side` 缺省→注入成 Buy。修法=回执 verbatim 映射：MT4 提取 `mt4OpToSideAndType`（新共享 helper，读 `o.GetType()`）；MT5 复用 `mt5OrderTypeToSideAndOrderType` 并补缺的 `BuyStopLimit` case。State 判定同步改读回执 `o.GetType()`/`GetOrderType()`（原用请求 op）。顺带拆 `orders.go` 466 行超红线→事件流 funcs 迁 `order_events.go`（369+110）。 | ✅done（2026-09-20 Devin CLI 施工+验收——T 层 adapter 测试双侧先红[rec.Side=0/rec.OrderType=0]→修复绿；T11 `TestSyncDispatch_ConfirmedPending_InjectsAsPendingOrder` 端到端挂单注入断言 pending 池+Type=OrderLimit/Side=SideSell 绿；mutation 删映射行双侧精确复红→恢复；mt4/mt5/strategy 714 绿；check-lines 0 errors；**已部署 `f0508fd9` + 实盘复验**：demo 904d14e6 run ddbca301，buy_limit `394081292` → `pend_select type=2`（OP_BUYLIMIT，修复前实测 0）、`pend_delete=true`、`sel_after_delete=false`；DB 落 `order_type=1` LIMIT+FILLED 终态，账户零残留）|
| VM-IMPLICIT-VAR-READ-1 | **未声明标识符的"读"位置被隐式声明为零值全局——typo 常量/变量静默变 0（P1 语义类，实盘探针实证）**：`resolveVar`（compile.go:284）对 mql4/python 的隐式声明容忍无读写区分——`SymbolInfoDouble(Symbol(), SYMBOL_TICK_VALUE)`（该常量名在 MQLConstants/KB/CompatFixes 均不存在）经 `ExprVar`→`resolveVar` 落 mql4 分支注册零值全局+仅记 blind spot，prop=0→`SYMBOL_BID`→实盘实测 `tv=81337.02`（bid 冒充 tick_value，同 tick `MarketInfo MODE_TICKVALUE` 正确报 0）。真实 MQL4/Python 对未声明标识符的**读**一律编译错/NameError；隐式声明只对**赋值目标**成立。修法：读位置（compileExpr ExprVar / subscript 读 / `x++`/`x+=` 读写合一 / by-ref 读参）走严格 resolve——未声明即 compile error；写位置（赋值目标/compileDecl）保留 mql4 隐式声明。回归护栏：7 个真实用户策略源码全量 strict-compile 兼容扫描。 | ✅done（2026-09-21：resolveVar 拆读严格/写容忍双路 resolveVarWrite；读位 ExprVar/PUSH_ARRAY/update/compound 严格，写位 assign/decl/subscript-store/ArrayResize 保留 mql4 隐式；顺带修复 `int x;` 裸声明被静默丢弃（declaration→identifier 分支）+补 clrNone 与 20 个缺失 web 色常量。测试 compile_implicit_var_test.go 5读位拒+3写位容；M1 mutation 复辟 mql4 隐式读→5 例精确复红；真实策略兼容扫描 7 个：3 个 IR 段预存失败、Violet 缺常量已补、可编译面全绿；mql2go 785 绿。**已部署+实盘复验**：run d457d820 用 SYMBOL_TICK_VALUE 别名→`compile MQL failed: unknown variable: SYMBOL_TICK_VALUE` fail-closed（原静默 prop0→bid 冒充）|
| VM-ERR-CODE-COLLAPSE-1 | **signal-mode 派发错误全折叠为 `lastError=146`——broker 真实错误码丢失（P1 语义类，实盘探针实证）**：链三段各丢一层——①`coordinateMutation` 拿到带 broker code 的 typed err（`ErrBrokerRejected`+`code=136`）但 `mutationResult` 无 err 字段，只记 state；②`dispatchSignalSync` 对 deterministicRejected 重造 `fmt.Errorf("rejected by broker")` 把码丢光；③builtin `vm.lastError=146` 一刀切。实盘实测：`OrderModify` broker `code=136 Off quotes`→MQL `err=146`（ERR_TRADE_CONTEXT_BUSY 误导）。修法：typed 链路——adapter 产 `mthub.BrokerRejectError{Code,Msg}`（Is→ErrBrokerRejected 保分类不变）、`mutationResult.err` 透传、`dispatchSignalSync` 译成 `sdk.BrokerRejectError`（sdk 是 VM 契约层，mql2go 不 import internal）、builtin `errors.As`→`lastError=code`（MT4 码即 MQL4 ERR_* 值）；无码错误仍 146 兜底。 | ✅done（2026-09-21：mthub.BrokerRejectError{Op,Code,Msg} 产码于 mt4/mt5 adapter 四订单位（Is→ErrBrokerRejected 分类不变）、mutationResult.err 透传、live_sync_dispatch errors.As→sdk.BrokerRejectError、signalErrorCode 映 lastError=native code（无码仍 146）。T12 dispatch 136 透传+VM lastError 136/146 双断+mt4/mt5 adapter 码保留测试；mutation×3 RED→GREEN：边界翻译删除/adapter 回退 fmt.Errorf/helper 恒 146 均精确复红。**已部署+实盘复验**：run 3559decb OrderModify broker 拒绝→`err=130`（原 146）|
| RISK-DEDUP-KEY-1 | **risk guard 去重键丢 comment/magic 且硬编码 OrderType——同 tick 合法多单被误杀（P2，实盘探针实证）**：`GuardRequest`（service_orders.go:93）只填 `Symbol|Side|Volume|OrderType:"market"(硬编码)|Price`——dedup key 无 comment/magic、挂单全撞 "market"。实盘实测：同 tick 第二单（comment=magic 不同的合法新单）被 `duplicate order within dedup window` 拒绝。网格/加仓策略同价同参多单必然中招。修法：GuardRequest 补 `Comment/Magic` 字段+`OrderType` 用 `req.OrderType` 真值；dedup key 纳三字段——真重试（全同含 comment/magic）仍被拦，distinct-intent 放行。 | ✅done（2026-09-21：GuardRequest 扩 AccountID/Magic/Comment+OrderType 用 orderTypeToString 真值；dedup key 对齐 Gate 形状 account|symbol|side|vol|type|price|magic|comment；Paper PlacePaperOrder 无 comment 面不动。guard_test.go 三例+M1 复红；risk 459 绿。**已部署+实盘复验**：同 tick 异 comment v2=394688565 放行、全同重试仍拦；复验抓出 Gate 层同缺陷（intent 丢 comment）已补 eda7911|
| ACCOUNT-TRADE-ALLOWED-DEAD-1 | **`accountTradeAllowedLookup` 读永不存在的 `account_status='trade_allowed'`——`IsTradeAllowed()` 实盘恒 false（P2，实盘探针实证）**：状态机只写 `connected`/`reconnecting`/`disconnected`（runner_gateway.go:180 等），`trade_allowed` 值全代码库零写入点；mtapi proto 也无 server 侧 trade-allowed 字段（MT4 语义上该旗标是终端侧概念）。实盘实测：demo 账户 `tradeAllowed=false` 但 OrderSend 成功——VM 看到的"账户轴不可交易"是假事实。修法：谓词放宽为 `connected \|\| trade_allowed`（connected 会话+非 investor 即平台轴可交易——订单可发即证明；broker 侧真实拒绝由各单 code 如实回传；investor 门另层已在）。 | ✅done（2026-09-21：抽纯函数 accountTradeAllowedPredicate 谓词放宽 connected||trade_allowed；account_trade_allowed_test.go 四例+M1 复红。**已部署+实盘复验**：connected 账户 `tradeAllowed=true`（原 false）|
| OMS-REPLAY-SPAM-1 | **broker 订单流回放对终态订单反复尝试非法 OMS 转换——生产 error 日志风暴（P2，生产日志巡检抓出）**：`transitionOMSByUpdate` 的 `default→WORKING` 把所有非 close/delete 事件（`modify`/`balance`/`credit`/`unknown`/无票号 payload）都当生命周期转换，且 `TransitionOrderByTicket` 无幂等/终态守卫——重连回放 `close` 对已 FILLED 订单刷 `invalid transition FILLED → FILLED`（实测 27 连发），`open`/`modify` 回放对已 FILLED 订单每 ~60s 刷 `FILLED → WORKING`（ticket 394135301 实证）；`balance`/`credit` 账户事件的票号不在 orders 表还会触发虚假 `order not found→retry→reconcile` 链。修法：`omsTargetForUpdateType` 只放行生命周期事件（close/pending_close→FILLED、delete→CANCELLED、open/pending_open/modify/pending_modify→WORKING，其余+ticket==0 不转换）；`shouldAttemptOMSTransition` 守卫同态幂等 skip+终态（FILLED/CANCELLED/REJECTED/FAILED/EXPIRED）拒绝转出，`TransitionOrderByTicket`+`retryTransitionByTicket` 双接线。观察项：`pending_close→FILLED` 语义存疑（被删挂单记为成交而非取消）——预存映射，未动。 | ✅done（2026-09-20 Devin CLI 施工+验收——`TestOMSTargetForUpdateType` 11 例+`TestTransitionOMSByUpdate_SkipsTicketlessUpdate`+`TestShouldAttemptOMSTransition` 12 例全绿；**M1** default→WORKING 复辟→balance/credit/unknown/empty 4 例精确复红→恢复；**M2** 守卫恒 true→终态回归例复红→恢复；cmd/server 20 绿、mthub 16 绿、build/vet/check-lines/diff-check 净；已部署）|
| LIVE-POS-SNAPSHOT-LAG-1 | **mtapi OpenedOrders 快照滞后+瞬时丢仓 → `UpdateLiveState` 整表覆盖回退 broker 已确认事实 → `OrdersTotal` 闪烁 → total<1 门禁误判连环开仓（P0 真钱路径，XAUUSD 实盘 26min 堆 36 仓实证）**：`livePositions`/`livePendingOrders` 每事件被快照 wholesale 覆写；实测已确认成交的新仓在 broker 事件流中 ~6s 才出现，期间快照反复把它挤出 → VM 读侧在 0/N 间抖。修法：Runner `confirmedMutationRetention=120s` 保留窗——`ApplyConfirmedPosition/RemoveConfirmedPosition`（挂单同）记录票号+时刻，`UpdateLiveState` 调和时滞后快照丢不掉已确认开仓、复活不了已确认平仓，快照追平或超时后归还权威。 | ✅done（2026-09-21：mergeConfirmedPositions/Pending+track/prune；live_state_reconcile_test.go 四例（lagging 保留/close 不复活/pending 同语义/过期归还）+mutation：整表覆盖回退→3 例精确复红。**已部署+实盘复验**：BTCUSDm probe `after_send total=2` 跨 tick 稳定不回落、`after_close=1` 孤儿仓正确保留、POOL_C 快照一致无抖动）|
| LIVE-HISTORY-POOL-1 | **`HistoryOrders` 实盘恒空——`OrdersHistoryTotal`/`OrderSelect MODE_HISTORY` 返回 0，历史依赖型策略（胜负统计/网格恢复）失真（P2）**：`brokerImpl.HistoryOrders` 未接数据源。修法：`historyFn` provider 注入（`Runner.SetHistoryProvider`），`initVMSession` 按 account 装配 `mtHub.OrderHistory`（0,0→epoch~now 全量），OrderRecord→sdk.Position 带 ClosePrice/CloseTime；runner 在 `Start()` 创建后才注入（session 暂存字段）；无 provider→fail-closed lastError（非伪造空池）。 | ✅done（2026-09-21：TestHistoryOrders_LiveProviderWired/NoProviderFailsClosed+mutation（provider 短路→wired 复红）。**已部署+实盘复验**：BTCUSDm `hist=1051` 全量枚举+XAUUSD `hist=131`；初版 runner 接线点错误（runner 在 Start 才建）已修并复验通过）|
| VM-STATIC-LOCAL-1 | **函数内 `static` 局部变量不持久——每次调用重新初始化（P3 语义缺口，实盘探针抓出）**：探针 `static int done` 每 tick 重打证明 static 被当普通局部变量。依赖 static 记状态的 MQL 策略会行为漂移（只跑一次的初始化每 tick 重跑、计数器失效）。修法：static 局部需提升到函数级持久存储（VM frame 外）。 | ✅done（2026-09-21 施工 0af39991 + **Devin CLI 独立复审通过**——实证复核：compileBlock 返裸 []Statement+compileIf 不 pushScope（if/else 花括号体共享 map 层）→ 施工方偏差申报属实，S3 dup 检查与 T4 冲突为真，改重绑定语义裁决**批准**（兄弟枝同名 static 各占独立 __static_N 槽+独立旗标，引用在编译点捕获当时槽位；同层重定义宽容与既有 plain-local 行为一致）。复审独立重跑：mql2go 797 绿、race Static 13 绿、check-lines 0 errors、build/vet 净；独立 mutation：M1 删 e.Static 分支→6 例精确复红（g_stat=6≠8/calls=1≠9/arr=1≠3/InitA×3/lazy g_out=2≠1/s=3≠9），M3 删旗标置位→T2/T3 复红（init 跑 3 次）；恢复后 12/12 绿、diff 与提交零偏差。**复审顺手发现预存缺口**→VM-BLOCK-SCOPE-1 新债。**部署+实盘闭环（2026-09-21）**：镜像重建部署后 BTCUSDm 1m 实盘探针 `static int counter` 跨 90+ tick 单调递增 1→90 永不归零、`acc` 0.5→45 累积、`once=100` 初始化器单次执行——修复前每 tick 重打 init，实盘语义恢复）|
| VM-BLOCK-SCOPE-1 | **if/else/switch 花括号块不建作用域——块内声明泄漏到外层（预存缺口，VM-STATIC-LOCAL-1 复审发现，P3）**：`compileBlock`（compile_interp.go:366）返回裸 `[]Statement` 不产 StmtBlock，`compileIf`（compile.go:504）对 Body/ElseBody 直接 compileStmts 不 pushScope——`if(x){int n=1;}else{int n=2;} n++;` 可编译且 n 绑定到 else 枝（真实 MQL 报 n undeclared）。所有局部变量受影响（非 static 独有）；`compileFor`（compile_loops.go:8）有 pushScope 故 for 域正确。修法：compileBlock 产 StmtBlock 节点或 compileIf/compileSwitch 对枝体补 pushScope/popScope——注意存量策略兼容扫描必做（声明泄漏可能被隐式依赖）。 | 🟦open 待施工（2026-09-21 立项完成+决策方设计审计 v2 修订：设计 SSOT `docs/audits/design-vm-block-scope-1.md` + 施工单 `builder-handoff-vm-block-scope-1.md`。审计实证纠正 v1：for 体 decl 本不泄漏[外层域罩住]、写位引用走隐式注册 shim 测不到拒[全钉纯读位]、mutation 映射修正、B1 改 pin。**兼容扫描前置门已过**：CST 扫描器实测 7 真实策略+4 fixture 共 11 源 CLEAN，已入库常驻回归门 b388b94e）✅done（**Devin CLI 独立复审通过 2026-09-21**，施工 2f188877——复审独立重跑：mql2go **811 绿**、race BlockScope|Static 26 绿×3、build/vet/gofmt/check-lines 0 errors/diff-check 净、CompatScan 20 CLEAN；**独立 mutation 亲手变异**：M1 删 compileIf 双包裹→B2/B7/B12+错误形状测试精确复红（B1 pin 不承重✓）；M2 删 for 内层→仅 B5 红（重绑定使 cond 读旧槽→死循环撞指令上限）+B3 不红✓；M3 删 while/do/switch 包裹→B4/B9/B10 精确复红；恢复后 diff 与提交零偏差、14/14 绿。实现复核：5 插入点全按设计 v2 落位、loopStack 序保持、compileSwitch popScope 置纯 patch 区前正确；提交范围 5 文件全属本任务。**部署+实盘闭环（2026-09-21）**：镜像重建部署后 BTCUSDm 实盘双探针——A 拒收态 `if(g>0){int n=1;} g=n;` 编译拒 `unknown variable: n`（修复前泄漏编译过）；B 接受态 `runs=4 a=1 b=2 cnt=3` 全对（兄弟枝 static 各占各槽、for 体遮蔽计满 3 圈、裸块正常））（施工交付明细 2f188877：按设计 v2 方案 A（IR 层 pushScope 包裹）落地——`compile.go` compileIf 双枝各包 push/pop；`compile_loops.go` compileWhile/compileDoWhile 体包裹、compileFor 内层体包裹（外层 for 域保留，序 pushScope(for)→init→cond→pushScope(body)→body→popScope(body)→update→jmp→popScope(for)）、compileSwitch 整个 case 编译区单层包裹（case 不建域，C 语义）；`compile_block_scope_test.go` B1–B12+错误形态断言全落地（拒收用例全纯读位 `g=n`，v2 纪律）；先红：stash 双文件实证 B2/B4/B5/B7/B9/B10/B12 红+B1/B3/B6/B8/B11 pin 绿（与设计现状矩阵逐一吻合）→后绿 13/13；mutation M1（删 compileIf 双包裹→B2/B7/B12 红+B1 不承重绿）/M2（删 for 内层→仅 B5 红+B3 不红）/M3（删 while/do/switch 包裹→B4/B9/B10 红；首做因脚本误删 pop 致栈失衡的 B10 红证无效已作废重做，对称剥离后有效）逐条 RED→restore→GREEN（md5 校验还原一致）；机检 build/mql2go 全量绿[零新增失败，存量 fixture 无泄漏依赖，停报条件未触发]/vet/gofmt/check-lines 0 errors/race×3/diff --check 全绿；`TestBlockScopeCompatScan` 重跑 PASS（11+ 源 CLEAN，扫描器文件保留）；测试基建自审发现并修复：runBlockTicks 首版漏跑 OnInit[全局初始化器在 OnInit 前导]致 B11 假红，已修，B11 修复前后均为绿 pin |
| CQ-5 | eslint-disable 残留 ~~11 处~~缺注释（2026-09-19 实查 **33 处**非 gen/，几乎全 `react-hooks/exhaustive-deps` 无理由豁免）| 🟦open（设计实查完成→派工 LOWPRI-SWEEP-1，builder-handoff-lowpri-sweep-1.md） ✅done（Devin CLI 验收 2026-09-19——33 处 disable 全部补 `-- reason` 且逐处具体非模板[mount-only/zustand setter/modal cycle/compartment reconfigure]；eslint-disable 零裸豁免；vitest 217 绿）|
| CQ-10 | `AIStrategyTemplatesRepository`（`internal/repository/ai_strategy_templates_repository.go`）**零生产调用方**——ARCH-3 把调度运行时取码源从 `ai_strategy_templates` 改为 `strategy_templates` 后，该 repo 全 `backend/` 仅剩自身定义。`purchase-to-live-link-spec.md:89/164` 仍称"保留（AI 生成仍用）"，已不成立（2026-09-19 复核零调用方仍成立）。**🟦open（施工完成，待独立复审）**：ai_strategy_templates_repository.go 已删 Repository struct+New+ListActive+GetByID+ListByCategory 5 func（**等价修正：AIStrategyTemplate 类型+ParameterSlotsString 被 internal/ai 活引用保留**——整文件删除 build 红，build 绿即死代码删净证明）；表/migration/种子保留；spec :93 追加订正句。| ✅done（Devin CLI 验收 2026-09-19——repo struct+5 DB 方法删净，build 绿=引用清零；类型+ParameterSlotsString 保留系 strategy_prompt.go 语法引用。**复审发现传递性死链**：TemplateLibrary/PromptParams/BuildSystemPrompt/BuildUserPrompt 全仓零生产构造点（NewStrategyPromptBuilder 仅 auto_fix.go 喂 FeedbackPromptParams）→ AIStrategyTemplate 实为死链锚点——另立 CQ-11）|
| CQ-11 | **internal/ai 传递性死代码簇（LOWPRI-SWEEP-1 复审发现，2026-09-19）**：`template_library.go`（TemplateLibrary/NewTemplateLibrary/Match*/topByCategory/countMatches 全方法零调用方）、`strategy_prompt.go` 的 `PromptParams`+`BuildSystemPrompt`+`BuildUserPrompt`+`Template` 字段+`ParameterSlotsString`（PromptParams 生产零构造——NewStrategyPromptBuilder 唯一调用方 auto_fix.go:66 只用 FeedbackPromptParams）+ `repository.AIStrategyTemplate` 类型（CQ-10 删除后仅存语法引用）。`DetectCodeStrategyType` 调用方需复核。删法=整簇删除（非 CQ-10 原范围）。**🟦open（施工完成，待独立复审 2026-09-19）**：`template_library.go`+`ai_strategy_templates_repository.go` 整文件删（含 CQ-10 保留的 `AIStrategyTemplate` 类型+`ParameterSlotsString`——本簇裁定其为死链锚点随簇删）；`strategy_prompt.go` 删 `PromptParams`/`BuildSystemPrompt`/`BuildUserPrompt`+`repository`/`strings` import；保留 `StrategyPromptBuilder`/`NewStrategyPromptBuilder`/`BuildFeedbackPrompt`/`FeedbackPromptParams`（auto_fix.go:68）/`DetectCodeStrategyType`（auto_fix.go:67）/`IntentResult`（clarification.go 自有链）。复核门 grep 删后零有效命中（唯一命中=`FeedbackPromptParams` 子串，派工单豁免）；`go build ./...` 绿=引用清零（删除型债，断言级 mutation N/A 如派工单）；ai/repository/mdgateway 测试绿；check-file-lines --strict 0 errors；表/migration/种子未动。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——施工 5c855630：复核门重跑 grep 删后全仓零有效命中[唯一命中=FeedbackPromptParams 豁免子串]；strategy_prompt.go 活成员保留正确；build 绿=引用清零；删除型债 mutation N/A 接受）|
| CQ-12 | **`npm run lint` 主枝存量红（LOWPRI-SWEEP-1 复审发现，2026-09-19）**：`WorkspaceCenterColumn.tsx` 313 行——eslint `max-lines` error（281 计行>250）+2 处 `react-hooks/exhaustive-deps` warning；文件自 `0d52f0a6` 起即超限，非本批引入。eslint error 使 lint 命令恒 exit 1（与 `--max-warnings 73` 无关）。修法=拆文件（UI 分层，非本批范围）。**🟦open（施工完成，待独立复审 2026-09-19）**：`onNewSource`/`onSectionChange` 包 `useCallback`（warning×2 消，useMemo 失效源除）；抽 `useWorkspaceSidebarProps`（sidebarProps useMemo+handleNewStrategy+onNewSource 内聚，handleNewStrategy 原语义逐行保持含 `setCenterTab('code')` 收尾）+`WorkspaceDocks`/`MobileStrategyChat`/`WorkspaceBottomPanel` 三包装子组件（props 显式化）；主文件 313→242 行（eslint 计行 ≤250，error 消）。门禁：`eslint src` 零输出（3 problems 全消）+`npm run lint` 绿+tsc 0+vitest 全量 33 files/217 tests 绿+git diff --check 净。| ✅done（**Devin CLI 独立复审通过 2026-09-19**——handleNewStrategy/onNewSource 迁入 hook 语义逐行保持[含 setCenterTab('code') 收尾]；useCallback 依赖正确；主文件 242 行≤250；**独立重跑**：eslint src 零输出+npm run lint exit 0[自 0d52f0a6 起的存量红消除]+tsc 0+vitest 33/217 绿）|
...
| DATA-TRUTH-1-DESIGN | **DATA-TRUTH-1 设计方案落档（2026-08-28，🟦open 待业主决策）**：spec `docs/spec/fix-2026-08-28-data-truth-1-reconciliation-convergence.md`。根因 3 层：A ghost 仅 log.Warn 从不补写（违反 ADR-0013 §2.3 "broker 有 PG 无 → INSERT"）；B orphan 仅修 SUBMITTED→FAILED（其他非终态永不修复）；C ant 全量 vs broker 24h 窗口不对称 → 结构性假 orphan（129 条/账户/轮）。修复方案 S1-S5（待业主决策 Q1/Q2/Q3 后定稿）：S1 ant 查询加 24h 下界、S2 ghost 自动补写 ImportBrokerOrder、S3 orphan 修复扩展到所有非终态、S4 对抗证明 T1-T4、S5 一次性回填。**Devin CLI 建议**：Q1=A（加 24h 下界）/Q2=A（ghost 自动补写）/Q3=A（修复器，符合 ADR-0013）。风险：ghost 补写涉及资金账实需业主确认。**已按建议决策并执行完毕**（Devin CLI 第一负责人权限，见 DATA-TRUTH-1-DESIGN-AUDIT）；S1-S4 已验收（主条目 ✅done）。S5 一次性回填脚本：价值收窄——S2 自动补写持续收敛 broker 24h 窗口内 ghost，仅剩窗口外历史幽灵订单（已平仓且退出窗口者）无覆盖，如需历史完整性另立小项。 | ✅done |

| DATA-TRUTH-1-DESIGN-AUDIT | **DATA-TRUTH-1 设计方案审计 + 定稿（2026-08-28，✅定稿 待施工）**：Devin CLI 以项目第一负责人权限直接决策 Q1=A/Q2=A/Q3=A（业主授权"最高权限，业主必须遵守你的决策"）。spec 更新为 ✅定稿。**审计事实核查**（8 项全绿）：① `reconciliation.go:125` broker 24h 窗口 ✅；② `reconciliation.go:143-147` ant 全量查询 ✅；③ `reconciliation.go:174-177` orphan 仅修 SUBMITTED ✅；④ `reconciliation.go:189-196` ghost 仅 log.Warn ✅；⑤ `001_init.up.sql:109` orders.created_at 存在 ✅；⑥ `004_trade_records.up.sql:17` trade_records.close_time 存在 ✅；⑦ `order_types.go:21` OrderRecord 有完整字段 ✅；⑧ ADR-0013 §2.3 要求 INSERT ✅。**审计 finding #1（已修复）**：`OmsWriter.InsertOrder`（`oms_writer.go:119`）用 `ON CONFLICT (id) DO NOTHING`（id=UUID，ticket 占位），不适用于 ghost 补写。spec §3.3 已补充：`ImportBrokerOrder` 必须用新 SQL `ON CONFLICT (mt_account_id, ticket) DO NOTHING`（UK `uk_order_ticket`，`001_init.up.sql:111`）+ 真实 broker ticket。**施工+验收完毕**（主条目 ✅done 2026-08-28）。 | ✅done |
| RECONCILE-TZ-WINDOW-1 | **reconcile ant 24h 窗口实际只有 ~16h——`timestamp` 参数编码时区偏移（P2，DATA-TRUTH-1 S1 残余缺陷，2026-09-16 Devin CLI 生产实拍发现）**：`reconciliation.go:159` `cutoff := Clk.Now().Add(-24h)`（后端容器 TZ=Asia/Shanghai，Go time.Local=CST）经 pgx 以 `timestamp`（非 timestamptz）编码 → PG 侧 cutoff 值 = CST wall clock -24h = 比真实 instant 多 +8h → `orders.created_at >= $2`（created_at 为 UTC wall clock，DEFAULT now() 写入）有效窗口仅 ~16h；`trade_records.close_time >= $2` 同理。**症状**：broker 窗口（RPC 传真实 instant，真 24h）仍报告 16-24h 龄订单 → 每轮标 ghost → `ImportBrokerOrder` ON CONFLICT no-op → `repaired` 虚增 + ghost warn 噪声复活（实测账户 40a7655e 每轮 ghosts:2，ticket 387276098 自 16.1h 龄起每轮重复 flag）。**修法**：`cutoff := Clk.Now().UTC().Add(-24*time.Hour)`（最小改动，timestamp 语义对齐）或 SQL 侧 `created_at >= ($2)::timestamptz`（须确认参数类型推断）或列改 timestamptz（migration）。**对抗证明要求**：pin 测试断言生成的 cutoff 为 UTC location；同模式 `timestamp` 列 vs Go time 参数的比较须全仓排查（grep `>= \$` 时间比较）。**Devin CLI 验收通过 2026-09-16**（commit 1efbf678）：`reconcileCutoff()` helper + `.UTC()`；pin 测试×2（location==UTC + SimulatedClock CST 注入断言 UTC 钟面值 hour=2 非 10）；**独立 mutation**：删 `.UTC()` → 双测试精确 RED（location=Local/hour=10）→ restore → GREEN；机检全绿（build/vet/gofmt/test/race×3 4.3s/check-lines 0 errors）。sweep 报告 `docs/audits/tz-timestamp-sweep-2026-09.md` 交付——判定模型正确（写入编码 vs 参数编码一致性），发现 AFFECTED 批次与 MIXED 根因级问题已分立债 TZ-SWEEP-AFFECTED-1 / TZ-MIXED-ENCODING-1。范围干净：3 文件。 | ✅done |
| TZ-SWEEP-AFFECTED-1 | **`timestamp` 列 vs 非 UTC Go time 参数同型 bug 批次（P2，RECONCILE-TZ-WINDOW-1 sweep §2）**：`marketplace/analytics.go:45` `since := time.Now()`（CST，缺 `.UTC()`）驱动 :54/:66/:79/:91/:101/:126/:137/:179/:182 共 ~9 站统计查询（wallet_transactions/user_subscriptions/marketplace_strategies 的 created_at 均为 UTC DEFAULT）→ GMV/手续费/订阅/新策略/退款等周期统计窗口起点偏 +8h，少计 8h 内交易；`repository/backtest_run_worker.go:144` `backtest_runs.created_at` vs `since` 参数（当前无调用方休眠 API，启用前须约束）。修法：`analytics.go:45` 单点改 `time.Now().UTC()`（一处愈全站）；worker 加参数 UTC 约束注释。**Devin CLI 验收通过 2026-09-16**（commit 362d285e）：`analyticsSince()` helper + `.UTC()`；`CountRecentStartsByUser` 参数 `since.UTC()` 归一化（比注释强，M2 覆盖等价已注明——param 侧不可单测，机制经 M1 同类证明）；pin 测试×2（location==UTC + `time.Local=CST` 强制重释钟面断言 -8h 编码偏移）；**独立 mutation**：删 `.UTC()` → 双测试精确 RED（`location=Local`/编码钟面偏 -8h 复现）→ restore → GREEN；机检全绿（build/vet/gofmt/test/race×3 1.1s×2/check-lines 0 errors）。范围干净：3 文件。 | ✅done |
| TZ-PAIRED-CST-COLS-1 | **CST 配对一致性列——禁止单侧翻 UTC（P3 文档债，TZ-MIXED-ENCODING-1 验收分立）**：以下 `timestamp` 列当前为 **CST 写入 + CST 参数读取** 自洽配对，单侧 `.UTC()` 会制造新混合编码——`strategy_schedules.next_run_at`（写 ComputeNextRunAtFromConfig `time.Now()` local-wall-clock / 读 GetDueSchedules `nowTime()=time.Now()` CST 参数；cron/interval 语义本身是本地钟面）、`trade_logs.created_at`（写 auto_trading_settings.go:92 / 读 ListByDateRange·GetTradeLogsByAccount CST start/end 参数）、`account_connection_logs.created_at`、`system_operation_logs.created_at`（混合读侧：CST 参数 + `NOW()-interval` admin 查询——NOW() 读侧已 +8h 虚高）。**规则**：未来任何写入/读取这些列的新代码必须保持 CST 参数一致，或整列一次性迁移 UTC（写+读+存量回填同批）。 **2026-09-18 Devin CLI 设计实查完成**：四列配对坐标全核+**DB `NOW()`=UTC 活库实测**（postgres 容器无 TZ 默认 UTC；backend `TZ=Asia/Shanghai`→`time.Now()`=CST 钟面写 timestamp 列）——`system_operation_logs` 混合读侧 +8h 虚高实证（CST 值比 UTC instant 大 8h，`NOW()-1h` 实捞 ~9h）。写侧坐标：next_run_at=`model/strategy_schedule.go:221`、trade_logs=`repository/auto_trading_settings.go:92`、connection_logs=`model/logs.go:50`、operation_logs=`model/logs.go:135`，全 `time.Now()` CST。**裁定文档债零行为变更**：规则落 `constraints.md` Data Precision（紧邻名义 "Time: UTC" 行——正是需例外注记处）+pitfalls 发现路径指针+schema catalog 四表各一行注记+**写入点 4 处行内注释**（最高杠杆面：好心 dev 加 .UTC() 正是此债要防的形态）。顺手记录：`ListByDateRange` 仓外零调用死 API；`model/auto_trading.go:177/:195` .UTC() 属 RiskConfig/GlobalSettings 他表非配对列。对抗证明如实声明 N/A（文档债无可 pin 运行时行为）。派工单 `docs/audits/builder-handoff-tz-paired-cst-cols-1.md`（S1-S5+4 注释坐标）。**🟦open（施工完成，待独立复审）**：constraints Data Precision 节 CST 配对规则 + pitfalls DATA-TRUTH 段尾指针 + schema catalog 四表 Encoding note 行 + 4 写入点行内注释；零行为变更（仅注释与文档）；对抗证明如实 N/A（文档债无可 pin 运行时行为）。**Devin CLI 独立复审通过 2026-09-19**（施工 aed6ff70）：diff 与 S1-S4 逐格相符——constraints Data Precision 规则 bullet（四列清单+两种合法路径+单侧翻=混合编码根因+NOW()-interval +8h 存量注记）、pitfalls DATA-TRUTH 段尾指针、schema catalog 四表 Encoding note（system_operation_logs 行额外载 +8h 虚高注记）、4 写入点行内注释（logs.go:50/:135+strategy_schedule.go:220 helper 注释覆盖全调用方+auto_trading_settings.go:92）。**零行为变更机械实证**：Go diff 仅注释+gofmt 对齐（存量错位规范化已披露），无 time.Now/.UTC/SQL/NOW()/Exec 语义行变更。机检独立重跑：build ✓/vet model+repository ✓/gofmt 净/check-lines 0 errors/diff --check。对抗证明 N/A 如实声明（文档债）。范围 8 文件零越界。 | ✅done（Devin CLI 验收 2026-09-19，commit aed6ff70） |
| TZ-MIXED-ENCODING-1 | **同列混合时区编码根因级问题（P1→评估，RECONCILE-TZ-WINDOW-1 sweep §3）**：`trade_records.close_time`/`open_time`——实时流路径 `time.Unix(...)` Local→CST 写入（`pipeline_callbacks.go:162`）vs 历史/import 路径 `GetCloseTime().AsTime()` UTC 写入（`mt4/order_history.go:67`、`account_sync_service.go`、`ImportBrokerOrder`）并存；`user_subscriptions.expires_at`——首次购买 `time.Now().Add(30d)` CST（`purchase.go:101`）vs 续费 `now()+INTERVAL` UTC（`service_subscription.go:297`）并存。后果：任何单侧参数编码只对一半行正确——analytics CST 参数对 live 行对、对 import 行窗口 +8h；`live_performance.go:247` UTC 日界桶对 import 行对、对 live 行日界偏 -8h（跨日 PnL 错归）；订阅 expires_at SQL `now()` 对 CST 行晚判 8h 到期（超期仍 active）。修法方向（需决策）：统一写入方 `.UTC()` + 存量数据识别/回填（需区分哪些行是哪编码——created_at 或更新路径可推断）；或列改 timestamptz migration。**先 spike 估算各列受影响行数再定案**。**spike 追加排查点（2026-09-16 决策方补记）**：INSERT 侧 `time.Now()` 写入 `timestamp` 列的更多候选——`refund.go:184/:242/:247`、`settlement.go:52/:305`（settlements 相关列）、`purchase.go:100`（expires_at 已知）——spike 须逐列核类型并估算 CST 行占比。**spike 实测完成 2026-09-16（Devin CLI 亲跑生产库）**：`trade_records` 17780 行中 **10100 行（57%）close_time 为 CST 编码**——签名 `close_time - created_at ≈ +8.00h`（avg 8.00h，bucket 07:00 聚 10081 行），open_time 同型 8892 行；live 流路径 `pipeline_callbacks.go:162` `time.Unix(...,0)` 为 CST 写入源（broker epoch→Go Local），且**每日仍在产生新 CST 行（持续出血）**；`user_subscriptions` 仅 2 行 expires_at 全 UTC（renewal SQL now() 路径），purchase.go:100 CST 写路径尚未产出行；`marketplace_settlements` 7 行/`marketplace_refund_requests` 0 行——量级小。**定案（Devin CLI）**：①止血优先——所有 Go 写入端 `.UTC()`（pipeline_callbacks×2、purchase.go:100、refund.go×3、settlement.go×3 及 grep 全量枚举）；②存量回填——`close_time-created_at ∈ [7h,9h]` 签名行 UPDATE 减 8h（close_time+open_time 同行同写源同减；签名减后≈0 天然幂等）；③部署序——先上写入端修复（新行 delta≈0 不命中签名），回填在其后安全执行；④expires_at 无需回填。**Devin CLI 验收通过 2026-09-16**（commit da85f973）：34 文件全枚举止血——~50 站 `time.Now()/time.Unix()→.UTC()`（pipeline_callbacks open/close_time、purchase expires_at、refund×3、settlement 全列、mt5 fallback、share/reset token、backtest_runs lease+start/finish、ai_*、schedules created/updated、deposits/paper_orders/users.last_login_at 等）+ trade_records 读侧参数同步翻 UTC（列归一后一致）+ migration 278 签名回填（`close_time-created_at∈[7h,9h]` → -8h，幂等，down 为近似恢复已注明）。**NOT-flipped 独立核实**：`next_run_at`（写 ComputeNextRunAt CST / 读 GetDueSchedules nowTime()=CST 配对，单翻写端将致调度偏 8h）、`trade_logs/account_connection_logs/system_operation_logs.created_at`（CST 参数读侧配对）——裁定正确。漏站核查：残余 `time.Now()` 全为纯 Go 用途（cache TTL/throttle/UnixMilli epoch/local-day Format/内存 CapturedAt/broker-RPC 参数）零 DB 写入。**独立 mutation**：删 CloseTime `.UTC()` → TZ=Asia/Shanghai 下 `location=Asia/Shanghai` RED → restore → GREEN；机检全绿（build/vet/gofmt/test/race×3 1.16s+1.09s/check-lines 0 errors）。残余：settlements 7 行 settled_at 不可辨（negligible）；CST 配对列债务分立 TZ-PAIRED-CST-COLS-1。 | ✅done |
...
| DATA-TRUTH-3 | **双账户表并存**：`mt_accounts`（33 列，含状态机+指标，运行时唯一真源）与 `mt_accounts_v2`（15 列，仅凭据+`canonical_subscribed_symbols`）同时存在，命名暗示 v2 是迁移目标但实际 v1 才是主表 → 新代码易写错表。另 `mt_accounts.last_checked_at` 实测全为 NULL（死列）。**2026-09-19 Devin CLI 设计实查完成——裁定落地路径修正**：①`mt_accounts_v2` 实查是 **VIEW 非表**（`112_brokers.up.sql:44-59` DROP+CREATE VIEW，15 列凭据/连接字段，零写入方，`wiring.go:40/:204/:248` 唯一消费方）——"凭据-only 附属"裁定语义不变，落地=新 migration `COMMENT ON VIEW`+wiring.go/mdtick.go 代码注释+constraints 双源规则；②`last_checked_at` 死列**活库实测 16/16 行全 NULL**+全仓零写入方——删法=migration 279 `DROP COLUMN`+`sqlc generate`（`SELECT *` 展开自动丢列，须 `go install sqlc@v1.31.1` 精确版本——当前未安装）+手删 4 读点（admin_repo_accounts.go:28/:52/:108+model/mt_account.go:35+admin_account_handler.go:70-71）；proto `admin_account.proto:59` 字段保留（非破坏恒 unset；删除=API break 另案）。③视图零写入方——"新代码写入审计"落为 constraints 规则"勿扩展视图/勿新增消费方"。对抗证明如实 N/A（schema+文档债）——机械验证=`go build` 编译绿=引用清零证明+grep 零命中。派工单 `docs/audits/builder-handoff-data-truth-3.md`（S1-S6）。**🟦open（施工完成，待独立复审）**：migration 279 up/down（DROP COLUMN+COMMENT ON VIEW）；sqlc@v1.31.1 已安装且 generate 运行，但被 3 个历史 migration 解析错误阻断（138 `ADD CONSTRAINT IF NOT EXISTS` 非 PG 语法、142 引用当时不存在的列——schema 全目录重放不可达，Makefile 此前长期 skipped 兜底佐证）——生成物按 sqlc 将产出的删除手工等价编辑（accounts.sql.go 两 SELECT 列表+两 scan、sqlc_models.go 字段），diff 经通读+gofmt 后精确仅死列零 drift；手写 4 读点删除（model/admin_repo×3/handler 块）；wiring.go 三处+mdtick doc 注释；constraints 双源规则。机械验证：build 绿=引用清零+grep 零命中（proto 产物 `last_checked_at=25` 保留）。**2026-09-16 Devin CLI 裁定**：`mt_accounts`（v1）为运行时唯一真源，维持主表；`mt_accounts_v2` 降级为**凭据-only 附属表**（credentials + canonical_subscribed_symbols），不合并——v2 仅 15 列在独立写入路径，合并成本>收益；处置=① v2 注释标注"credentials-only, do not extend"；② 删死列 `last_checked_at`（migration，实测全 NULL）；③ 禁单侧扩展规则。工作量小，P3。**Devin CLI 独立复审通过 2026-09-19**（施工 de1d0975）：diff 与 S1-S6 逐格相符——migration 279 up（DROP COLUMN IF EXISTS+COMMENT ON VIEW 凭据-only 注记）/down（ADD COLUMN TIMESTAMP 近似恢复+COMMENT IS NULL，类型与 001:51 一致）；sqlc 生成物手工等价编辑（accounts.sql.go 2 SELECT 列+2 scan、sqlc_models.go 字段、model 字段）+4 读点删除（accountCols/2 scan/handler proto-fill 块）；wiring.go 3 站+mdtick.go doc 注释；constraints 双源规则（v1=运行时真源/v2=凭据-only 视图勿扩展勿新增消费方）。**sqlc 阻断声明独立复现**：装 sqlc@v1.31.1 实跑 generate→138 `syntax error at or near "NOT"`（ADD CONSTRAINT IF NOT EXISTS 非 PG 语法）+142 四列不存在——schema 全目录重放不可达属实，手工等价编辑成立（sqlc 对本列的产出面恰为这 4 处）。**机械验证**：grep 死列引用全仓零命中（仅 proto `last_checked_at=25`+生成物按 spec 保留）；视图 SELECT 列清单不含该列→DROP 对视图零影响；gofmt 存量债务机械实证（HEAD~1 版同样不净 19/4/10/56 行，施工方删除反减存量面，范围纪律正确）。机检独立重跑：build✓/vet✓/四包 **211**/check-lines 0 errors/diff --check。对抗证明如实 N/A（schema+文档债——build 绿=引用清零证明）。范围 12 文件零越界。 | ✅done（Devin CLI 验收 2026-09-19，commit de1d0975） |
| MDGATEWAY-5 | P2/P3 批：`staticBrokers` 硬编码 5 家含过期 Exness IP（brokersearch/search.go:22）/ mtapi host 散落硬编码 3 处 / 外汇 session 时间硬编码误判加密·CFD 休市（session_clock.go:36）/ 多处次要 GetError 漏查。**2026-09-19 设计实查**：staticBrokers 回退=mtapi error/空结果→发陈旧表（资金路径假数据，且 mtapi 宕时 Connect 本就走不通→纯 fail-open）；host 字面量实计 6 处；**session_clock 生产零消费=死代码**（仅 market_state_test/pure_test 引用）；GetError 已查 85 处需枚举补漏。**🟦open（施工完成，待独立复审）**：MDG-5a 删 staticBrokers/staticBrokerFilter/Search 回退（err 上浮/空结果如实）；MDG-5b 六处字面量收敛 defaultMT4/MT5Gateway 常量；MDG-5c session_clock.go 删+12 测试块随删；MDG-5d 枚举=全覆盖无新增漏查（orders 12 站/AccountSummary 3/TickValueWithSize 2/QuoteHistory 1 全有 GetError；profit.go:136 为流 Recv；OnQuote 流走重连）。T3 mutation 复辟回退→err=nil+1 静态 Exness RED 实证。| ✅done（Devin CLI 验收 2026-09-19——MDG-5a staticBrokers+回退删净 err 上浮 fail-closed；5b 六处字面量→defaultMT4/MT5Gateway；5c session_clock.go 删（复审复核零消费+banner 残留已清）；5d GetError 枚举全覆盖无新增（抽查 orders.go OrderClose/Delete ErrBrokerRejected 包装正确）；T3 测试判别性复核成立[闭端口→err 传播，复辟回退必含 Exness]）|
...
| EXT-BOUNDARY-WAVE2 | 第二梯队批（P2）：LLM SSE 流无死检测（`systemai/chat_stream.go:115`）+ 链上 monitor 无 stall 检测（`chain/monitor.go:117`）+ SMTP auth 失败吞（`notifier/email.go:105`）+ webhook 门控 transport 错 fail-open（`agent/hooks.go:173`）+ TronScan 第二源 fail-open（`monitor.go:293`）+ LLM `FinishReason` 丢弃截断静默交付（`chat.go:285`）。**2026-09-19 Devin CLI 设计实查**：六项全部实证仍在——①SSE `Timeout:0` 只有首字节 ResponseHeaderTimeout，`scanner.Scan()` 块间无超时且调用方 ctx 无 deadline 兜底（stall=永久挂起）；②monitor scanBlocks 持续 error 只 log，checkpoint 停滞零观测；③SMTP `conn.Auth` 失败 warn 后无认证续发（凭证腐烂静默）；④webhook transport 错→allow，与同文件 `>=400→Abort` 不对称；⑤TronScan 双败→`verified=true` 降级单源自动确认（真钱 fail-open，MANUAL_REVIEW 路径已存在）；⑥`parseChatResponse` 从不读已解析的 FinishReason，length 截断原样交付。派工单 `docs/audits/builder-handoff-ext-boundary-wave2.md`（S1-S6+对抗证明 M1-M5）。**🟦open（施工完成，待独立复审）**（builder commit 见 git log）：S1 看门狗（stallTimeout=effectiveTimeout 钳位默认 5min；atomic lastActivity/stalled；watchdog 关 body 解除 scanner 阻塞；stalled 判定先于 scanner.Err 报因）+chat_stream 拆分（handleStreamResponse 移 chat_stream_scan.go 解 🔴456>450）；S2 lastProgressAt/lastSafeLatest+checkpointStalled 谓词方法+1min stallTicker（链停不误报）；S3 通告 AUTH 才认证、失败显式 error（无 AUTH relay 静默）；S4 transport/请求构造错→Abort（对称 ≥400）；S5 删 verified=true 降级→createManualReviewDeposit helper 三路共用；S6 length→non-transient failoverErr。mutation：M1 删看门狗→挂起 RED；M2 S4 fail-open 复辟→RED；M5 S3 warn-continue 复辟→RED；M4 删 length 检查→err=nil RED；M3 恢复 verified=true→processEvent 端到端用例判别（ConfirmDeposit nil pg panic）。边界如实记：execCommand 非-2 退出 fail-open 同族另债；流式 FinishReason 消费方处置；content_filter 语义。**2026-09-19 Devin CLI 独立复审：通过**——机检独立重跑 build/vet/四包 137+race 137/check-lines 0 errors/diff --check 全绿；gofmt 偏差审计方规范化（e5f48850，施工"保持既存未格式化态"声明不实——旧版干净其编辑引入）；mutation×6 独立复验全 RED→restore→GREEN：M1 stalled 判定失效→raw `use of closed` 症状冒名；M2 fail-open 复辟→Abort=false；M5 warn-continue 复辟→`smtp mail: use of closed` 非 `smtp auth`；M4 删 length 检查→err=nil 截断交付；M3 `verified=true` 复辟→R1 E2E nil-ConfirmDeposit panic；R2 谓词守卫摘除→链停不误报用例 RED（另两用例不受影响，断言级精确）。首轮覆盖缺口实证记录：S5 helper 直测不判别（不触 processEvent）+S2 测试内联重写谓词（零生产代码）——两变异同施 20/20 绿，返修 R1 processEvent E2E+R2 谓词抽 `checkpointStalled` 方法修复判别性。 | ✅done（Devin CLI 验收 2026-09-19——施工 ac5b5773+返修 80470f83）|
| LLM-CONFIG-1 | 信任缺陷：LLM `Temperature`（chat.go:153 硬编码 0.3）+ per-provider `TimeoutSeconds`（service.go:29）是 DB+UI 可配但**从不消费**的死字段 → 用户以为可控实则无效，违反"配置即权威"，影响策略生成可复现性（P2 信任）。**2026-09-19 Devin CLI 独立复审：条目漂移——缺陷已于 `43f1e20a`（2026-09-08，FIX-2026-09-08-TEMP-RETRY 业主实测 kimi-k3 400 触发）修复，登记时未察觉**。全链实证：UI `AdvancedForm.tsx` temperature 滑杆+timeout_seconds 输入（draft 可配）→ DB `row.Temperature`/`row.TimeoutSeconds`→`chat_failover.go:212` populate（`defaultTemperature`/`effectiveTimeout` 解析+钳位）→`chat.go:258` `http.Client{Timeout: effectiveTimeout(p.timeoutSeconds,...)}` + `chat.go:263→158` `p.temperature`→请求体 `Temperature`。设计语义诚实：`t<=0`=未配→平台默认 0.3（非伪造——UI 侧"未配"与"设 0"无法区分时的既定解析，reasoning 模型 temperature=1 由 400-retry 自愈处理）。测试钉：`TestDefaultTemperature`/`TestEffectiveTimeoutClamp`（0→def/-5→def/300→300s/1→5s/99999→10min）/`TestTryChatCompletionTemperatureRetry`（配置 0.2 随重试保持进请求体）。**独立 mutation**：`defaultTemperature` 改 `if false`→`TestDefaultTemperature` 精确 RED→restore→4/4 GREEN。 | ✅done（Devin CLI 验收 2026-09-19——修复 `43f1e20a` 已落地+mutation 实证判别覆盖）|
...
| MAGIC-ENRICHMENT | **Magic 列 `-` 三条断裂修复（2026-08-28，✅done 已部署）**：前端持仓+历史 tab magic 列永远 `-`。根因三条独立断裂：① **断裂 1（history magic=0）**：broker OnOrderUpdate 实时事件不携带 magic → `buildClosedTradeRecord` 的 `o.UpdateMagic` 永远为 0 → trade_records.magic_number=0（近 4 天 273 条全 0）。`orders` 表同 ticket 有 magic=-1486243899（下单时写入）。修复：`buildClosedTradeRecord` 当 `UpdateMagic==0` 时从 orders 表按 ticket 回查 magic（`MtHubService.GetOrderMagic` 方法，`service_order_magic.go`）。新增 `MagicLookup` 接口（`pipeline_callbacks.go:122`）让测试可注入 stub。② **断裂 2（positions proto 缺字段）**：`OrderUpdateEvent` proto（`stream_event_trade.proto:7`）无 `magic_number` 字段 → `emitPositionSnapshot` 无法传 magic → 前端 `OrderUpdate` 接口无 `magicNumber` → 永远 undefined。修复：proto 加 `int32 magic_number = 17` + `emitPositionSnapshot` 传 `pos.Magic` + `orderRecordToUpdateEvent` 传 `rec.Magic` + 前端 `OrderUpdate` 加 `magicNumber?: number` + `mapOrderToPosition` 映射 `magicNumber`。③ **断裂 3（schedule_id 全 NULL）**：是断裂 1 的后果——magic=0 → `ResolveScheduleID` 返回 nil。修复断裂 1 后新 trades 自动解析。已有 252 条 trades 用 SQL 回填（`UPDATE trade_records SET magic_number = o.magic_number, schedule_id = ss.id FROM orders o LEFT JOIN strategy_schedules ss ...`），65 条得到 schedule_id。**对抗证明（独立重跑 RED→restore→GREEN）**：断裂 1 `TestBuildClosedTradeRecordMagicEnrichment` + `TestBuildClosedTradeRecordMagicNoEnrichmentWhenBrokerSends`；断裂 2 `TestEmitPositionSnapshotMagicNumber` + `TestOrderRecordToUpdateEventMagic`（审计补加——初版漏断裂 2 对抗测试）。门禁：build/vet/test/race×3/check-lines 0 errors/tsc --noEmit 全过。**部署**：docker compose build+up backend 2026-08-28，container healthy。 | ✅done |
...
| TRUST-1 | Demo 账户（虚拟金）与真实金账户战绩混为一体公开展示无标注 → 信任护城河风险（P1，业务决策：real-only/标注/允许）| 🟦open（2026-08-28 S1-S7 施工完成，待独立复审）：**S1** adapter mt4+mt5 FetchAccountInfo+FetchBrokerInfo 读 `s.GetType()`（mt4 enum→`Mt4AccountTypeToString` / mt5 string→`NormalizeAccountType`，helper 放 mdtick 包）；**S2** `MTAccountInfo`+`BrokerInfo` 加 `AccountType` 字段；**S3** `AccountInfoUpdate` 加 `AccountType` + `UpdateAccountInfoTx`/`UpdateAccountInfo` SQL 写 `account_type` + 新增 `UpdateAccountType` 方法（OnBrokerInfo 路径，不改 sqlc `UpdateAccountMetrics` 签名）+ `pipeline.go:282` OnBrokerInfo 调用；**S4** `CreateAccount` 传 `info.AccountType`；**S5** `LinkLiveAccount` real-only 校验（Q1=A）；**S6** migration 276（daily+summary 表加 `account_type` 列）+ `LivePerformanceCollector.cache` 扩展为 `map[string]livePerfCacheEntry{StrategyID,AccountType}` + `loadCache` JOIN mt_accounts + `OnProfitUpdate` 跳过非 real + `UpsertDailyPerformance`/`recomputePerformanceSummary` 写 account_type + `leaderboard.go` `lps.account_type = 'real'` 过滤；**S7** 前端无改动（Q1=A）。**对抗证明 11 测试 RED→restore→GREEN**：T1 `TestMt4AccountTypeToString`、T2 `TestNormalizeAccountType`、T3 `TestMTAccountInfo_HasAccountTypeField`+`TestBrokerInfo_HasAccountTypeField`、T4 `TestUpdateAccountInfoTx_WritesAccountType`+`TestUpdateAccountType_MethodExists`+`TestAccountInfoUpdate_HasAccountTypeField`、T5 `TestLinkLiveAccount_RejectsDemo`、T6 `TestLeaderboard_FiltersRealOnly`、T7 `TestOnProfitUpdate_SkipsDemo`+`TestUpsertDailyPerformance_WritesAccountType`。门禁：build/vet/race×3/check-lines 0 errors/git diff --check clean。**风险/gap**：部署后需实测 12 unknown 账户回填为 real/demo/contest；S9 一次性回填脚本待编写。**验收+部署完成（2026-08-28，commit 897e6c8c 验收 / b7689f97 部署验证）**：migration 276 自动执行；OnBrokerInfo 60s 内回填 real=4/demo=3/contest=4。**S9 裁定不建（2026-09-16 Devin CLI，DB 实拍）**：现存 2 unknown（login 17001900/95300910）均为 2026-08-03/04 起 disconnected 死账户——0 条 marketplace_live_performance、0 条 strategy_schedules 绑定，对 real-only 过滤零影响；`unknown` 被 `='real'` 过滤天然排除（fail-closed 方向）；重连即由 OnBrokerInfo 自动回填覆盖。脚本边际价值为零，关闭。 | ✅done |

| TRUST-1-DESIGN | **TRUST-1 设计方案落档（2026-08-28，🟦open 待业主决策）**：spec `docs/spec/fix-2026-08-28-trust-1-demo-real-account-distinction.md`。根因 3 层：A `mt_accounts.account_type` 列（migration 011，DEFAULT 'unknown'）无任何代码路径写入——`UpdateAccountInfo`/`CreateAccountTx`/`UpdateAccountMetrics` 均不写 account_type；B broker `AccountSummary.Type` 字段（MT4 enum Real=0/Contest=1/Demo=2，MT5 string "real"/"demo"）是权威数据源，但 `FetchAccountInfo`（mt4/connection_account.go:48 + mt5/connection_account.go:44）**丢弃 `s.GetType()`**，`MTAccountInfo`/`BrokerInfo` 结构体无 AccountType 字段；C `marketplace_live_performance` 表无 account_type 列，`LinkLiveAccount` 不校验 account_type，`LeaderboardEntry` 无 demo/real 字段，前端无标注。实测 12 个 connected 账户 account_type 全 'unknown'。修复方案 S1-S9（待业主决策 Q1/Q2/Q3 后定稿）：S1 adapter 读 Type、S2 MTAccountInfo 加字段、S3 AccountInfoUpdate 加字段+写入、S4 CreateAccount 传值、S5 LinkLiveAccount 校验（Q1=A 时）、S6 marketplace_live_performance 加列+过滤、S7 前端标注（Q1=B 时）、S8 对抗证明 T1-T5、S9 一次性回填脚本。**Devin CLI 建议**：Q1=A（real-only，符合 AGENTS.md §1 "实盘战绩公开"）/Q2=A（broker RPC 权威，符合"服务器唯一真相"红线）/Q3=A+B（重连自动回填+脚本加速）。**已按建议决策并执行完毕**（Devin CLI 第一负责人权限，见 TRUST-1-DESIGN-AUDIT）；S1-S7 已验收+部署（897e6c8c/b7689f97）；S9 经实拍裁定不建（见 TRUST-1 主条目）。 | ✅done |

| TRUST-1-DESIGN-AUDIT | **TRUST-1 设计方案审计 + 定稿（2026-08-28，✅定稿 待施工）**：Devin CLI 以项目第一负责人权限直接决策 Q1=A（real-only）/Q2=A（broker RPC 权威）/Q3=A+B（重连自动+脚本加速）。spec 更新为 ✅定稿。**审计事实核查**（8 项全绿）：① `mt4/mt4.pb.go:2254` `AccountSummary.GetType() AccountType` enum ✅；② `mt5/mt5.pb.go:3718` `AccountSummary.GetType() string` ✅；③ `mt4/connection_account.go` 0 处 GetType（丢弃）✅；④ `mt5/connection_account.go` AccountSummary 不读 Type（丢弃）✅；⑤ `account_lifecycle.go` 0 处 account_type（不写入）✅；⑥ `live_performance.go` 0 处 account_type（LinkLiveAccount 不校验）✅；⑦ `214_marketplace_live_performance.up.sql` 0 处 account_type（表无列）✅；⑧ `marketplace_live_performance_summary` 也无 account_type 列 ✅。**审计 finding #1（已修复）**：spec §3.7 原只说给 `marketplace_live_performance`（daily 表）加 account_type 列，但 leaderboard 查的是 `marketplace_live_performance_summary`（summary 表，`leaderboard.go:90`），summary 表也需加列。spec §3.7 已补充 summary 表加列 + `recomputePerformanceSummary` 写入 account_type。**审计 finding #2（已修复）**：`UpsertDailyPerformance` 当前不查 mt_accounts，spec §3.7 已补充方式 A（扩展 LivePerformanceCollector.cache 存 account_type，OnProfitUpdate 传入）优于方式 B（子查询）。**施工+验收+部署完毕**（897e6c8c 验收 / b7689f97 部署，11/14→14/16 账户自动回填）。 | ✅done |
...
| LIVE-REDESIGN-2TAB | Live 页 3-tab → 2-tab 重设计（策略为中心）：Tab1「我的策略」双流 join（watchSchedules×watchActive 按 scheduleId）+ 行展开（持仓/信号/日志/配置）+ 操作全按 schedule_id；Tab2「运行历史」。删 Active Runs/Schedules tab。**前置全部就绪**（§0 MARGIN-GATE-2 ✅ / §2 magic+GetSchedulePositions ✅ / 双流字段 ✅）| ✅done（2026-08-15 施工 `acd5fff1`+`7aeb83eb`+`959f3569` 初版交付 + `00a7f6a4` 返工修复。**返工 3 项全部修复 + 红队自审通过**：① **UI-4 对抗测试同源**——抽 `joinSchedulesWithActive`/`findOrphanRuns` 到 `components/live/strategyJoin.ts` 纯函数模块，`LiveStrategyPage` 调用该模块（非内联），UI-4 测试 import 同一函数（非拷贝）。**对抗证明 ×2**：a) 中和 join body（`return schedules.map(s => ({...s}))` 不带 active）→ 测试 RED（`expected undefined to be defined`）✓ 恢复 GREEN ✓；b) 中和 findOrphanRuns（`return activeStrategies` 不过滤）→ 测试 RED（`expected [ …(2) ] to have a length of 1 but got 2`）✓ 恢复 GREEN ✓。新增 2 个 `findOrphanRuns` 测试（无匹配 schedule + 空 scheduleId）。② **4 个 `strategy.schedules.*` i18n key 补齐**——`status.disabled`/`actions.runNow`/`deleteConfirm.title`/`table.schedule` × 5 locale 真翻译（en/zh-cn `已禁用`/zh-tw `已停用`/ja `無効`/vi `Đã tắt`），`i18n-build.ts` 重生，资源文件验证 en/zh-cn base.ts 嵌套对象包含 4 key ✓。③ **死代码消除**——`isLogButtonDisabled`/`isHealthButtonDisabled` 从 `LiveStrategyPage.tsx` 移至 `strategyJoin.ts`，`MyStrategiesTable` actions 列 :182/:186 改用（替换内联 `disabled={!row.id}`），UI-3 测试 import 同模块。`LiveStrategyPage.tsx` 不再导出这两个函数（grep 零引用 ✓）。**红队自审 11 项**：① map 覆盖（同 scheduleId 多 active 取最后，语义正确）；② `!a.scheduleId` 捕获 undefined/null/''（正确）；③ `isLogButtonDisabled('')` = true（一致）；④ delete 按钮 `!row.id` 语义等价但函数名不适用（保持内联，正确）；⑤ ScheduleExpandedRow 无 unused import；⑥ ActiveStrategy 在 MyStrategiesTable Props.orphanRuns 仍需要；⑦ JoinedRow 全部从 strategyJoin.ts 导入（零本地定义）；⑧ `?tab=active|schedules` 零残留；⑨ OrphanRunsTable 自包含；⑩ `secondsSince` 返回 Infinity 时比较正确；⑪ i18n 全量 key 扫描无遗漏。**门禁**：tsc 0 / vite build green / vitest 10/10 / file lines 全 <250（LiveStrategyPage 228 / MyStrategiesTable 228 / ScheduleExpandedRow 164 / strategyJoin 28）。**⚠️待Claude复审**：返工 3 项 + 红队自审 11 项已自验，但需独立复测确认）。**2026-09-16 Devin CLI 独立复测**：`strategyJoin.ts`/`OrphanRunsTable.tsx` 在仓、ExpandedRow 全部从 strategyJoin 导入、`?tab=active|schedules` 旧式零残留（现存均为 `?tab=strategies` 新约定）；vitest live-ui-antitest 12.08s 全绿 + tsc 0 err → ✅done|

| I18N-MIXED-1 | `marketplace_messages_published: '策略 published to marketplace!'` + `marketplace_messages_publish_failed: '失败 to publish strategy'`（base_zh-cn.textproto）中英混杂（P3 i18n，2026-08-15 审计 LIVE-REDESIGN-2TAB 时发现）。**2026-09-19 Devin CLI 设计实查——债面远超登记**：①**Class A**（i18n-check 可检）：zh-cn 全 locale **216 条**值与 en 完全相同（`npx tsx scripts/i18n-check.ts --strict` 报 Untranslated，base 占 150+）；②**Class B**（不可检）：base_zh-cn **~95 条**中英混杂模板（`失败 to X`×25/`删除 this Y`/`Top 策略 by Revenue`/`Please enter 基础 URL` 等）；③缺字段 `strategy_gen.exec_feedback_placeholder`；④**门禁缺口**：`i18n-check.ts --strict` 存在但 pre-commit/CI 零接线（strict 全量 1519 错误含 vi/zh-tw 缺字段无人问津）。管线：`i18n-translate-zh-cn.ts`（DICT exact-match 优先已存在 :226，但 main() 仅处理 base）→ `i18n-build.ts` → `frontend/src/i18n/resources/zh-cn/*.ts`（git 跟踪产物）。派工单 `docs/audits/builder-handoff-i18n-mixed-1.md`：S1 DICT 补全 ~200 整句条目+main() 泛化全 locale 文件；S2 Class B 附录 B ~99 行直接替换；S3 缺字段补齐；S4 i18n-check 加 `--locale` 旗标+pre-commit 接 zh-cn strict。附录 C 白名单（品牌/语言名/指标符号/AI prompt 内部串 {{text}}系）。zh-tw 212/ja/vi 缺口另立 I18N-MIXED-2。 | 🟦open（设计实查 09-19→施工 9a641ea2→**复审返修**：核心面合格[DICT+209/泛化/--locale/门禁/ClassB清零/M3独立实证]，但重生存活检查缺——43 key/locale 生成文件私货未入SSOT被丢[diag×38+logs.signalType×5 全真译文回退]，R3-R6 返修规格 docs/audits/rework-i18n-mixed-1.md）。**🟦open（R3-R6 施工完成，待独立复审）**（builder commit 见 git log）：R3 43 keys×5 locale 吸收 SSOT——textproto 追加（en 原文/四 locale 历史真译文逐字恢复）+base_map 38 条/logs_map 5 条映射（完整路径 strategy.live.diag.* 含 lifecycle./execState./source. 嵌套）+重生成；R4 DICT 补 5 条用户面漏译（tour×2/iCustom/DLL/Acknowledge）+删 5 条 stopgap EXEMPT_PATTERNS（保留 Example:\n prompt 豁免）；R5 close_position 孤儿三行删除（消费方零实证，3 warnings→0）；R6 i18n-key-presence.test.ts vitest 守卫（43 路径×5 locale 非空+zh-cn≠en 跨 locale 回退守卫+S3 placeholder+R4 五值生效）。mutation：R3 删 order_truth key→重生成→R6 两断言 RED 实证。门禁：zh-cn strict 0 errors 0 warnings/tsc 0/vitest 5 passed/keydiff 0 lost。 | ✅done（**2026-09-19 Devin CLI 独立复审通过**——施工 `9a641ea2`+返修 `f46cb8be`：①**语义 keydiff 独立复算**：5 locale base.ts 0 lost/+38、logs.ts 0 lost/+5、zh-cn 仅 5 条 R4 值变更精确命中；②**43×5=215 键值与 9a641ea2~1 历史值逐字相符**（订单真相/注文の真実/訂單真相/Thực tế Lệnh 等全真译文恢复，lifecycle./execState./source. 嵌套 snake 值保全）；③**R4** DICT 五值逐字落地+5 条 stopgap EXEMPT_PATTERNS 删除（Example:\n prompt 豁免保留）；④**R5** zh-cn strict **0 errors 0 warnings**（close_position 孤儿三行删除，消费方零实证复核）；⑤**R6** key-presence vitest 5 测试——**独立 mutation 复验**：删 `strategy_live_diag_orderTruth`→i18n-build→断言精确 RED→restore→GREEN；重生成确定性实证（rebuild 后工作区与提交逐字节一致）；⑥机检独立重跑：tsc 0/vitest 全集 **33 文件 213 测试全绿**/check-lines 0 errors/diff --check 净；⑦**首轮已实证**：pre-commit 门禁端到端（篡改 zh-cn 值→exit 1 精确拦截）、M3 placeholder 删除→Missing error。camelCase textproto key 偏离规格 snake 但有存量先例（monitoring_uptimeDays/sre_killSwitch_title），接受。范围零越界；未部署、未 push）。**返修 R7（复审修订记录 2026-09-19）**：R7a 删 base_zh-tw.textproto 五个死孤儿 key（sre_breakers/sre_canary/strategy_live_close_position/_success/_failed——en/map 零对应、frontend 消费方 grep 零引用复核实证，sre_breakers_state_*/columns_* 有效 key 未动）→zh-tw strict warnings 5→0；R7b strategy_import_e_a_import_success 三语修正（en 恢复英文原句、zh-cn/zh-tw 引号内对齐各 locale apply 按钮实际译名「应用到编辑器」/「套用至編輯器」）；重生成+zh-cn/zh-tw 双 strict 0/0 复验。 | ✅done（R7 复审通过——验收落档 518152c3，计入 I18N-MIXED-2 相位1）|
| I18N-MIXED-2 | zh-tw/ja/vi locale 同类未译缺口（I18N-MIXED-1 设计实查时发现）：zh-tw base 212 条多词英文未译（比 zh-cn 更重）、ja/vi 同类 + vi/zh-tw 另有缺字段（`i18n-check --strict` 全量 1519 errors 主要来源）。业主指令"信息以中文显示"优先 zh-cn/zh-tw；vi/ja 排期待裁定。修复路径同 I18N-MIXED-1（DICT 表需 per-locale 重建——translate 脚本仅 zh-cn）。 | 🟦open（2026-09-19 登记→**设计实查完成**——三 locale 债面量化：zh-tw 424[300未译+124缺]/ja 424[300+124]/vi 442[318+124]；Missing 124 三 locale 同构=admin×116+base×5+strategy_gen×3 en 新功能键未传播；混杂类 zh-tw 仅 1 候选/ja/vi 零——Class B 为 zh-cn 特有。**分相位施工：相位1 zh-tw**[中文指令范围]，派工单 docs/audits/builder-handoff-i18n-mixed-2.md 含全 424 条清单；zh-tw 验收后续相位 ja/vi 同模式）。**相位 1 zh-tw ✅done（2026-09-19 Devin CLI 独立复审通过——施工 `bf35cc82`+返修 `67b80ad8`**：①zh-tw strict **0/0**（300 未译全繁中 DICT+124 Missing 补齐 admin×116/base×5/gen×3）+zh-cn 仍 0/0 无回归；②引擎共享化 `i18n-translate.ts --locale`+`i18n-dict/{zh-cn,zh-tw}.ts`+zh-cn 薄壳不断链；③语义 keydiff zh-tw 0 lost/+124 added/296 值变更精确；④繁体质检过（訂單真相/發佈/帳戶/執行/網路无简体残留）；⑤**独立 mutation×2**（值回英文→Untranslated RED；删 placeholder→Missing RED）；⑥R7 五死孤儿删 warnings 5→0+import_success 三语修正（en 源错语言中文句恢复英文——复审发现**第二 drift 类**：strategy_extra.ts override 层遮蔽 SSOT key，用户面实由 extra 渲染本已正确，builder 选「套用至編輯器」与 base 内自洽但被遮蔽，采纳为死层无害；en 修正确属 hygiene）。守卫测试 7 绿+tsc 0+全 vitest 不回归。遗留：**相位 2 ja/vi** 待开工同模式；commit title 误标 I18N-MIXED-1（实为 MIXED-2 工作）簿记噪声如实记）。**相位 2 ja+vi ✅done（2026-09-19 Devin CLI 独立复审通过——施工 `3eb5c69a`+返修 `2388e919`**：①ja/vi strict 各 0 errors→R8 后全量 **0/0**（DICT ja×276/vi×278、Missing 124×2 补齐、语义 keydiff 0 lost/+124added/~295chg、日越译质过）；②泛化引擎零改动直接服务两 locale 验证成立；③**独立 mutation×2**（ja 回英文→Untranslated RED、vi 删 placeholder→Missing RED）；④R8 六死孤儿删（sre_breakers/canary×2 locale+marketplace_load_failed snake 重复行——builder 自造 typo）；⑤pre-commit 已升全量 strict（去 --locale）四 locale 全覆盖。报告不实项二次记录："0 errors 0 warnings"实报仅为 errors（warnings 漏检相位 1/2 同型）。**I18N-MIXED-2 整债 ✅done**——五 locale strict 全 0/0）|
...
| VM-COMPILER-SEMANTICS-3 | **switch jump-table 返工破坏 default 顺序与 break 栈清理（P1）**：`compile_loops.go:115-187` 将 `default` 从原始 `s.Cases` 中抽出并强制放到所有 regular cases 之后；MQL/C 的 `default` 可以出现在中间，匹配/落入 default 的 fallthrough 顺序因此错误。同时 `break` patch 到 `endPC`（位于最终 `OP_POP` 之后），会绕过 switch value 的 POP，留下栈值；现有 `TestVM_Audit_SwitchFallthrough` 只检查最终 global 值，未检查栈，仍会假绿。修复：保留 case 原始顺序并建立 dispatch targets，所有退出路径（包括 break）必须消费 switch value；补 default-before-case、break 后栈深度/重复执行测试及删除 patch 的 RED/GREEN。 | ✅done（2026-09-16 Devin CLI 独立复审通过）：builder commit `c5d1a7e0`。S1 保留 `s.Cases` 原始顺序（default 不抽出到末尾，作为 fallthrough target 参与顺序）+default 首位 emit skip JMP 防 default body 无条件执行；JMP_IF_FALSE fallthrough case 跳 `caseBodyStarts[caseIdx+1]`（含 default body），normal case 跳 `regularCaseStarts[ri+1]`（跳过 default 无 comparison）或 defaultBodyStart 或 POP。S2 `popPC = len(Code)` → emit OP_POP → endJumps/breakJumps patch 到 popPC（OP_POP 位置），break 执行 OP_POP 消费 switch value（旧代码 patch 到 endPC 绕过 OP_POP 留栈）。S3a SwitchFallthrough 加栈深度断言 `len(vm.stack)==0`；S3b SwitchDefaultBeforeCase（default 中间 fallthrough → g_result=1010，旧代码 fallthrough 到 case2 → 20 错误）；S3c SwitchBreakStackCleanup（break 后 switch 外语句不被栈残留污染 → g_result=15 + stack=0）。**独立对抗证明×2**：① fallthrough target `i+1`→`i+2` 跳过 default → S3b RED `g_result=20`；② break patch 回 endPC 绕过 OP_POP → S3a/S3c RED `stack depth=1`。**机检独立复测**：build ✓ / mql2go 386 ✓ / race×3 1158 ✓ / golden/e2e 7 ✓ / vet ✓ / gofmt ✓ / check-lines 0 errors / diff --check clean。 | ✅done |
...
| VM-TEST-EVIDENCE-3 | **新增测试仍有语义错位**：`vm_audit_test.go:2114-2141` 名为 `FloorDivByZero` 但源码执行的是 `/` 而不是 `//`，因此没有证明 `floorDiv`；`TestVM_Audit_SwitchFallthrough` 在 break 走到 `endPC` 后的残留栈值也未断言。修复：测试表达式必须命中目标 opcode/分支，并验证 observable stack/control-flow invariant；对抗删除关键实现后必须因目标行为失败。 | ✅done（2026-08-25 返工已验收，见返工表 VM-TEST-EVIDENCE-3 行；2026-09-16 对账修正主表漂移）|
...
| QS-1.4 | **Python `bool(x)` 编译为 `x != 0` → `bool(None)=true`（P0 语义，2026-09-16 VM 深度审计 P1-4）**：`compile_py_expr.go:165-172` + `value.go:153-168` 混合 None/Int `Equal=false`。修法：编译为双重 `OP_NOT`（复用 `IsTrue()`），零新 builtin/opcode；顺手修正 `vm_helpers.go:250` 过时注释。spec `docs/spec/vm-pipeline-quality-stability-improvement-plan.md` §3.1。【最终决策：Devin CLI（[角色:决策终] 激活）】**施工完成 2026-09-16**（施工方按 `docs/audits/builder-handoff-qs-1.4.md` S1–S3）：S1 `compile_py_expr.go:165-172` `bool(x)` 由 `ExprBinary "!="` 改为 `ExprUnary{"!", ExprUnary{"!", x}}`（两条 OP_NOT → `IsTrue()` 语义）；S2 `vm_helpers.go:250` 注释更正为"到达此处 = builtins 表有名但无 handler 且注册表未标 fatal；StatusUnsupported（Object/Chart/File 等）编译期由 `compile_expr.go:386-393` 拒绝且 `isFatalUnimplemented` 标 fatal，不会到此"；S3 新增 `compile_py_bool_test.go` 行为级测试 `TestCompilePython_BoolSemantics`（bool(None)/0/0.0/""/False→false，bool(1)/"a"/0.5→true，bool() 零参→false）+ `TestCompilePython_BoolCompilesToDoubleNot`（IR 双重 `!` 形态守卫）；既有 `TestCompilePython_BoolConversion`（`compile_py_test.go:1740`）断言旧 `!=` 形态同步更正为新形态。**对抗证明**：先红——S1 实施前新测试 RED（r_none/r_dec/r_empty/r_false=true want false，IR op="!="）；mutation——恢复 `ExprBinary "!="` → `TestCompilePython_BoolSemantics` RED + `TestCompilePython_BoolCompilesToDoubleNot` RED → restore → GREEN。机检：`go build ./...` ✅、`go test ./tools/mql2go/...` ✅、`-race -count=3` ✅、`check-file-lines --strict` 0 errors（57 警告均 pre-existing，本任务文件零命中）、`go vet` 零输出、gofmt 本任务文件零新增（包内 pre-existing drift 未动）。**遗留疑问**：`Decimal("0")` 经 `case "Decimal"` 直接返回 `StringVal("0")`（编译期不产 ValDecimal）→ `bool(Decimal("0"))`=true 与该层失真自洽，但暴露 `Decimal()` 构造器本身的既有失真——超出 QS-1.4 边界（"不动 Decimal 分支"），未处理，待决策方裁定是否另立条目。**Devin CLI 验收通过 2026-09-16**：独立重跑 `go build` / `go test -count=1` / `go test -race -count=1` / `check-file-lines --strict` / `vet` 全绿；mutation（恢复 `!=`）独立重跑 → 3 测试语义级 RED（`r_none=true want false`）→ restore → GREEN。裁定：Decimal 构造器失真成立立债 `PY-DECIMAL-CTOR-1`；`BoolConversion` 断言最小同步批准。流程记录：施工方 commit 含交接层文件改动（违 `dual-terminal-roles.md` §4），内容经独立核验准确保留，下不为例。 | ✅done |
| PY-DECIMAL-CTOR-1 | **Python `Decimal("0")` 经 `case "Decimal"` 直返 `StringVal("0")` → 构造器产 ValString 非 ValDecimal（P2 类型失真，QS-1.4 复审发现）**：`backend/tools/mql2go/compile_py_expr.go` `case "Decimal":` `return &args[0]` 原样透传；`Decimal("0")` 得 string → `bool(Decimal("0"))=true`（真 decimal 应为 false）、`str()`/类型判定失真。修法（待施工）：`Decimal(...)` 参数为字符串/数字字面量时应产生 ValDecimal（常量折叠或 builtin 调用）。 **2026-09-18 Devin CLI 设计实查完成**：缺陷实证比原载更重——`ToDecimal()` 无 `ValString` 分支（default→0）+`argD` 同通道，`Decimal("0.1")` 在数值语境**静默归零**（`ctx.ask()-Decimal("0.0050")`→ask-0 sl 静默失效、`broker.buy(lot=Decimal("0.1"))`→invalid volume）、`bool(Decimal("0"))`=true 应 false、`Decimal("0.0")==Decimal("0")`=false 应 true。裁定**常量折叠**：字面量按 Val.Kind 折叠（String→NewFromString err→`c.errorf` 编译拒绝/Int→NewFromInt/Bool→1,0/None→errorf TypeError 等效）；非字面量→`ExprCall{StringToDouble}`（已注册 implementedPlatform，残留如实记录：garbage→0/bool→0）；零参 `Decimal()`=Decimal("0") Python 合法语义保留。**自审计**：StringToDouble 经 LookupAPI 验证 StatusImplemented；存量 12+ 处 Decimal 测试全编译断言零 IR 形态断言——爆炸半径干净。**同族观察**（边界外）：`int`/`float`/`str` 同型透传（:150-164），`int("42")`→StringVal→ToInt→0 同静默零族——另债处置。派工单 `docs/audits/builder-handoff-py-decimal-ctor-1.md`（D1-D6+S1-S3+mutation×3+9 测试）。 **Devin CLI 独立复审验收通过 2026-09-18**（commit `e928722c`）：`case "Decimal"` 常量折叠按 Val.Kind 分派（String→NewFromString err→`c.errorf` 编译拒绝/Int→NewFromInt/Bool→1,0/None→errorf）+非字面量 `ExprCall{StringToDouble}`+零参保留；`compile_py_test.go:1910` 存量断言从 ValString 翻正 ValDecimal（实查漏计的一处 arg.Val.Kind 形态断言，施工方正确发现并翻转——该断言钉的恰是本债缺陷本身）。9 新测试全判别。机检独立重跑：build ✓ / mql2go **742** / race×3 **2226** / vet / gofmt 三文件净 / diff --check。**独立 mutation×3**：①整体还原透传→8 RED（Bool/Arithmetic/Equality/IntLiteral/InvalidLiteral/None/NonLiteral，ZeroArg 正确存活——两形态等价）②仅删 ValString 折叠→5 字符串依赖测试 RED（IntLiteral/None/NonLiteral/ZeroArg 不受影响仍过）③非字面量还原透传→仅 NonLiteral RED；均 restore→GREEN，树与 `e928722c` 逐字节一致。边界：int/float/str 透传未动（另债）、无新增 builtin/registry、StringToDouble 语义未动。范围 3 文件零越界。 | ✅done |
| QS-1.6 | **read-after-write verify 成功后 `return barrierConfirmed` 绕过状态机（P0 状态机 SSOT，审计 P1-7）**：`mutation_coordinator.go:330-334`，barrier 停在 `acceptedUnconfirmed` 即被 Release。v1 修法 `Reconcile(true)` 在该状态是 no-op（`trade_barrier.go:333`），否决。修法：新增 `TradeBarrier.ConfirmByAuthoritativeRead()` 正式迁移；施工方自报中回答 `NotifyConfirmationEvent` 为何未迁移，由决策方回填此处（施工者不写交接层）。spec §3.2。【最终决策：Devin CLI（[角色:决策终] 激活）】**Devin CLI 验收通过 2026-09-16**（commit 5be48f30）：根因核实——coordinator 把动作名当 updateType 传入 `NotifyConfirmationEvent`，`"cancel"` ∉ cancel 兼容集 {close,pending_close}（`trade_barrier.go:101-104`），open/close/modify 名字恰好在自身兼容集故不暴露；第二分支 `ticket==0` 被 `:219` 早退。修法与设计一致：`ConfirmByAuthoritativeRead` 持锁迁移 {submitting,acceptedUnconfirmed,outcomeUnknown}→confirmed、幂等、不覆盖 deterministicRejected。独立重跑：build / `go test -count=1 ./internal/connect/strategy/`（98s 全绿）/ `-race -count=1`（102s）/ vet / check-lines（0 errors，本任务文件无新增警告）。**独立 mutation**：删调用点 → 2 coordinator 测试 RED（`state=accepted_unconfirmed` 语义断言）；删 `barrierAcceptedUnconfirmed` 分支 → barrier 单测 RED；restore → GREEN。范围干净：4 个后端文件，零交接层改动（§4 合规）。 | ✅done |
| QS-1.3 | **Python 函数内赋值落 VM 全局槽 → 跨事件/跨函数状态污染（P0 语义，审计 P1-3）**：`compile.go:282-289` 对 `Version=="python"` 隐式全局。修法：在 `astCompiler` 赋值编译中复用既有 `localScopes/nextLocalSlot`（`compile.go:244-263`, `compile_expr.go:200`）分配局部槽，不在 pyCompiler 另造；`global` 语句未支持则编译期报错。风险：依赖全局污染的存量策略行为改变，覆盖度报告标记。spec §3.3。**Devin CLI 验收通过 2026-09-16**（commits 9940eda4 + ee47292d）：`resolveAssignTarget` + `isDeclaredGlobal`（GlobalDecls 判定谓词）+ `compileDecl` python 函数域分配（`localScopes[0]`）+ `compileCompoundAssign` 未声明名编译报错。过程修正 v2（施工方两处 [转交决策] 采信：黑名单非白名单、Option B）+ v3（决策方复审退回：for 循环域消亡回退，已独立复现）。独立重跑：build / test 8.4s / race -count=3 49.8s / vet / gofmt / check-lines 0 errors 全绿；**独立 mutation×4**：resolveVar 回退→多点 RED、GlobalDecls→GlobalSlots→2 RED（泄漏复现）、localScopes[0]→内层→2 RED（ValNone 复现）、删 forbidden 条目→S4e RED；restore 全绿。范围干净零交接层改动。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done |
| PY-SCOPE-KNOWN-1 | **Python 作用域已知限制（P3 文档化行为，QS-1.3 遗留）**：① `self.x` 与裸 `x` 同槽混同——pyCompiler 剥前缀映射同名，`self.x=1` 声明的 GlobalDecl 会被函数内裸 `x=1` 命中写全局（Python 语义应为局部）；② 读路径隐式注册保留——函数内未声明读仍产 blind spot + 隐式 GlobalSlots（ValNone），非 NameError；③ `for i in range(N)` 脱糖退出值 `i=N`（C 式 `i=0;i<N;i++`），Python 语义为 `N-1`——已 pin 为文档化行为。均为裁剪子集语义偏差，触发真实策略问题再立项。 **2026-09-18 Devin CLI 设计实查完成**：三偏差全部探针实证，①比原载更深——**双向两副面孔同一根因**（self.x 与裸 x 编译后同名 `"x"` 经同一 resolveVar/resolveAssignTarget 解析，字段无独立命名空间）：**①a 写侧**裸 `x=1`（含带注解）命中 `isDeclaredGlobal` 覆写 self.x 全局槽（探针 `GetGlobal("x")=1`）；**①b 读侧**同名局部存在（`for x in range(N)` 经 ExprDecl→localScopes[0]）时 `resolveVar` 局部优先使 `self.x` 读局部（探针 `r=3` 而字段 `GetGlobal("x")=10` 完好——遮蔽非覆写）。②实证 `r=ValNone`+`bc.Coverage.BlindSpots` 含 "implicit variable: never_declared"（非全静默，经 coverage 上报）。③已 pin（`TestQS13_ForLoopVarSurvivesLoop` r=3+注释）。**裁定文档化不修**：pin 测试补 ①a/①b/②（③已有）；文档分层 P3——细节一处（mql-compiler README 已知限制节）+pitfalls.md 发现路径指针+`PythonSubsetRules` 补三条防踩坑规则（Agent 是主要代码作者，文档化含生成侧约束）。判别设计要点：①a 判别量=`GetGlobal("x")` 值非 `r`（局部写形态下 self.x 读仍经局部优先得 1，r 不可判别）。派工单 `docs/audits/builder-handoff-py-scope-known-1.md`（S1-S5+mutation M1/M1b/M2+3 pin 测试）。**🟦open（施工完成，待独立复审）**：3 pin 测试（SelfBareWriteClobbers/SelfReadShadowed/ImplicitReadYieldsNone）+ 模块 README 已知限制节 + pitfalls 指针 + Agent 规则 3 条。mutation：M1 删 isDeclaredGlobal→T1a RED（x=0≠1，连带 19 存量测试属预期混同面）；M1b 删基域分配→T1b RED（r=10 遮蔽消失，连带 ForLoopVarSurvivesLoop 属 v3 修复面预期）；M2 隐式分支改拒→T2 RED（编译错）；均恢复 GREEN。零行为变更（compiler/runtime 零改动）。 **Devin CLI 独立复审验收通过 2026-09-18**（commit `373ca8d6`）：diff 与派工单逐格相符——3 pin 测试判别量正确（T1a `GetGlobal("x")==1` 非 r、T1b `r==3`+`x==10` 遮蔽非覆写、T2 `ValNone`+`BlindSpots` 通道断言先例 vm_array_oob）；README 已知限制节 ①a/①b/②/③ 全条带机制坐标+pin 名；pitfalls 单行指针不复制细节（P3）；`PythonSubsetRules` FORBIDDEN 尾追加 3 条（①b 读侧遮蔽已并入"or loop variable/shadows field reads"表述）。机检独立重跑：build ✓ / mql2go **745** / race×3 **2235** / vet / gofmt 两文件净 / check-lines **0 errors** / diff --check。**独立 mutation×3 复核**：M1 删 isDeclaredGlobal→**3 pin 全 RED**（机制比 spec 预测更深——连 self.x=10 也变局部→全局 x 恒 0 非预测 10，施工方如实报"x=0≠1"准确；判别力成立）；M1b 还原内层域→T1b RED `r=10 want 3` 精确命中+连带 `ForLoopVarSurvivesLoop`（实际连带比预估 3 个少——for-body `x=i` 走 `resolveAssignTarget` 恒基域不受影响，仅 ExprDecl 路径循环变量被击中）；M2 隐式分支改拒→T2 RED 编译错；均 restore→GREEN，工作区与 `373ca8d6` 逐字节一致。范围 6 文件零越界，compiler/runtime 零改动（零行为变更债如实）。 | ✅done |
| QS-1.2a | **`GetLastError/ResetLastError/SetUserError` 为 no-op（P1 语义，审计 P1-2）**：`vm_builtin_impls.go:73-74` + `vm_builtin_checkup.go:102`。修法：VM 加 `lastError int32`，三 builtin 真实实现（读后清零，不按事件清零），本轮唯一写入点 `SetUserError`。**交易失败路径保持 FAILCLOSED-1 fatal（D-009），QS-1.2b 不立项。** spec §3.4。**Devin CLI 验收通过 2026-09-16**（commit 65e2cccf）：`vm.lastError` 跨事件驻留字段 + `builtinGetLastError`（读后清零）/`builtinResetLastError`/`builtinSetUserError`（`65536+argI(args,0)`）+ `ERR_USER_ERROR_FIRST=65536` 常量 + 6 项行为测试（含跨事件驻留）。独立重跑：build / test 8.1s / race -count=3 51.4s / vet / check-lines 0 errors 全绿（gofmt 包内 drift 为 pre-existing，本任务文件零新增）。**独立 mutation×2**：SetUserError 不写 → 3 RED；GetLastError 不清零 → 1 RED；restore 全绿。范围干净：5 文件零交接层改动。遗留：`SetReturnError` 维持 no-op（MQL5 语义另行评估，边界内裁定）；SetUserError 不写 runtime 状态与 MQL4 一致。流程记录：施工自报首次缺 @hash（D-014 第二次偏差），补发后定位。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done |
| QS-1.7-INV | **open outcomeUnknown 永久锁仓可恢复性调研（P1，只查不改，审计 P1-8）**：确认 `strategyOrderClientID`（`live_dispatch.go:366`）是否随 Comment 下发 mtapi 并在 MT4/MT5 `OpenedOrders` 回显。可靠 → 立 QS-1.7 按 ClientID 精确单匹配 recovery；不可靠 → 维持 ④-② fail-closed + runbook。模糊匹配永久否决（D-009）。spec §3.5。**Devin CLI 验收通过 2026-09-16**（commit 05138758，findings `docs/audits/qs-1.7-inv-findings.md`）：**结论——ClientID 不可回显，QS-1.7 不立项，维持 ④-② fail-closed**。证据（决策方抽验属实）：①`submitOrder` 只设 ClientID 不设 Comment，全链路五跳无一跳复制 ClientID→Comment（mt4 adapter 连 `req.Comment` 都不下发，`mt4/orders.go:61-68`；mt5 下发恒空串）；②`pb.OrderSendRequest`（mt4.pb.go:7785+ / mt5）无 client_id/external_id 字段，唯一载体 comment 未接线且 MT4 未透传；③即使接线仍有两层未证——comment 长度截断（`start-<uuid>-<ts>-<type>` ~55+ 字符，仓内无记录）与 broker 回显改写/清空（仓内有 comment 不可信反例：非 UTF-8 sanitize、MT5 Comment/CloseComment 分列）；④magic/expertID 为 schedule 级非订单唯一，RequestId 只读。**裁定**：模糊匹配已否决 + 精确匹配链路不存在且回显保真度未证 → open outcomeUnknown 维持永久锁仓 fail-closed + 运维手册；若将来确有运营需求，路径为"ClientID→Comment 接线 + MT4 adapter 透传 + 定长编码 + mtapi 实测回显保真度"，按新债单独立项。提示词事实 d 一处偏差（mt4 同构透传 comment 不成立，施工方纠正属实）。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done |
| QS-2.2 | **goleak 集成（P1 工程纪律）**：`go.uber.org/goleak`（≥7 天版本）进 `strategy`/`mql2go` 包 `TestMain`；附 `TestWaitConfirmed_NoGoroutineLeak` 作为 QS-2.1 "watcher 泄漏"否决证据。spec §4.1。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done（Devin CLI 验收 2026-09-16，commit `174b8405`：goleak v1.3.0 钉版 + strategy/mql2go 两包 `TestMain` VerifyTestMain + `TestWaitConfirmed_NoGoroutineLeak` 双路径（ctx-cancel + terminal）。独立复审：①mutation 删 `defer close(stopWatcher)` → RED 精确抓 `WaitConfirmed.func1` watcher 泄漏→restore→GREEN；②豁免合法性独立验证——移除两条 `IgnoreTopFunction` → RED 仅列 `rjeczalik/notify` dispatch/internal（goroutine 1 创建的三方进程级常驻树，链：hdwallet→gotron-sdk→notify@v0.9.3）→restore→GREEN；`go test -count=1` 两包全绿 + `-race -count=3` strategy 301s 绿 + check-lines 0 errors。裁定：sleep(20ms) 仅影响覆盖率不影响正确性（ctx 先 cancel 时 watcher select 立即就绪）；豁免仅限两条第三方 top function，未来间接依赖新增常驻 goroutine 会 RED = 设计意图，批准。） |
| QS-2.4 | **VM 管线 data race 审计（P1）**：`-race -count=3` 覆盖 `tools/mql2go`/`internal/connect/strategy`/`strategy`；PositionCache mutex 覆盖、TradeBarrier cond 配对审计。spec §4.2。**Devin CLI 验收通过 2026-09-16**（commit 8e393cae，报告 `docs/audits/qs-2.4-race-audit.md`）：三树 race×3 决策方独立重跑全绿零 `DATA RACE`（strategy 子树 ~38s + mql2go 55.2s 亲跑；connect/strategy 301s 沿用 QS-2.2 验收实跑）；审计表逐项抽验属实——PositionCache 字段全锁内访问 + retained 快照发布后不原地写（put 恒拷贝换新指针）+ 5 调用方只读（抽验最复杂的 `enrichFromPositionCache` 属实）；TradeBarrier 10 Broadcast 全在 mu 内 + 2 Wait 正确范式 + NotifyOutcomeUnknown 幂等不饿死论证成立（outcomeUnknown ∈ isTerminal）。**发现 2 项立债**：F1→TEST-WAITSTATE-ACQUIRE-BCAST-1、F2→SNAPSHOT-SLICE-ALIAS-1（均 P3）。报告一处未列覆盖空洞：`tools/mql2go/cmd/parse_headers` 无测试文件（已补记）。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done |
| TEST-WAITSTATE-ACQUIRE-BCAST-1 | **`Acquire` 无 `cond.Broadcast` 致 `WaitState(submitting)` 前序等待退化 ctx 超时（P3 测试侧 footgun，QS-2.4-F1）**：`trade_barrier.go:162-176` Acquire 持锁迁移 idle→submitting 但不广播；WaitState 若在 Acquire 前进入 `cond.Wait`(:411)，后续广播（:200/:205 等）时 state 已越 submitting → 睡到 ctx 超时。现存 2 调用方（`mutation_coordinator_test.go:~1238`、`live_order_reentry_r4_redo_test.go:197`）忽略返回值带 2s ctx 行为正确，但 `r4_redo_test.go:195` 注释"会被唤醒"与实现不符（误导性）。修法建议：Acquire 迁移后加 Broadcast（无害多醒一次）或修正测试注释。**2026-09-18 Devin CLI 设计实查完成**：全扫 `b.state=` 9 迁移点——**仅 `Acquire:168` 缺 Broadcast**，其余全带（含 Release idle→:377），condvar 契约断裂不对称。实证调用点两站均破形态：`r4_redo_test.go:191-199` WaitState(submitting) 欲 submitting 窗口注入事件——实际睡到 ctx 超时/下迁移才醒，注入点漂移（事件迟到落 acceptedUnconfirmed 直配，非意图 cache 路径）+`:195` 注释误导；`mutation_coordinator_test.go:1237` 同型。**自审计判别难点**：返回值不可判别（无广播时 ctx 超时返回的恰是 submitting）——唯一判别量=延迟，测试须 select 限时断言（ctx 5s vs done 2s 边距）。裁定：Acquire 迁移后加 Broadcast（condvar 契约，waiter 重检）+r4 注释修正——修复后注释语义才成立；副作用=两测试提前醒至意图窗口（更忠实）+WaitState(confirmed) 类多醒一次无害。边界：等值语义不改、非 idle 早退不广播。依赖面：两调用点全量回归。派工单 `docs/audits/builder-handoff-test-waitstate-acquire-bcast-1.md`（D1-D4+S1-S4+mutation）。 **Devin CLI 独立复审验收通过 2026-09-18**（commit `53e886e9`）：`Acquire:169` 补 `b.cond.Broadcast()`（锁内，`defer unlock` :164）——9 迁移点广播对称性恢复；`r4_redo_test.go:195` 注释修正为"Acquire 迁移带 Broadcast"；新增 `TestTradeBarrier_AcquireBroadcastsSubmitting`（waiter goroutine + 50ms grace + ctx 5s vs done 2s select 限时——延迟判别设计如实落地）。机检独立重跑：build ✓ / strategy **417** / race×3 **1251** / vet / gofmt 本批净（6 存量文件未格式化与本债无关）/ check-lines **0 errors**（trade_barrier.go 448/300 🟡警告级未破阈）/ diff --check。**独立 mutation**：摘除 `Acquire` 广播→新测试 RED（`:410` 2s 超时命中，waiter 睡到 5s ctx——判别点精确命中）→restore→20 GREEN（含 R4 reentry+WaitConfirmed 家族全回归），工作区与 `53e886e9` 逐字节一致。豁免项按派工单记录：广播移锁外（defer unlock 后）语义等价不可判别，未计入。范围 3 文件零越界。 | ✅done |
| SNAPSHOT-SLICE-ALIAS-1 | **retained 快照 slice 别名依赖跨文件 immutable 约定（P3 健壮性，QS-2.4-F2）**：`position_cache.go:120` `merged := *snap` 浅拷贝（PositionsAuthoritative 时 :121-129 深拷分支跳过）+ `mthub/broker_types.go:143` `retained := *merged` 浅拷贝——retained/发布对象共享 `Positions`/`PendingOrders` 底层数组。当前安全：全部写入路径 `append(nil,...)` 新建 slice（broker_types.go:90-91/:112-113，position_cache.go:95-96/:124-125）。风险：任一方未来加原地写即静默 race。修法建议：注释钉住 immutable 约定或改深拷贝。 **2026-09-18 Devin CLI 设计实查完成**：别名图从原载 2 点扩到 **5 边**——①`Publish:143` retained 浅拷与全投递对象共享数组 ②`Subscribe:170`/`SubscribeAll:194` retained **原指针**回放（连浅拷都无）③`merge:94` carry-forward 别名（旧 retained :147 后无残留，不修）④`position_cache.go:120` 权威路径直存 snap 数组 ⑤`GetSnapshot/GetFresh×3` 原指针返回存储对象。生产端 3 站 fresh-build 验证、消费端全只读、测试无指针身份依赖——爆炸半径干净。**裁定边界不变量**（拒绝仅钉注释——约定横跨 5+ 文件正是本债形态）：broker/cache 永不外泄 retained 数组——Publish retained+两 replay+put 权威路径+4×Get 共四处深拷贝私有化；live 投递共享只读契约注释钉住（copy-per-delivery 不成比例已否决）。判别测试全确定性（删拷贝→隔离断言观察损坏），无时序。派工单 `docs/audits/builder-handoff-snapshot-slice-alias-1.md`（D1-D4+S1-S5+mutation×4）。 **Devin CLI 独立复审验收通过 2026-09-18**（commit `40148ede`）：边界不变量落地——`Publish` retained 深拷+`Subscribe`/`SubscribeAll` replay 私副本+`put` else 分支 snap 数组拷贝+`snapshotCopy` 应用于 4×Get；契约注释钉住共享只读投递面（struct doc+Subscribe×2）。机检独立重跑：build ✓ / mthub **333**+strategy **419** / race×3 **999+1257** / vet / gofmt 四文件净 / diff --check。**独立 mutation×4**：①删 retained cloneItems→RetainedIsolation RED（replay 见 Ticket:999）②Subscribe replay 还原原指针→ReplayIsolation RED ③删 put else 拷贝→PutIsolation RED ④GetSnapshot 还原原指针→GetCopyIsolation RED；每个变异仅命中对应测试（判别精确），均 restore→GREEN，树与 `40148ede` 逐字节一致。边界核实：E3 carry-forward 未动、live 投递共享语义未动、mergePositionSnapshot 未动、GetFresh* 判定逻辑未动。范围 4 文件零越界（测试文件 gofmt 对齐空白 churn 顺带）。 | ✅done |
| QS-2.5 | **coordinateMutation/dispatchLiveSignal 无 panic recovery（P1 稳定性）**：recover 必须同时 `barrier.NotifyOutcomeUnknown()` + `SetCircuitOpen(true)` + `RecordError`，不能只改返回值。spec §4.3。**Devin CLI 验收通过 2026-09-16**（commit 89353004）：`coordinateMutation` 命名返回 + 首注册 recover defer（unwind 最后执行，confirmUnsub 清理先行）+ `acquired`/`brokerCalled` 双标志 → `convergeMutationPanic`（mutation_helpers.go）三分支收敛（未 acquire→idle / 未发 RPC→deterministicRejected+Release / 已发 RPC→outcomeUnknown+circuit+RecordError）；`dispatchLiveSignal` recover + `State()` 门控（仅 submitting/acceptedUnconfirmed 才 NotifyOutcomeUnknown，idle 禁锁——决策方 spec 的关键安全约束已实现）。5 测试全绿（S3a-e，S3c 覆盖等价已注明）。独立复审：机检全绿（build/test 100.5s/race×3 303.3s/vet/gofmt/check-lines 0 errors，goleak 门禁仍绿）；**独立 mutation×3**：删 recover→panic 逃逸崩测试进程（生产爆炸半径复现）RED、删 NotifyOutcomeUnknown→barrier 停 submitting RED、删 State() 门控→idle 被误锁 outcome_unknown RED；restore 全绿。范围干净：4 文件零交接层改动。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done |
| QS-2.3 | **`vm.ctx == nil` 101 处散布检查（P2 洁净）**：`NewVM` 注入 noopContext，删检查。连带记债：`builtinOrderSend:15-17` 无 broker 时静默 -1 + nil error 非 fail-closed（另立→ORDERSEND-NILBROKER-FAILCLOSED-1）。spec §4.4。**Devin CLI 验收通过 2026-09-16**（commit 5ad339a9）：`noopContext`（vm_context.go，全 ~30 Context 方法 + `noopIndicatorSet` ~35 方法零值 + `emptyBars=BarsToSlice(nil)` 非 nil 空序列）+ `NewVM` 注入 + `SetContext(nil)` 归一化；**99 处守卫消除，17 处非等价站点保留**（决策方裁定见下）。独立复审：机检全绿（build/test 8.2s/race×3 48.2s/vet/gofmt/check-lines 0 errors，goleak 门禁仍绿）；**独立 mutation×2**：删 NewVM 注入→`builtinBid` nil panic RED（不变量必要性实证）、`emptyBars`→nil interface→`TestQS23_NoopBarsNonNilEmpty` RED；restore 全绿。**17 站点保留裁定（批准）**：测试可 `vm.ctx = nil` 显式构造（`vm_api_truth3_batch3_test.go:100/111/122`）故非死代码；且多站不可 noop 化——checkup IsConnected/IsDemo/IsTradeAllowed（nil→true vs noop→false）、datetime Day/DayOfWeek/Year/Month（nil→0 vs epoch 非零）、TimeGMT（nil→墙钟 vs 0）、ACCOUNT_LEVERAGE（nil→100）、iADX（静默零 vs mode dispatch+fatalError）、SymbolInfoTick/CopyBuffer/POSITION_PRICE_CURRENT/iTickVolume。语义等价第一原则下保留为正确处置（handoff"不等价不许改"条款的正当执行，披露经 commit message 送达）。范围干净：17 文件零交接层改动。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done |
| ORDERSEND-NILBROKER-FAILCLOSED-1 | **`builtinOrderSend` 无 broker 静默 `-1`+nil error 非 fail-closed（P2，QS-2.3 连带记债，spec §4.4 指定另立）**：`vm_builtin_trade.go` OrderSend 在 `vm.ctx.Broker()==nil` 时返回 `IntVal(-1), nil`——策略侧无法区分"无 broker"与"真实拒单"，且 nil error 使 VM 不进入错误路径。修法方向：无 broker 时应 fail-closed（明确 error 或专用错误码），需先审计依赖 -1 返回的存量策略/测试再动。**2026-09-18 Devin CLI 设计实查完成——扩族至 12 站点**：同构缺陷遍布 `vm_builtin_trade.go`（OrderSend/ctradeOrder）+ `vm_builtin_trade_signals.go`（OrderClose/CloseBy/Modify/Delete + CTrade×5 + CloseAll）——所有交易写函数 nil-broker 假值检查均在 `signalMode` 分支**之前**，signalMode 本不依赖 broker（服务端 OMS 派发）→ nil broker 双重谎言（信号静默丢弃+假拒单）。生产可达性：live 恒 `brokerImpl` 非 nil、回测恒 `SimBroker` 非 nil——nil broker 仅见裸 VM/自定义 ctx（防御纵深修复）。附加发现：OrderSend signalMode 下 `cmd>5`→ActionNone→静默 -1 子路径（validate 前置同批消灭）。裁定：signalMode 前移 + nil-broker→`fmt.Errorf("no broker configured")` fatal；**缺陷 B**（broker error 吞没→false ×11 站 + RetCode 未查 + CloseAll allOK）语义决策不同→另立 `TRADE-BUILTIN-ERR-SWALLOW-1`；查询 API→0/false 不同类不动。依赖面=2 断言（`vm_context_test.go:53/100` 占位断言翻转）。派工单 `docs/audits/builder-handoff-ordersend-nilbroker-failclosed-1.md`（S1-S6+mutation×3）。**Devin CLI 独立复审通过 2026-09-18**（commit `2739f100`，自审计修正版派工单 @0461ff34）：12 站点全部重排核实——`builtinOrderSend`（parse→validate→signalMode→nilcheck→broker，validate 前置消灭 signalMode cmd>5→-1 子路径）、`ctradeOrder`、signals.go 10 站（OrderClose/CloseBy/Modify/Delete+CTrade×5+CloseAll），nil-check 均落于 signalMode 后 broker 调用前，消息对齐 `"no broker in the VM"` 约定；缺陷 B（err→false 吞没）与查询 API（:128/:142/:448）边界正确保留。测试：`vm_nilbroker_failclosed_test.go` +131（12 站×(a)非signal nil-broker→err 含 "no broker"+无信号+假值 / (b)signalMode+nil-broker→err==nil+信号 emitted+Action 正确——排序修复直接证据）+ `vm_context_test.go` 2 占位断言翻转（OrderSend 显式传 volume=1 args 规避 invalid-volume 拦截，OrderClose err+false 断言），OrdersTotal→0 保留。**独立 mutation×3**：①OrderSend nil-check 回顶→(a)+QS23 断言 RED（-1/nil 复活）②OrderClose 恢复 `false,nil`→(a)+QS23 RED ③OrderDelete nil-check 移回 signalMode 前→(a) RED（静默 false 复活）；均 restore→709 GREEN，工作区与 commit **逐字节一致**。机检独立重跑：build ✓/mql2go 709/race×3 2127/vet/gofmt 净/check-lines 0 errors/diff --check。范围：4 文件零越界。 | ✅done（Devin CLI 验收 2026-09-18，commit 2739f100）|
| QS-3-BASELINE | **VM 性能基线测量（贯穿）**：benchmark（dispatch/builtin/Decimal/1000 tick）落 `docs/audits/vm-perf-baseline-2026-09.md` + 生产单事件耗时直方图 metric。v1 阶段 3 全部优化项已否决（D-009），只有数据触发阈值才立优化条目。spec §5。**Devin CLI 验收通过 2026-09-16**（commit b8ad1674）：B1-B4 benchmark（vm_bench_test.go，bare NewVM+noopContext）+ 报告落盘——dispatch 地板 ~116-128ns/事件、builtin 附加 ~310-480ns、decimal add/mul 2.6-2.7× int、div 7.2×、B4 MA 交叉 1000 tick 单事件均值 67.9µs/~310 allocs——**无单项 >30% 阈值**；3 条 live metric（`ant_strategy_vm_event_duration_seconds`/`_event_instructions`/`_fatal_total`，label=event）接 `vm_live_handlers.go` 5 个 vmHandle* 调用帧；stats 四级透传链 `vm.Ticks()/FatalError()`→`VMRunner.LastEventStats()`→`Runner.EventStats()` type-assert→`observeVMEvent`，非 VM 策略降级为仅记 duration；backtest 不接（自身成本即偏差）。独立复审：pin 测试×2 绿；**benchmark 数据决策方独立重跑全部复现**（11 项偏差 <10% 噪声、alloc 计数一致）；**独立 mutation**：`Ticks()` 返常数 0 → `TestQS3Baseline_EventStatsChain` RED（ticks=0 断言）→ restore → GREEN；gofmt/vet/build/check-lines 0 errors；race×3 mql2go 47.4s+runner 1.1s 绿，connect/strategy race×3 补跑收尾。范围干净：8 文件、零优化改动、零交接层改动。生产 p99 判据待 metric 上线观测。【最终决策：Devin CLI（[角色:决策终] 激活）】 | ✅done |
| VM-ARRAY-OOB-FAILCLOSED-1 | **用户数组越界读写静默（P2，VM-RUNTIME-FAILCLOSED-2 独立复审分立）**：`vm_execute.go` `executePushArray`（:298-309）索引越界/非数组/槽位越界 → 静默返回 `NoneVal`（假值入栈）；`executeStoreArray`（:311-323）同条件 → 静默丢弃写入。真实 MQL `array out of range` 是致命运行时错误（EA 终止）。编译器对 `arr[i]` 直接 emit OP_PUSH_ARRAY/OP_STORE_ARRAY（`compile_expr.go:144,156`），动态索引运行时可达——与 FAILCLOSED-2 修复的 slot 越界同类（静默 OOB → 假值/丢失），但 spec 证据链只列 slot 分支故未含。修法方向：两处越界 → `setStackError`（同 `OP_PUSH_VAR` 模式），注意区分"局部数组"（compiler 已记 blind spot 降级）与全局数组路径；补 OOB 读/写行为测试 + mutation RED→GREEN。**2026-09-17 Devin CLI 设计实查补记（B 面更深缺陷）**：`compileSubscript`（`compile_expr.go:140-156`）对非全局变量记 blind spot 后仍 emit `OP_*_ARRAY` 且 slot 为**局部索引**（`resolveVar` `compile.go:257-267`），运行时 `globals[localIdx]` 被误用——该槽恰为数组时静默读写一个**无关全局数组**（真实正确性破坏，超出 blind spot 降级语义）。裁定：局部负编码 `-(slot)-1` → 运行时 `ins.A<0` → `setStackError`，编译契约/blind spot 不变。派工单 `docs/audits/builder-handoff-vm-array-oob-failclosed-1.md`。**Devin CLI 独立复审通过 2026-09-18**（commit `bdb3733f`，修订派工单 @d7f74310）：A 面——executePush/StoreArray 五分支（负槽/槽越界/非数组/负索引/索引越界）全 `setStackError` 带精确消息，合法访问不阻断；OP_PUSH_ARRAY dispatch 加 fatalError 守卫（同 OP_CALL_BUILTIN 形态，防 NoneVal 假值入栈）。B 面——compileSubscript 读/写两路局部槽负编码 `-int32(slot)-1`（uint16 先转 int32 防溢出），blind spot 契约保留。测试：手工 VM/bytecode 6 用例覆盖全分支 + 源码可达 3 路径（StringSplit 产数组入全局→OOB/负索引/合法读；标量下标→not-an-array；局部标量下标→local array error+blind spot 断言+同索引全局数组未污染断言）。**独立 mutation×4 重跑**：①索引分支静默化→TestIndexOOBDirect+TestGlobalArrayFromBuiltinOOB×3 RED；②store 非数组静默→TestNonArraySlotDirect RED；③读+写两路恢复 int32(slot)→TestLocalArrayAccessFails RED（局部下标静默读 globals[0] 数组，OnBar 无错——误读实证）；④拆 dispatch 守卫→TestPushArrayDispatchGuard RED（NoneVal 入栈 stack=1）；均 restore→611 测试 GREEN+工作区与 commit 完全一致。机检独立复测：build ✓/mql2go 611/race×3 1833/vet/gofmt/check-lines 0 errors/diff --check。范围：3 文件零越界（vm_execute.go+compile_expr.go+新测试 339 行），全局数组声明修复另立 VM-GLOBAL-ARRAY-DECL-1（S4-1/2/6 迁移）。 | ✅done（Devin CLI 验收 2026-09-18，commit bdb3733f）|
| VM-FUNC-FATAL-DELAY-1 | **用户函数内 fault 后非-builtin 指令继续执行至 RETURN（P3，VM-RUNTIME-FAILCLOSED-2 独立复审分立）**：`executeCallUser` 内层循环（`vm_execute.go:347-383`）只查 ctx/ticks/MaxStackDepth/`execute` 返回 error，**不逐指令检查 `vm.fatalError`**——函数内 `x=10/0` 后，后续 `OP_STORE_GLOBAL` 等纯栈/槽位指令仍执行（写入泄漏），直到 OP_RETURN 回 runLoop 顶部检查才传播。缓解面：broker 副作用被 `OP_CALL_BUILTIN` 的 popN 后 `fatalError` 检查（:123-125）门控，事件级 fail-closed 仍成立（Engine.Run 收到 error）。属既有架构属性（所有 setStackError 路径共享），非本批回归。修法方向：内层循环顶部加 `vm.fatalError` 检查（对称 runLoop:14）+ 函数内 fault 后 global 写入不生效的行为测试。**2026-09-18 Devin CLI 设计实查完成**：坐标复核无漂移——全仓仅两处 fetch-execute 循环（runLoop:38 已查、executeCallUser:385 缺）。新增实证：`:356` popN 下溢也经 setStackError（vm_helpers.go:56）→**带 fatal 入口函数体照执行**（嵌套调用可达，比原载更糟）；调用方续跑泄漏——inner() fault 净返回 nil 后调用方 post-call 指令照执行。裁定：内层循环顶部加 fatalError 检查（镜像 runLoop:14 序位 fatal→ctx→ticks→depth→fetch→execute），错误出口恢复 locals/callDepth 同其他出口惯例——一处改动同覆三泄漏路径。语义边界：OP_RETURN 遇 pending fatal→error 优先（fault 不得被 return 吞没）；OP_HALT 函数内=return 不动；vm.pc 错误出口不恢复。依赖面零（无"函数内 fault 后续执行"断言；getGlobalInt 断言通道先例 vm_timeseries_failclosed_redo_test.go:438；10/0 无常量折叠实证）。派工单 `docs/audits/builder-handoff-vm-func-fatal-delay-1.md`（D1-D3+S1-S3+mutation×3）。**Devin CLI 独立复审通过 2026-09-18**（施工 `de6f672c`+返修R1 `6ef18536`）：`executeCallUser:375` 循环顶 fatalError 检查与 D1 逐格相符（fatal→locals/callDepth 恢复→`"VM fatal: %s"`，序位镜像 runLoop:14）——一处覆三泄漏路径（函数内 fault 后续指令/嵌套入口 popN fatal/调用方续跑）。测试 `vm_func_fatal_delay_test.go` +125：(a) f() 内 fault→g_after==0+err 含 "VM fatal"、(b) inner() fault→outer() 不续跑 g2==0、(c) 手工字节码 popN 入口 fatal→err+`len(vm.stack)==0`。**施工方 spec 缺陷报告经实证成立**：mutation#2 break 形态级联净返回，三层断言全等价（仅上报层不同）——诚实上报非伪造 RED，从验收项剔除。**复审 R1 抓测试判别弱点已返修**：(c) 原仅断言 err 文本（M1 下仍 PASS——body 照执行净返回），补 `len(vm.stack)==0` 判别断言后变异下 `stack len=1` RED。**独立 mutation×2**：①摘检查→(a)+(b)+(c) 全 RED（g_after=42/g2=9/stack len=1 泄漏复活）②entry-only 检查→(a)+(b) RED（逐迭代放置必要性）；均 restore→733 GREEN，树与 `6ef18536` 逐字节一致。机检独立重跑：build ✓/mql2go 733/race×3 2199/vet/gofmt 净/check-lines 0 errors/diff --check。范围：2 文件零越界。 | ✅done（Devin CLI 验收 2026-09-18，commit de6f672c+6ef18536）|
| VM-ENUM-NUMBERING-1 | **prop-switch 函数族的"自造紧凑编号 vs 真枚举"跨文件偏移（P2，2026-09-17 Devin CLI 批次2c 设计实查发现）**：已证实 `AccountInfoInteger`（代码 case 32/35/36 vs 真 ENUM_ACCOUNT_INFO_INTEGER 2/5/6）、`AccountInfoString`（case 顺序 vs 真 NAME=0/SERVER=1/CURRENCY=2/COMPANY=3 错位）、`constants.go`（ACCOUNT_MARGIN_INITIAL=7/MAINTENANCE=8/SO_CALL=9/SO_SO=10 与真值 9/10/7/8 互换）、ACCOUNT_* 命名常量大量缺失。疑似同类：`SymbolInfoDouble`/`SymbolInfoInteger`（`vm_builtin_account.go:35-93` 紧凑 0-7 编号 vs 真 ENUM_SYMBOL_INFO_* 大枚举）、`MarketInfo`（`:110-147` MODE_* 与 MQL4 编号亦不符）。修法方向：一次性全枚举对齐审计（勿分散到各批）——逐个 prop-switch 函数对照真枚举表核对编号+补命名常量+假分支处置。批次2c 已修 AccountInfo* 一处。**2026-09-17 Devin CLI 设计实查完成（对照官方 MQL5/MQL4 文档枚举序）——比"疑似"更严重，是活的静默错标**：`constants.go` 值与 switch case **互相不自洽**——`SymbolInfoDouble(s,SYMBOL_POINT)`（常量=2）命中 case2 返回 `VolumeMax`、`(s,SYMBOL_BID)`（常量=0）返回 `Point`、`(s,SYMBOL_SWAP_LONG)`（常量=9）落 default 返 0、`SymbolInfoInteger(s,SYMBOL_DIGITS)`（常量=12）落 default 返 0、`(s,SYMBOL_SELECT)`（常量=0）返回 `Digits`；`SymbolInfoString` 无 switch 恒返 `info.Name`（真 STRING 枚举无 NAME、`SymbolInfo` 无任何字符串字段）→ **整 API 重分类裁定**；`MarketInfo` 常量↔switch 自洽但整组偏离真 MQL4 编号（TICKVALUE=17 vs 真 16 等 15 处错值 + 4 个非真模式常量 MODE_SWAPTYPE/SWAPMODE/PROFITCALCMODE/MARGINCALCMODE 与真值撞号）。数据源裁定：23 prop 实接（info 12 字段 + ctx.Bid/Ask/ServerTime/IsTradeAllowed 当前品种守卫 + MARGININIT/MARGINREQUIRED 按 FreeMarginCheck 公式），venue 事实 17 项（forex LAST/VOLUME_REAL=0、EXIST/SELECT/VISIBLE=1、TRADE_MODE_FULL=4、EXECUTION_MARKET=2、ORDER_MODE=63 等），无源分支 fail-closed error，非真/无源常量删名编译期拒。派工单 `docs/audits/builder-handoff-vm-enum-numbering-1.md`（官方枚举全表 + 逐 prop 处置表 + S1-S6 + mutation×4）。**Devin CLI 独立复审通过 2026-09-18**（施工 @477e8273 + 返修R1 @bfb42ea3）：常量/switch 值与 §1a-1d 官方枚举序**逐值相符**（DOUBLE 19 项/INTEGER 21 项/MODE_* 26 项，含 SUBSCRIPTION_DELAY=0 新序位移裁定）；§3a-3d 处置表逐格落地——23 prop 实接带源（info 12 字段+ctx.Bid/Ask/ServerTime/IsTradeAllowed 当前品种+非零守卫+MARGININIT/REQUIRED FreeMarginCheck 公式+leverage 守卫）、venue 事实 17 项、无源分支 `fmt.Errorf`、Broker nil/SymbolInfo err 静默 0 顺手修；SymbolInfoString 整 API 重分类（注册/implemented/wiring/registry 四路摘除干净）。**复审 R1 发现 1 处 spec 自身缺陷已返修**：`SYMBOL_TIME_MSC` unix_ms（~1.7e12）超 `Value.Int` int32 上限→截断假值（设计责任在 Devin CLI，非施工方偏离）——按修订记录转不可实现类：删常量（名不解析→隐式 0→prop0→error）+删 case16（裸 IntVal(16)→default error），TIME=15 秒级 int32 保留。**独立 mutation×5 重跑**：①SYMBOL_POINT 回 2→S6b RED（unsupported prop 2）②注释 SymbolInfoString 条目→S6a RED（编译静默接受）③摘 MODE_MARGININIT case28→S6b RED ④EXIST 改返 0→S6c RED（got 0 want 1）⑤恢复 case16 截断返回→裸 16 断言 RED；均 restore→680 GREEN+树与 bfb42ea3 逐字节一致。机检独立复测：build ✓/mql2go 680/race×3 2040/vet/gofmt/check-lines 0 errors/diff --check。范围：7+3 文件零越界。 | ✅done（Devin CLI 验收 2026-09-18，commit 477e8273+bfb42ea3）|
| VM-GLOBAL-ARRAY-DECL-1 | **全局数组端到端从未工作：声明丢弃+下标赋值错译（P2，2026-09-17 Devin CLI 对 VM-ARRAY-OOB-FAILCLOSED-1 施工方阻断报告的独立实证）**：**发现 1**——`collectGlobalVar`（`compile_interp.go:174-223`）只处理 `init_declarator`/`declarator`，无 `array_declarator` 分支 → `double g_arr[3]` 全局声明整体不进 `ir.Globals`（探针实证：`ir.Globals` 无该条；槽位靠 astCompiler 首次引用隐式分配为标量，`vm.go:243-250` 的 `IsArray→ValArray` 初始化路径永不触发）。**发现 2**——`compileAssignment`（`compile_interp_expr.go:286`）先查 `findIdent(lhs)`（subscript 内的数组名总能命中）后查 `subscript_expression` → `arr[i]=v` 恒退化为 `ExprAssignment` 整槽标量写（探针实证：`g_arr[0]=5` 发射 `PUSH_CONST 5; STORE_GLOBAL slot1`，下标丢弃、无 OP_STORE_ARRAY），line 304 的 subscript 分支不可达。后果：声明静默丢弃 + 写静默错标整槽（读路径尚可达 PUSH_ARRAY 并于非数组槽 fail-closed）。修法方向（设计时定）：补 `array_declarator` 收集（IsArray+ArraySize→GlobalDecls→vm.go ValArray 初始化）+ compileAssignment 检查顺序对调（subscript 优先）+ 验证 ExprSubscript 赋值路径发射 OP_STORE_ARRAY；若选择不支持全局数组则改编译期拒绝（fail-closed 同局部数组先例），不得继续静默。**验收含**：原 VM-ARRAY-OOB 派工单 S4-1/2/6 迁移用例（`int g[2]` 声明→真实数组读写→OOB error）；波及面审计（扫描 testdata/e2e 策略中全局数组声明用法，评估存量依赖）。**2026-09-18 Devin CLI 设计实查完成——修正发现 1 为三处叠加缺口并扩族至 4 错标点**：①裸 `array_declarator` 无 case（`int g[3]`）；②`init_declarator` 嵌套 array_declarator 时 findIdent 只看直接子节点→name==""→丢弃（`int g[2]={1,2}`）；③`findArraySize` 找 `subscript_expression` 但声明处实为 `array_declarator` 节点→死分支。新增错标点：`g[0]+=v`→ExprCompoundAssign 整槽复合写、`g[0]++`→ExprUpdate{Name:"g[0]"} 创建幻影全局、`void f(int a[])`→findIdent 丢参→函数内写幻影全局。**波及面审计=零**（testdata 4 策略+Venus.mq4 812 行+全量内嵌源均无全局数组用法）。裁定：实现主路径（VM 后端管线全就绪），多维/initializer/参数数组/元素级复合与更新→编译期显式拒。派工单 `docs/audits/builder-handoff-vm-global-array-decl-1.md`（parse 形态表+S1-S6+mutation×4）。**Devin CLI 独立复审通过 2026-09-18**（commit `da902af6`，自审计修正版派工单 @37358629）：`compile_interp.go` +93——`parseArrayDeclarator`（直接 identifier/number_literal 提取+嵌套多维检测），`collectGlobalVar` 补 `array_declarator` 分支（多维→拒、size≤0→拒、append IsArray+ArraySize）；`init_declarator` 内含 array_declarator→无条件拒（`{1,2}` 与 `=5` 同路）；`collectFuncParams` 参数含 array_declarator→拒（名取 decl 子节点/pd 原文）。`compile_interp_expr.go` +36/−9——subscript 判定前移至 findIdent 之前（`=`→compileSubscript+`IsAssign=true`+Args=[rhs]，`!=`→显式拒，nil→拒；死分支删除）；`compileUpdate` 下标子节点→显式拒（堵幻影全局 "g[0]"）。`IsAssign` 补充经核实为必要正确性修正：`isStackNeutral`（compile_loops.go:240-243）对 ExprSubscript 仅在 IsAssign 时判栈中立，否则逗号/多声明 Seq 场景多发 OP_POP 栈下溢——与 ExprField 写路径惯例一致。测试 `vm_global_array_test.go` +236：读写回路 x==12、读 OOB `index 5 out of range (len=2)`、写 OOB `index 9`、负索引、double/string/bool 零值默认、先引用后声明、static、编译期拒×6（多维/{}/=5/参数/+=/++）、局部仍拒、标量不误伤。**独立 mutation×4**：①短路 array_declarator 收集→合法用例全挂+多维静默放行 RED；②恢复 findIdent 优先→g[9]=1 静默写、d 数组被整槽复合写摧毁（kind=2 len=0）、compound 静默放行 RED；③摘 compileUpdate 守卫→g[0]++ 静默编译（幻影全局复活）RED；④摘参数守卫→f(int a[]) 静默放行 RED；均 restore→696 GREEN，工作区与 commit **逐字节一致**。机检独立重跑：build ✓/mql2go 696/race×3 2088/vet/gofmt 净/check-lines 0 errors/diff --check。范围：3 文件零越界，vm.go 初始化路径未动（S5 保留）。 | ✅done（Devin CLI 验收 2026-09-18，commit da902af6）|
| LIVE-ACCOUNT-FIELDS-1 | **live runner `Account()` 未填充 Leverage/Currency/Company/Mode（P3 管线缺口，2026-09-17 Devin CLI 批次2c 设计实查发现）**：`strategy/runner/broker.go:203-219` harness 模式 `Account()` 只填 Balance/Equity/Margin/FreeMargin/Login/IsDemo/IsConnected/IsTradeAllowed——Leverage→0、Currency/Company/Mode→零值；`sdk.AccountInfo` 字段存在但无 live 填充通道（mt_accounts/AccountSummary 是潜在权威源）。批次2c 如实暴露为 0/error（诚实），本债修 live 管线补全。修法方向：live context `live*` 字段补 leverage/currency/company/mode 通道（proto/session 契约扩展 + mtapi AccountSummary 数据源接入）。**2026-09-19 Devin CLI 设计实查完成——原载"proto/session 契约扩展"半过时，按字段分治**：①**Company**：proto `company=27` 已存在且 `live_context.go:273` 已由 `brokerCompanyLookup` 填充——只差 runner ctx 字段+setter+`Account()` 接线，零 proto 变更；②**Leverage/Currency**：`mt_accounts.leverage/currency`（`account_lifecycle.go:72` 从 mtapi AccountSummary 同步）为权威镜像→新单行三列 `accountIdentityLookup`（REUSE lookup 惯例，一次查询替代 3 个单列查询）→新 proto `leverage=31`/`currency=32`（号已核未占）→`SetAccountIdentity`+`liveLeverage/liveCurrency`→`Account()`；③**Mode(hedging/netting)**：proto `mode=13` 是执行模式 live|paper **禁直接映射**；MT4 平台语义恒 hedging（`mt_type` 同查询带出，非伪造）；MT5 mtapi `AccountSummary.Method`(AccMethod) 存在但 adapter 未映射（`mdtick` 三结构全无该字段）→新 proto `account_mode=33`，MT5 置 `""` 保持 VM fail-closed，adapter 映射另债。**消费端实况**：`AccountLeverage()`/`AccountInfoInteger(ACCOUNT_LEVERAGE)` 静默 0（本债主要失真），Currency/Company/Mode 消费者已 fail-closed error；`AccountFreeMarginCheck`/`MarketInfo(MARGININIT/REQUIRED)` leverage-zero 已 error。**live fail-closed**：`injectAccountTruth` 增完备性检查——live 且 `leverage<=0||currency==""`→error（真账户无合法零值）；paper 容忍；`account_mode`/`company` "" 容忍（VM 层已 fail-closed）。**边界外登记**：`AccountInfoDouble(ACCOUNT_MARGIN_LEVEL=6)` 返 Equity/Margin 比率疑与真 MQL5 百分比 100× 偏差（另债）；`mt_accounts.account_method`="master"|"investor" 是 investor 标志非 margin mode（禁误用）。接线点位=现有 SetLogin/SetAccountStatus 同点（dispatch:109/session:121/handlers:34），tick/trade/timer 不动。派工单 `docs/audits/builder-handoff-live-account-fields-1.md`（S1-S5+mutation×4）。**🟦open（施工完成，待独立复审）**：S1 proto 31-33（leverage/currency/account_mode，make proto 双端生成物同提交）；S2 accountIdentityLookup 单查询注入+live 完备性 fail-closed（leverage≤0/空 currency/lookup err；paper 容忍）+accountModeForMTType（MT4=hedging 平台语义、MT5=""）；S3 handlers_strategy 三列 SQL；S4 runner ctx 四字段+SetAccountIdentity+Account() 接线+sdkAccountMode 白名单+三点位（init+OnBar 同 SetLogin/SetAccountStatus 点）；S5 T1 harness VM 执行断言/T2 bogus fail-closed/T3 注入+MT5 未知/T4 三向 fail-closed/T5 paper 容忍；既有 live 测试补 identity stub×2。mutation：M1 摘 Account 四字段→T1 RED（VM fatal AccountCurrency）；M3 删完备性块→T4 RED；M4 MT4→""→T3 RED；**M2 sdkAccountMode 直转→T2 PASS**（VM prop-7 default 第二层独立 fail-closed 兜底，双层冗余使该变异在 T2 VM 断言通道不可观测，如实记录待裁定）。**Devin CLI 独立复审通过 2026-09-19**（施工 45767c9f）：diff 与派工单逐格相符——proto `leverage=31`/`currency=32`/`account_mode=33`（注释明载与 mode=13 执行模式区分）+`accountIdentityLookup` 单行三列 `COALESCE` SQL+live 完备性 fail-closed（leverage<=0/currency==""/lookup error）+paper 容忍+`accountModeForMTType` MT4→hedging/MT5→""+`SetAccountIdentity` 三站接线（dispatch/session Init 前+vmHandleBar 同 SetLogin 点）+`sdkAccountMode` 白名单（""/garbage→""）。测试：T1 VM 执行断言 5 builtin 读回真值（AccountLeverage=200/HEDGE_ALLOWED=1/MARGIN_MODE=2/Currency/Company 经 GetGlobal）+T2 bogus→VM fatal+T3 注入+MT5 unknown+T4 三向 fail-closed+T5 paper 容忍；2 存量 live 测试补 identity stub。**机检独立重跑**：build✓/vet✓/gofmt（审计方规范化测试注释对齐）/connect/strategy 426+mql2go+runner 829/race 1255/check-lines 0 errors/diff --check。**独立 mutation**：M1 摘 Account 四字段→T1 RED（VM fatal AccountCurrency error）✓；M3 删完备性块→T4 两子用例 RED（err=nil fabricated zeros 复现）✓；M4 accountModeForMTType→""→T3 mt4 RED（AccountMode="" want hedging）✓；**M2 sdkAccountMode 直转→双测 PASS——施工声明"双层冗余不可观测"独立核实准确**（VM prop-7 default 第二层 fail-closed 兜底，白名单为边界防御纵深非行为必需，保留合理）。范围 17 文件零越界，生成 pb.go/pb.ts 经 make proto 真生成。 | ✅done（Devin CLI 验收 2026-09-19，commit 45767c9f） |
| TRADE-BUILTIN-ERR-SWALLOW-1 | **交易写 builtin broker error 吞没→`BoolVal(false)`（P3，2026-09-18 Devin CLI 对 ORDERSEND-NILBROKER-FAILCLOSED-1 设计实查分立）**：`vm_builtin_trade_signals.go` 11 站（OrderClose/CloseBy/Modify/Delete + CTrade 对应 5 站 + CloseAll）+ `vm_builtin_trade.go` `ctradeOrder:628-631`——`err!=nil → BoolVal(false),nil`，**infra 失败**（`brokerImpl` "no executor configured" `broker.go:37-94`）伪装成业务拒单，策略侧无法区分且 VM 不进错误路径。同族附随：`builtinOrderSend:71-77` 未检查 `result.RetCode`（对 brokerImpl 无害——其拒单恒 RetRejected+err 配对；其他 sdk.Broker 实现 RetCode-only 拒单会返假票号）；`builtinCloseAll:243-249` 逐仓 err→allOK=false 聚合吞没。处置需独立设计决策：真 MQL 拒单语义（false+`_LastError` 通道，vm.go:40 已有 lastError 基建）vs fatal——与 nil-broker（环境缺失→fatal，本债）不同类。修法方向候选：infra error→fatal 或 lastError 通道+false，先审计 RetCode/error 语义边界再定。**2026-09-18 Devin CLI 设计实查完成——通道分裂裁定**：根因=双通道混用（SimBroker `:143/:214/:231/:234/:237` 业务拒单配对 RetCode+err；`:351/:363/:375` 却是 `RetRejected`+**nil err**→当前 builtin 返 `BoolVal(true)` 假成功）。裁定 **channel-split**：`error`=仅 infra→fatal；`RetCode`≠done→false/-1+`vm.lastError` 映射码（MQL 忠实，`GetLastError` 基建已存但无 builtin 写入——同族假数据洞纳入本单）；RetCode `""`→fail-closed error（契约违反三态分裂，自审计实证：现有 mock `{Ticket:X},nil` 空 RetCode 若当拒单会全挂）。`_LastError` 映射用 `interp/constants.go:377-404` 表内真实 MQL4 码（INVALID_PRICE=13 非 4107），补 `ERR_TOO_MANY_ORDERS=148`+`ERR_TRADE_NOT_ALLOWED=4109`。附随：engine.go dispatchSignal 补 RetCode 日志（RetCode-only 拒单当前 stderr 静默丢）；SimBroker 5 处 err 配对拒单→RetCode-only；语义变更明示：回测保证金拒单 fatal→-1+lastError=134（MQL 忠实）。边界：brokerImpl err 路径 RetRejected 噪声不动（err→fatal 先行忽略）、executor 无 RetCode 通道→fatal 保守响、查询 API 不动。依赖面：零 err!=nil 业务断言 + mock 卫生 6 处补 RetDone。派工单 `docs/audits/builder-handoff-trade-builtin-err-swallow-1.md`（D1-D5+S1-S6+mutation×4）。**Devin CLI 独立复审通过 2026-09-18**（commit `6eae8160`）：13 调用站点三态分裂全部核实（12 函数，OrderModify 双调用串联）——err→`fmt.Errorf("... broker error: %w")` fatal、RetCode `""`→`"empty RetCode"` fatal、done/partial→成功、其他→false/-1+`vm.lastError=mqlErrFromRetCode`（表内真实码 134/131/136/13/148/4109/146）。SimBroker 5 处拒单搬 RetCode-only（理由留注释，fmt import 摘除干净）；engine.go dispatchSignal 5 路补 RetCode 拒单日志（修复 RetCode-only 拒单 stderr 静默丢盲区）；sdk/broker.go 契约注释钉死双通道语义；constants.go 补 ERR_TOO_MANY_ORDERS=148+ERR_TRADE_NOT_ALLOWED=4109（真实 MQL4 码，段尾注释）。测试 `vm_trade_errchannel_test.go` +274：(a)-(g) 全项+GetLastError 读后清（首读 146/二读 0）+10 站 done 回归表；mock 卫生 6 处补 RetDone。**独立 mutation×4**：①OrderDelete 摘 RetCode switch→(a)假成功复活+读后清 RED；②OrderClose 恢复 err→false,nil→(c)+(d) RED（infra 吞没+空码当成功）；③OrderSend 删 lastError 写→(b) RED（lastError=0 want 134）；④OrderClose 删 `""` 分支→(d) RED（空码被当拒单 err==nil）；均 restore→912 GREEN，工作区与 commit **逐字节一致**。机检独立重跑：build ✓/mql2go+backtest 912/race×3 2736/vet/gofmt 净（compile_py* 系存量与本次无关）/check-lines 0 errors/diff --check。范围：9 文件零越界；语义变更如期——回测保证金拒单 fatal→-1+lastError=134（MQL 忠实）。 | ✅done（Devin CLI 验收 2026-09-18，commit 6eae8160）|
| MT5-ACCMETHOD-ADAPTER-1 | **mtapi MT5 AccountSummary.Method(AccMethod) 存在但 adapter 未映射→MT5 账户 margin mode 无权威源（P3，2026-09-19 LIVE-ACCOUNT-FIELDS-1 设计实查分立）**：`reference/grpc/mt5.proto` AccountSummary f10 `Method`（AccMethod: Netting=1/Hedging=2）是 MT5 账户 hedging/netting 唯一权威源，但 `mdtick.BrokerInfo`/`MTAccountInfo`/`ProfitUpdate` 三结构全无该字段（`mt5/connection.go:345-358` 只映射 9 字段，`connection_account.go:44-56` 同缺）；`mt_accounts.account_method` 存 "master"/"investor" investor 标志非 margin mode（`mdgateway/runner_gateway.go:175-178` 写入）。后果：LIVE-ACCOUNT-FIELDS-1 的 `sdk.AccountInfo.Mode` 对 MT5 账户恒 `""`→VM `AccountInfoInteger(ACCOUNT_MARGIN_MODE/HEDGE_ALLOWED)` 保持 fail-closed error（诚实但不可用）。修法方向：mdtick 结构加 MarginMode 字段+mt5 adapter `s.GetMethod()` 映射（MT4 proto AccountSummary 无 Method 字段——MT4 恒 hedging 平台语义已在 runner 侧处理）+流向 snapshot/lctx。**2026-09-19 Devin CLI 设计实查完成**：写入通道实证——`MTAccountInfo` 唯一消费链=绑定时 `verifyMTCredentials`→`AccountInfoUpdate`→`UpdateAccountInfoTx`（leverage/currency 同路）；connect 路径 `updateAccountStatusOnConnect` 只写 status 不写 summary 字段——margin_mode 随绑定通道即全栈一致（静态配置绑定即足）。落地链：mt5 adapter `s.GetMethod()`→`accMethodToString`（Netting/Hedging/Default→""）+mt4 adapter 恒 `"hedging"`（平台语义非 lookup）→migration 280 `margin_mode VARCHAR(16)`（NULL=unknown；**禁复用** `account_method`=investor 标志列）→`AccountInfoUpdate`/`UpdateAccountInfoTx` `NULLIF` 写入→`AccountIdentity.MarginMode`+lookup SQL 第四列→`lctx.AccountMode` 直读（`accountModeForMTType` 降级为 MT4 legacy 行兜底）。MT5 `Default(0)`/NULL=""=合法 broker 响应非缺失——live 完备性不加 mode 要求。边界：`BrokerInfo`/`ProfitUpdate`/PositionSnapshot 不接（DB 路径已够）；admin_repo 显式列表不加。依赖 LIVE-ACCOUNT-FIELDS-1 验收后开工。派工单 `docs/audits/builder-handoff-mt5-accmethod-adapter-1.md`（S1-S4+mutation×3）。 **🟦open（施工完成，待独立复审）**：S1 mdtick.MTAccountInfo+MarginMode/mt5 accMethodToString（Netting/Hedging→值、Default/未知→""）/mt4 恒 "hedging" 平台语义；S2 migration 280 up/down（margin_mode VARCHAR(16) NULL=unknown+COMMENT 禁混淆 account_method）+model *string+AccountInfoUpdate.MarginMode+两 UPDATE NULLIF($N,'')+crud 入参+sqlc 生成物等价手编（重生成仍被 138/142 历史 migration 阻断，同 DATA-TRUTH-3）；S3 AccountIdentity+MarginMode+lookup SQL 第四列 COALESCE+lctx.AccountMode DB 直读（accountModeForMTType 降级 MT4 legacy 兜底）；S4 T1 helper 四态（含 99 未知）✓/T2 SQL 审查代（无 DB 环境，NULLIF+参数序位核对）/T3 四子优先级（DB 值>MT4 兜底>MT5 未知）✓/T4 netting VM 端到端 ✓。mutation：M1 helper 恒 ""→T1×3 RED；M2 摘直读→T3 netting RED（mt4 兜底子用例不受影响）；M3 mt4→"" 无直接 RED 向量（mt4 adapter 无直测、T3 走 stub——如实记录）。**Devin CLI 独立复审通过 2026-09-19**（施工 4578cb4e）：diff 与 S1-S4 逐格相符——`MTAccountInfo.MarginMode` 字段+mt5 `accMethodToString`（Netting/Hedging→值、Default/99→""）+mt4 恒 `"hedging"`（平台语义注释）；migration 280 `margin_mode VARCHAR(16)` NULL=unknown+COMMENT 明载"NOT the investor-flag account_method"；写链 `account_crud`→`AccountInfoUpdate.MarginMode`→双 UPDATE `NULLIF($N,'')`（""→NULL）；sqlc 生成物等价手编（margin_mode 尾追加，与 v1.31.1 惯例一致，重生成仍被 138/142 阻断同 DATA-TRUTH-3）；读链 `COALESCE(margin_mode,'')` 第四列→`AccountIdentity.MarginMode`→`lctx.AccountMode` DB 直读优先、`accountModeForMTType` 降级 MT4 legacy 兜底；model `MarginMode *string`+全构体 gofmt 顺带还清存量债。测试：T1 helper 四态（Netting/Hedging/Default/99）+T3 四子优先级（DB netting wins/MT5 empty=""/MT4 legacy→hedging/MT4 DB wins）+T4 netting VM 端到端（MARGIN_MODE=0/HEDGE_ALLOWED=0）+T2 SQL 审查代（无 DB 环境如实声明，NULLIF+参数序位核对）。**机检独立重跑**：build✓/vet✓/五包 1497+race 1440（service 3 DB 测试 connection refused 环境性 HEAD 同败）/gofmt 触碰文件全净/check-lines 0 errors/diff --check。**独立 mutation×2**：M1 accMethodToString 恒""→T1 Netting/Hedging 精确 RED（Default/99 不受影响）✓；M2 摘 DB 直读→T3 `mt5 DB netting wins` 精确 RED（`="" want netting`——netting 无法由 MT4 兜底产出，判别精确；mt4 兜底子用例不受影响同施工报告）✓；M3 无直接向量如实记录接受（mt4 单行字面量常量，无 adapter 直测）。范围 18 文件零越界（account_method/BrokerInfo/ProfitUpdate/PositionSnapshot/proto mode=13 未动）。 | ✅done（Devin CLI 验收 2026-09-19，commit 4578cb4e） |
| ACCOUNT-MARGIN-LEVEL-PCT-1 | **`AccountInfoDouble(ACCOUNT_MARGIN_LEVEL)` 返回比率而非 MQL5 百分比——100× 语义偏差（P3→已证真 bug，2026-09-19 LIVE-ACCOUNT-FIELDS-1 设计实查分立+同日复核定案）**：`vm_builtin_mql5_info.go:60-64` 返回 `Equity.Div(Margin)`（如 9.92）。**MQL5 官方文档实证**：`ACCOUNT_MARGIN_LEVEL`="Account margin level in percents"=equity/margin×100，官方示例 9921.24/1000→992.12——本实现得 9.92，**100× 偏差确认**；佐证 mtapi f7 注释 "Margin percent"、`mt_accounts.margin_level` 同按百分比存储——全栈约定一致 VM 是唯一异类。后果：margin call/stop-out 阈值判断类策略（level>500 等）在 VM 下永不触发。**裁定修复**：`Div(Margin)` 后 `.Mul(decimalHundred)`（`vm_builtin_math.go:97` 旁加包级变量——本文件未 import shopspring）；`Margin.IsZero→0` 边界保留（MQL5 约定）；本地重算权威（Equity/Margin 两输入已接），**不改用** mt_accounts.margin_level/PositionSnapshot 直读（避免第三条同步链路）。边界记录：`sdk.AccountInfo.MarginLevel` 死字段（零写入方零读取方）另债不阻；`AccountMarginLevel` MQL4 builtin 不存在无需同步；存量零 margin-level 值断言无波及。派工单 `docs/audits/builder-handoff-account-margin-level-pct-1.md`（S1-S2+mutation）。 | 🟦open |——**🟦open（施工完成，待独立复审）**：S1 ×100（decimalHundred 包级变量，math.go 仅 +2 行手插未触发该文件既存未格式化面重排）；S2 T1 百分比断言（992.124 精确值=派工单 992.12 展示值的未舍入形态，accountNoopTestContext 补 margin 控制字段）+T2 zero-margin 保留 pin；M1 摘 ×100→T1 RED（9.92124）→恢复 GREEN；T2 独立不受影响。零范围外。**Devin CLI 独立复审通过 2026-09-19**（施工 ac509e20）：S1 与 spec 逐格相符——`.Div(Margin).Mul(decimalHundred)`（`decimalHundred` 包级变量置 math.go `decimalZero` 旁，safeDecimalFromFloat 惯例——本文件未 import shopspring 的既定处置）+零边界 `Margin.IsZero→0` 保留+官方文档出处注释。测试 `vm_margin_level_pct_test.go` +64：T1 精确值判别 992.124（官方示例 9921.24/1000 未舍入形态，失败消息含比率值对照）+T2 零边界 pin；`accountNoopTestContext` 补 margin 控制字段（+2 行）。**机检独立重跑**：build✓/vet✓/mql2go **749**/race **749**/check-lines 0 errors/diff --check；gofmt——math.go 存量未格式化面 builder 声明"+2 行手插不触发重排"核实属实（gofmt diff 仅 @@76 存量区，新增行不在内，范围纪律正确）。**独立 mutation**：M1 删 `.Mul(decimalHundred)`→T1 精确 RED（`=9.92124 want 992.124` 比率复活）+T2 边界独立不受影响仍绿；restore→GREEN 工作区与 ac509e20 逐字节一致。范围 6 文件零越界（死字段 MarginLevel/直读链路/其余 SO_* prop 未动）。 | ✅done（Devin CLI 验收 2026-09-19，commit ac509e20） |
...
## MQL-COMPILER-LOCAL-ARRAYS：局部动态数组编译盲区（✅done 2026-09-19 Devin CLI 验收；2026-09-08 立项）
...
**🟦open（施工完成，待独立复审）**（builder commit 见 git log）：S1 ExprArrayNew kind；S2 compileDeclaration initializer 前置拒（关 bug B）+array_declarator 分支产 ExprDecl{ExprArrayNew}（非常量维度 `int a[n]` 编译拒——派工单检查规格修正为"跳过首个 identifier"）；S3 OP_NEW_ARRAY/OP_ARRAY_RESIZE 追加尾；S4 astCompiler ArrayNew case+ArrayResize 拦截（ExprVar arg0→负/正槽 OP_ARRAY_RESIZE，非变量回退 builtin）+删两盲记；S5 两 opcode 实现（NEW_ARRAY NoneVal 零值/RESIZE 槽回写=bug A 修复核心）+负路径 vm.locals 实读实写（OOB/not-an-array/slot-range fail-closed 全保留）；S6 T1-T7+三 pin 随语义升级反转/更新（TestNegativeSlotDirect 消息更新/TestLocalArrayAccessFails 标量局部 not-an-array/TestLocalArrayStillRejected→TestLocalArrayDeclAccepted——派工单点名反转）。mutation：M1 NEW_ARRAY→NoneVal→T1 RED（RESIZE not-an-array）✓；M2 负路径恢复拒收→T6 RED ✓；M3 删槽回写→T3 RED（g[4] 写 OOB——bug A 复活等价判别）✓；M4 删前置扫描→T5 两 initializer 子用例 RED（静默误编译复活）✓。  **Devin CLI 独立复审通过 2026-09-19**（施工 `5c947ce3`）：diff 与派工单 S1-S6 逐格相符——ExprArrayNew kind/OP_NEW_ARRAY+OP_ARRAY_RESIZE 尾追加/compileDeclaration 三拒绝（initializer 前置扫描+非常量维度"跳过首 identifier"+multiDim 镜像 global）+ExprDecl{ExprArrayNew} 产码/ArrayResize ExprVar 拦截（负/正槽 OP_ARRAY_RESIZE）/两盲记删除/OP_NEW_ARRAY NoneVal 零值+范围钳位/RESIZE 槽位回写（bug A 修复核心）/负路径 vm.locals 实读实写（三级 fail-closed 保留）/三 pin 语义升级（TestNegativeSlotDirect 消息更新/TestLocalArrayAccessFails 标量 not-an-array/TestLocalArrayStillRejected→TestLocalArrayDeclAccepted）。**机检独立重跑**：build✓/vet✓/gofmt 触碰文件全净（compile_py* 5 脏为存量未触碰）/mql2go **761**/race **761**/check-lines 0 errors/diff --check。**独立 mutation×4**：M1 OP_NEW_ARRAY→NoneVal→T1 RED（`OP_ARRAY_RESIZE slot -1 is not an array`）✓；M2 负路径恢复"not supported"→T6 RED（reject 先于 OOB 触发，vet unreachable 佐证死码插入位正确）✓；M3 删 RESIZE 槽回写→T3 RED（`index 4 out of range (len=2)`——bug A 精确复活）+T1 RED（`index 1 out of range (len=0)`）✓；M4 删 init_declarator 前置扫描→T5 两 initializer 子用例 RED（err=nil 静默误编译复活；non-const/multi-dim 不受影响仍绿）✓；restore→761 全绿，工作区与 5c947ce3 逐字节一致。范围 10 文件零越界。 | ✅done（Devin CLI 验收 2026-09-19，commit 5c947ce3） |
