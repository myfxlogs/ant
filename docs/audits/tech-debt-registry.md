...
| VM-STATIC-LOCAL-1 | **函数内 `static` 局部变量不持久——每次调用重新初始化（P3 语义缺口，实盘探针抓出）**：探针 `static int done` 每 tick 重打证明 static 被当普通局部变量。依赖 static 记状态的 MQL 策略会行为漂移（只跑一次的初始化每 tick 重跑、计数器失效）。修法：static 局部需提升到函数级持久存储（VM frame 外）。 | 🟦open（2026-09-21 实盘探针实证登记→立项完成；设计 SSOT `docs/audits/design-vm-static-local-1.md`（Devin CLI 独立复审通过）+ 施工单 `docs/audits/builder-handoff-vm-static-local-1.md`：编译层脱糖 mangled-global+init-guard，零字节码格式变更；MQL 官方语义实锤=首达声明时初始化一次；7 个真实策略 static 零命中=零兼容风险；待施工）|
