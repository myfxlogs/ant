...
| POST-2 | 性能/容量压测（下单/回测/SSE）。**2026-09-19 设计实查**：本机即生产（compose 全栈+实盘账户）→全量压测不可行；容量模型代码实查：回测 3-worker+SKIP LOCKED+lease 有界、per-user 配额有界、**SSE 扇出无界**（待探针量化）、下单真路径不可测于本机。派工探针批（in-process 零触生产）：VM 并发退化曲线+SSE 扇出成本+paper 下单延迟+容量基线文档。| 🟦open（设计实查完成→派工 POST-2 探针批，builder-handoff-post2-capacity-probe.md）|
