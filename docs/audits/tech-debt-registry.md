...
| CODE-SIZE-MT5-ORDERS-1 | **mt5/orders.go 行数贴预警线（P3 存量债，VM-LIVE-VENUE-1 复审裁决单独立项）**：HEAD 施工前已 449/300 🟡（1.497×），R2 +4 行顶过 1.5× ERROR 线后 builder 压缩注释回 450=正好 1.5× 贴线——下次该文件任何净增行即 🔴 阻断。修法：按语义边界抽函数（参照 VM-CODE-HYGIENE-1 先例：FetchSymbolParams/PlaceOrder/历史查询拆 orders_*.go 子文件）。 | 🟦open（待施工——独立批，非 R2 范围）|
