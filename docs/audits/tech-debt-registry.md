...
| ORDERSEND-NILBROKER-FAILCLOSED-1 | **`builtinOrderSend` 无 broker 静默 `-1`+nil error 非 fail-closed（P2，QS-2.3 连带记债，spec §4.4 指定另立）**：`vm_builtin_trade.go` OrderSend 在 `vm.ctx.Broker()==nil` 时返回 `IntVal(-1), nil`——策略侧无法区分"无 broker"与"真实拒单"，且 nil error 使 VM 不进入错误路径。修法方向：无 broker 时应 fail-closed（明确 error 或专用错误码），需先审计依赖 -1 返回的存量策略/测试再动。 | 🟦open |
