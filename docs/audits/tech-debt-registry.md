...
| G-POST2-2 | stream 类 RPC 无活跃流数 metric（P3 观测，POST-2 探针批发现）：无 metric 则扇出成本外推无法在生产侧验证（QS-3「生产 p99 待 metric」同族）。 | ✅done（随 G-POST2-1 同批复审通过 2026-09-19——GaugeVec/Counter promauto 注册+T2 testutil 断言独立核实）|
