...
| FIX-2026-08-27-SCHEDULE-HEALTH-ORDER-HISTORY-GAP | **schedule_health_repo 仍查死表 order_history（P2 数据展示，2026-08-27 S3 验收发现）**：`schedule_health_repo.go:136`（`GetLatestOrderProfit`）和 `:172`（`ListRunLogs`）仍直接 SELECT FROM order_history（S2 已将 GetOrderHistory 改查 trade_records，但这两处未同步）。order_history 表 0 行 → schedule health API 的 latestOrderProfit 和 runLogs 返回空。被 `schedule_health_handler.go:96,122` 调用。**修复方案**：改查 trade_records，镜像 S2 的 GetOrderHistory 改法（显式 SELECT + scan 适配 schema 差异）。 | 🟦open（待施工）|
