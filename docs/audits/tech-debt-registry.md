...
| CQ-11 | **internal/ai 传递性死代码簇（LOWPRI-SWEEP-1 复审发现，2026-09-19）**：`template_library.go`（TemplateLibrary/NewTemplateLibrary/Match*/topByCategory/countMatches 全方法零调用方）、`strategy_prompt.go` 的 `PromptParams`+`BuildSystemPrompt`+`BuildUserPrompt`+`Template` 字段+`ParameterSlotsString`（PromptParams 生产零构造——NewStrategyPromptBuilder 唯一调用方 auto_fix.go:66 只用 FeedbackPromptParams）+ `repository.AIStrategyTemplate` 类型（CQ-10 删除后仅存语法引用）。`DetectCodeStrategyType` 调用方需复核。删法=整簇删除（非 CQ-10 原范围）。 | 🟦open（低优死代码，复审副产登记）|
| CQ-12 | **`npm run lint` 主枝存量红（LOWPRI-SWEEP-1 复审发现，2026-09-19）**：`WorkspaceCenterColumn.tsx` 313 行——eslint `max-lines` error（281 计行>250）+2 处 `react-hooks/exhaustive-deps` warning；文件自 `0d52f0a6` 起即超限，非本批引入。eslint error 使 lint 命令恒 exit 1（与 `--max-warnings 73` 无关）。修法=拆文件（UI 分层，非本批范围）。 | 🟦open（低优，存量 lint 门禁红）|
