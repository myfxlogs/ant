...
| DEPLOY-LIVE-4 | gate fail-open：`evaluatePlaceGate`/CloseOrder `gate==nil || accountStateProvider==nil` → 静默放行（P2）| ✅done（实现 2026-08-12 `826fbf5a`；**审计方验收：NilGate 对抗 4/4 无效（删 gate 分支仍绿，2026-08-13 独立删行实测）-> 批次5 补强待施工**） |
...
| DEPLOY-LIVE-7 | 死代码/断链：handlers.go:208-210 未用 gate；WatchSchedules SSE `schedule_change` 无人 NOTIFY 且前端不消费（P3）| ✅done（2026-08-12 删除死 gate + 接 pg_notify；**LIVE-7b 对抗缺失（NOTIFY->SSE 零测试）-> 批次5 冒烟补强待施工**） |
