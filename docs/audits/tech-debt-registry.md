...
| VM-STATIC-LOCAL-1 | **函数内 `static` 局部变量不持久——每次调用重新初始化（P3 语义缺口，实盘探针抓出）**：探针 `static int done` 每 tick 重打证明 static 被当普通局部变量。依赖 static 记状态的 MQL 策略会行为漂移（只跑一次的初始化每 tick 重跑、计数器失效）。修法：static 局部需提升到函数级持久存储（VM frame 外）。 | 🟦open（2026-09-21 实盘探针实证登记，未修）|
