...
| QS-2.2 | **goleak 集成（P1 工程纪律）**：`go.uber.org/goleak`（≥7 天版本）进 `strategy`/`mql2go` 包 `TestMain`；附 `TestWaitConfirmed_NoGoroutineLeak` 作为 QS-2.1 "watcher 泄漏"否决证据。spec §4.1。【最终决策：Devin CLI（[角色:决策终] 激活）】 | 🟦open |
