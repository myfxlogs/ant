...
| QS-2.5 | **coordinateMutation/dispatchLiveSignal 无 panic recovery（P1 稳定性）**：recover 必须同时 `barrier.NotifyOutcomeUnknown()` + `SetCircuitOpen(true)` + `RecordError`，不能只改返回值。spec §4.3。【最终决策：Devin CLI（[角色:决策终] 激活）】 | 🟦open |
