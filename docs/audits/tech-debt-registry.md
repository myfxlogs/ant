...
| VM-ARRAY-OOB-FAILCLOSED-1 | **用户数组越界读写静默（P2，VM-RUNTIME-FAILCLOSED-2 独立复审分立）**：`vm_execute.go` `executePushArray`（:298-309）索引越界/非数组/槽位越界 → 静默返回 `NoneVal`（假值入栈）；`executeStoreArray`（:311-323）同条件 → 静默丢弃写入。真实 MQL `array out of range` 是致命运行时错误（EA 终止）。编译器对 `arr[i]` 直接 emit OP_PUSH_ARRAY/OP_STORE_ARRAY（`compile_expr.go:144,156`），动态索引运行时可达——与 FAILCLOSED-2 修复的 slot 越界同类（静默 OOB → 假值/丢失），但 spec 证据链只列 slot 分支故未含。修法方向：两处越界 → `setStackError`（同 `OP_PUSH_VAR` 模式），注意区分"局部数组"（compiler 已记 blind spot 降级）与全局数组路径；补 OOB 读/写行为测试 + mutation RED→GREEN。 | 🟦open |
