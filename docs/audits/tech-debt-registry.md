...
| PY-DECIMAL-CTOR-1 | **Python `Decimal("0")` 经 `case "Decimal"` 直返 `StringVal("0")` → 构造器产 ValString 非 ValDecimal（P2 类型失真，QS-1.4 复审发现）**：`backend/tools/mql2go/compile_py_expr.go` `case "Decimal":` `return &args[0]` 原样透传；`Decimal("0")` 得 string → `bool(Decimal("0"))=true`（真 decimal 应为 false）、`str()`/类型判定失真。修法（待施工）：`Decimal(...)` 参数为字符串/数字字面量时应产生 ValDecimal（常量折叠或 builtin 调用）。 | 🟦open |
