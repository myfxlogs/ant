...
| I18N-MIXED-1 | `marketplace_messages_published: '策略 published to marketplace!'` + `marketplace_messages_publish_failed: '失败 to publish strategy'`（base_zh-cn.textproto）中英混杂（P3 i18n，2026-08-15 审计 LIVE-REDESIGN-2TAB 时发现）。**2026-09-19 Devin CLI 设计实查——债面远超登记**：①**Class A**（i18n-check 可检）：zh-cn 全 locale **216 条**值与 en 完全相同（`npx tsx scripts/i18n-check.ts --strict` 报 Untranslated，base 占 150+）；②**Class B**（不可检）：base_zh-cn **~95 条**中英混杂模板（`失败 to X`×25/`删除 this Y`/`Top 策略 by Revenue`/`Please enter 基础 URL` 等）；③缺字段 `strategy_gen.exec_feedback_placeholder`；④**门禁缺口**：`i18n-check.ts --strict` 存在但 pre-commit/CI 零接线（strict 全量 1519 错误含 vi/zh-tw 缺字段无人问津）。管线：`i18n-translate-zh-cn.ts`（DICT exact-match 优先已存在 :226，但 main() 仅处理 base）→ `i18n-build.ts` → `frontend/src/i18n/resources/zh-cn/*.ts`（git 跟踪产物）。派工单 `docs/audits/builder-handoff-i18n-mixed-1.md`：S1 DICT 补全 ~200 整句条目+main() 泛化全 locale 文件；S2 Class B 附录 B ~99 行直接替换；S3 缺字段补齐；S4 i18n-check 加 `--locale` 旗标+pre-commit 接 zh-cn strict。附录 C 白名单（品牌/语言名/指标符号/AI prompt 内部串 {{text}}系）。zh-tw 212/ja/vi 缺口另立 I18N-MIXED-2。 | 🟦open（设计实查完成 2026-09-19，待施工）|
