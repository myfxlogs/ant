...
| QS-1.3 | **Python 函数内赋值落 VM 全局槽 → 跨事件/跨函数状态污染（P0 语义，审计 P1-3）**：`compile.go:282-289` 对 `Version=="python"` 隐式全局。修法：在 `astCompiler` 赋值编译中复用既有 `localScopes/nextLocalSlot`（`compile.go:244-263`, `compile_expr.go:200`）分配局部槽，不在 pyCompiler 另造；`global` 语句未支持则编译期报错。风险：依赖全局污染的存量策略行为改变，覆盖度报告标记。spec §3.3。【最终决策：Devin CLI（[角色:决策终] 激活）】 | 🟦open |
