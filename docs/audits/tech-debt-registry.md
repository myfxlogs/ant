...
| VM-FUNC-FATAL-DELAY-1 | **用户函数内 fault 后非-builtin 指令继续执行至 RETURN（P3，VM-RUNTIME-FAILCLOSED-2 独立复审分立）**：`executeCallUser` 内层循环（`vm_execute.go:347-383`）只查 ctx/ticks/MaxStackDepth/`execute` 返回 error，**不逐指令检查 `vm.fatalError`**——函数内 `x=10/0` 后，后续 `OP_STORE_GLOBAL` 等纯栈/槽位指令仍执行（写入泄漏），直到 OP_RETURN 回 runLoop 顶部检查才传播。缓解面：broker 副作用被 `OP_CALL_BUILTIN` 的 popN 后 `fatalError` 检查（:123-125）门控，事件级 fail-closed 仍成立（Engine.Run 收到 error）。属既有架构属性（所有 setStackError 路径共享），非本批回归。修法方向：内层循环顶部加 `vm.fatalError` 检查（对称 runLoop:14）+ 函数内 fault 后 global 写入不生效的行为测试。 | 🟦open |
