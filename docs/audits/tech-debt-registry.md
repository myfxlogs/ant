...
| ACCOUNT-MARGIN-LEVEL-PCT-1 | **AccountInfoDouble(ACCOUNT_MARGIN_LEVEL) 返回比率疑与真实 MQL5 百分比 100× 偏差（P3 待核，2026-09-19 LIVE-ACCOUNT-FIELDS-1 设计实查分立）**：`vm_builtin_mql5_info.go:60-64` 返回 `Equity.Div(Margin)`（如 5.0），真实 MQL5 `ACCOUNT_MARGIN_LEVEL` 报百分比（如 500.0）；mtapi AccountSummary f7 `MarginLevel` 注释 "Margin percent"、`mt_accounts.margin_level` 亦按百分比存储。需先核 MQL5 官方语义（百分比 vs 比率）再定修否——若确为 100× 偏差，全部依赖 margin level 阈值判断的策略失真。修法方向：确认官方语义后 `Mul(100)` 或维持现状+注释；同时评估是否应改从 `mt_accounts.margin_level`/`PositionSnapshot.MarginLevel` 直读（与 Equity/Margin 本地重算一致性核对）。 | 🟦open |
