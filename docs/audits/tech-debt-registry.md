...
| QS-1.2a | **`GetLastError/ResetLastError/SetUserError` 为 no-op（P1 语义，审计 P1-2）**：`vm_builtin_impls.go:73-74` + `vm_builtin_checkup.go:102`。修法：VM 加 `lastError int32`，三 builtin 真实实现（读后清零，不按事件清零），本轮唯一写入点 `SetUserError`。**交易失败路径保持 FAILCLOSED-1 fatal（D-009），QS-1.2b 不立项。** spec §3.4。【最终决策：Devin CLI（[角色:决策终] 激活）】 | 🟦open |
