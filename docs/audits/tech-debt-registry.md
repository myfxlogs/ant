...
| QS-1.7-INV | **open outcomeUnknown 永久锁仓可恢复性调研（P1，只查不改，审计 P1-8）**：确认 `strategyOrderClientID`（`live_dispatch.go:366`）是否随 Comment 下发 mtapi 并在 MT4/MT5 `OpenedOrders` 回显。可靠 → 立 QS-1.7 按 ClientID 精确单匹配 recovery；不可靠 → 维持 ④-② fail-closed + runbook。模糊匹配永久否决（D-009）。spec §3.5。【最终决策：Devin CLI（[角色:决策终] 激活）】 | 🟦open |
