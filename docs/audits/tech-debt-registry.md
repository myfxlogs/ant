...
| VM-STATIC-LOCAL-1 | **函数内 `static` 局部变量不持久——每次调用重新初始化（P3 语义缺口，实盘探针抓出）**：探针 `static int done` 每 tick 重打证明 static 被当普通局部变量。依赖 static 记状态的 MQL 策略会行为漂移（只跑一次的初始化每 tick 重跑、计数器失效）。修法：static 局部需提升到函数级持久存储（VM frame 外）。 | **⚠️待独立复审（2026-09-21 施工完成，不标 ✅done）**：按设计 SSOT 编译层脱糖落地——`interp/ir.go` Expr 加 `Static bool`；`compile_interp.go` `staticClassSpecifier` helper（镜像 isExternDeclaration 先例）+`compileDeclaration` 打标/局部非 static storage class 与 input/sinput fail-closed 编译错+`collectFunction` 拒函数位 static（CST dump 实证 function_definition 位与 declaration 位同为首 named child storage_class_specifier）；`compile.go` staticScopes 别名栈（pushScope/popScope 同步，长度不变量唯一改动点复核）+resolveVar/resolveVarWrite 每层先局部后 static；`compile_expr.go` compileDecl `e.Static` 分流 mangled `__static_N`/`__sinit_N` 全局槽+旗标守卫 init-once（不占 frame 槽，EventLocals=0 实证）；测试 `compile_static_test.go` T1–T11+IR pin 先红（stash 三行为文件实证 12/12 红）后绿；mutation M1（删 e.Static 分支→T1/T2/T3/T6 红）/M2（resolver 不查 staticScopes→T1 `unknown variable: d` 红）/M3（守卫恒不跳→T2/T3 红）/M4（helper 恒空→12/12 红）逐条 RED→restore→GREEN（md5 校验还原一致）；机检 build/test/vet/gofmt/check-lines 0 errors/race×3/diff --check 全绿、mql2go 全量含 golden/e2e 零新增失败。**施工方偏差申报 1 项待裁决**：S3 片段的两处同 scope 重定义 dup 检查与 T4 冲突（compileIf 不 pushScope→if/else 两枝共享 map 层，dup 检查拒绝 T4 要求编译通过的同名 static 并对「if 枝普通局部+else 枝 static」误报）→按验收标准优先去除 dup 检查改为重绑定语义（引用在编译点捕获当时槽位，兄弟枝互不串扰；与 plain 路径重声明宽容一致）|
