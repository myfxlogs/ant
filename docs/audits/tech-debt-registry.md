...
| QS-2.4 | **VM 管线 data race 审计（P1）**：`-race -count=3` 覆盖 `tools/mql2go`/`internal/connect/strategy`/`strategy`；PositionCache mutex 覆盖、TradeBarrier cond 配对审计。spec §4.2。【最终决策：Devin CLI（[角色:决策终] 激活）】 | 🟦open |
