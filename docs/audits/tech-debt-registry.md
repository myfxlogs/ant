...
| QS-2.3 | **`vm.ctx == nil` 101 处散布检查（P2 洁净）**：`NewVM` 注入 noopContext，删检查。连带记债：`builtinOrderSend:15-17` 无 broker 时静默 -1 + nil error 非 fail-closed（另立）。spec §4.4。【最终决策：Devin CLI（[角色:决策终] 激活）】 | 🟦open |
