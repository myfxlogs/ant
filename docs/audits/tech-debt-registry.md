...
| QS-1.4 | **Python `bool(x)` 编译为 `x != 0` → `bool(None)=true`（P0 语义，2026-09-16 VM 深度审计 P1-4）**：`compile_py_expr.go:165-172` + `value.go:153-168` 混合 None/Int `Equal=false`。修法：编译为双重 `OP_NOT`（复用 `IsTrue()`），零新 builtin/opcode；顺手修正 `vm_helpers.go:250` 过时注释。spec `docs/spec/vm-pipeline-quality-stability-improvement-plan.md` §3.1。 | 🟦open |
| QS-1.6 | **read-after-write verify 成功后 `return barrierConfirmed` 绕过状态机（P0 状态机 SSOT，审计 P1-7）**：`mutation_coordinator.go:330-334`，barrier 停在 `acceptedUnconfirmed` 即被 Release。v1 修法 `Reconcile(true)` 在该状态是 no-op（`trade_barrier.go:333`），否决。修法：新增 `TradeBarrier.ConfirmByAuthoritativeRead()` 正式迁移；施工方先回答 `NotifyConfirmationEvent` 为何未迁移并回填此处。spec §3.2。 | 🟦open |
| QS-1.3 | **Python 函数内赋值落 VM 全局槽 → 跨事件/跨函数状态污染（P0 语义，审计 P1-3）**：`compile.go:282-289` 对 `Version=="python"` 隐式全局。修法：在 `astCompiler` 赋值编译中复用既有 `localScopes/nextLocalSlot`（`compile.go:244-263`, `compile_expr.go:200`）分配局部槽，不在 pyCompiler 另造；`global` 语句未支持则编译期报错。风险：依赖全局污染的存量策略行为改变，覆盖度报告标记。spec §3.3。 | 🟦open |
| QS-1.2a | **`GetLastError/ResetLastError/SetUserError` 为 no-op（P1 语义，审计 P1-2）**：`vm_builtin_impls.go:73-74` + `vm_builtin_checkup.go:102`。修法：VM 加 `lastError int32`，三 builtin 真实实现（读后清零，不按事件清零），本轮唯一写入点 `SetUserError`。**交易失败路径保持 FAILCLOSED-1 fatal（D-009），QS-1.2b 不立项。** spec §3.4。 | 🟦open |
| QS-1.7-INV | **open outcomeUnknown 永久锁仓可恢复性调研（P1，只查不改，审计 P1-8）**：确认 `strategyOrderClientID`（`live_dispatch.go:366`）是否随 Comment 下发 mtapi 并在 MT4/MT5 `OpenedOrders` 回显。可靠 → 立 QS-1.7 按 ClientID 精确单匹配 recovery；不可靠 → 维持 ④-② fail-closed + runbook。模糊匹配永久否决（D-009）。spec §3.5。 | 🟦open |
| QS-2.2 | **goleak 集成（P1 工程纪律）**：`go.uber.org/goleak`（≥7 天版本）进 `strategy`/`mql2go` 包 `TestMain`；附 `TestWaitConfirmed_NoGoroutineLeak` 作为 QS-2.1 "watcher 泄漏"否决证据。spec §4.1。 | 🟦open |
| QS-2.4 | **VM 管线 data race 审计（P1）**：`-race -count=3` 覆盖 `tools/mql2go`/`internal/connect/strategy`/`strategy`；PositionCache mutex 覆盖、TradeBarrier cond 配对审计。spec §4.2。 | 🟦open |
| QS-2.5 | **coordinateMutation/dispatchLiveSignal 无 panic recovery（P1 稳定性）**：recover 必须同时 `barrier.NotifyOutcomeUnknown()` + `SetCircuitOpen(true)` + `RecordError`，不能只改返回值。spec §4.3。 | 🟦open |
| QS-2.3 | **`vm.ctx == nil` 101 处散布检查（P2 洁净）**：`NewVM` 注入 noopContext，删检查。连带记债：`builtinOrderSend:15-17` 无 broker 时静默 -1 + nil error 非 fail-closed（另立）。spec §4.4。 | 🟦open |
| QS-3-BASELINE | **VM 性能基线测量（贯穿）**：benchmark（dispatch/builtin/Decimal/1000 tick）落 `docs/audits/vm-perf-baseline-2026-09.md` + 生产单事件耗时直方图 metric。v1 阶段 3 全部优化项已否决（D-009），只有数据触发阈值才立优化条目。spec §5。 | 🟦open |
