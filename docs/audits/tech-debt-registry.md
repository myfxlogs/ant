...
- 2026-08-12 **批次3 审计方验收（独立删行复测，不信任施工方声明）**：实现核对 4/4 通过（fail-closed 双分支 / ClientIPFn 接线 / buildLiveRun 四道门 / 死 gate 零残留 + 4 写路径 NOTIFY :106/:153/:201/:231 + WatchSchedules Listen）。门禁实测：go build ./... ✓ + mthub/risk/strategy 3 包测试 ✓。**对抗复测**：LIVE-6 有效（删 entitlement 门→dispatch+launch 两路径同 RED，断言级）；LIVE-5 有效（rule 行为契约；接线无单测固有，代码核对）；**LIVE-4 对抗 2/4 无效**（施工方"删行→4 测试 RED"声明不实——删 gate 分支仍绿，err 实来自 stateProvider 分支，详见条目）；**LIVE-7b 对抗缺失**（无集成测试无冒烟，靠代码核对 + 待冒烟）。**裁决：LIVE-5/6 ✅ 验收通过；LIVE-4 实现 ✅ 对抗补强待施工；LIVE-7a ✅；LIVE-7b 实现 ✅ 对抗补强待施工。** 另：施工方工作树已实现 DEPLOY-LIVE-8 修复（lifecycleCtx + runParent + nil 守卫，未提交）——审计方顺带独立删行验证：删 lifecycleCtx 来源行→CancelledHandlerCtx RED / 删 nil 守卫→NilLifecycleCtx RED，**对抗 2/2 有效**，待施工方提交后正式验收。
