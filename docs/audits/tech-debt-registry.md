...
| VM-LIVE-PARITY-F1 | **实盘 OrderSend 结果回显请求值非 broker 事实（P2，实盘对账实锤）**：`OrderResult{Price: req.Price, Volume: req.Volume}`（strategy/runner/broker.go:51-55）——executor 只回 ticket，实际 fill 价/量不回读。实盘证据：请求 price=0→fill 81262.24、volume 0.015→broker 归一化 0.02，VM 返回值=请求回显≠broker 事实。修法：fill 后回读（position/OrderHistory 对账填实际值）或如实标注 requested 语义；deviation 形参丢弃同链处理（接线或声明）。回测无此问题（模型价=请求价）。 | 🟦open |
| VM-LIVE-PARITY-F2 | **SymbolParams 瘦数据链（P3，实盘对账实锤）**：mtapi 上游对 BTCUSDm 不返回 point/lotStep/stopLevel/maxLot/tradeMode（实测全 0，lotStep 实存 0.01 经归一化证明）→VM 实盘 `MarketInfo(MODE_POINT)`=0，按 point 算 SL 距离的策略拿零值。修法：point 从 digits 推导 fallback（10^-digits）并标注来源；其余字段缺省应如实"未知"非 0；或经 SymbolInfoEx（mt4.pb.go:3487 全字段）补链。 | 🟦open |
| VM-LIVE-PARITY-F3 | **comment 字段在途丢失+UTF-8 损耗（P4，实盘对账实锤）**：PARITY-* comment 到 broker 记录全空（magic 正常透传）；既有持仓 comment 显示 `\ufffd` 乱码——adapter→mtapi→MT4 comment 编码链有 ANSI/UTF-8 边界损耗。 | 🟦open |
