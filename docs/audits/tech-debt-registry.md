...
| EXT-BOUNDARY-WAVE2 | 第二梯队批（P2）：LLM SSE 流无死检测（`systemai/chat_stream.go:115`）+ 链上 monitor 无 stall 检测（`chain/monitor.go:117`）+ SMTP auth 失败吞（`notifier/email.go:105`）+ webhook 门控 transport 错 fail-open（`agent/hooks.go:173`）+ TronScan 第二源 fail-open（`monitor.go:293`）+ LLM `FinishReason` 丢弃截断静默交付（`chat.go:285`）。**2026-09-19 Devin CLI 设计实查**：六项全部实证仍在——①SSE `Timeout:0` 只有首字节 ResponseHeaderTimeout，`scanner.Scan()` 块间无超时且调用方 ctx 无 deadline 兜底（stall=永久挂起）；②monitor scanBlocks 持续 error 只 log，checkpoint 停滞零观测；③SMTP `conn.Auth` 失败 warn 后无认证续发（凭证腐烂静默）；④webhook transport 错→allow，与同文件 `>=400→Abort` 不对称；⑤TronScan 双败→`verified=true` 降级单源自动确认（真钱 fail-open，MANUAL_REVIEW 路径已存在）；⑥`parseChatResponse` 从不读已解析的 FinishReason，length 截断原样交付。派工单 `docs/audits/builder-handoff-ext-boundary-wave2.md`（S1-S6+对抗证明 M1-M5）。**🟦open（施工完成，待独立复审）**（builder commit 见 git log）：S1 看门狗（stallTimeout=effectiveTimeout 钳位默认 5min；atomic lastActivity/stalled；watchdog 关 body 解除 scanner 阻塞；stalled 判定先于 scanner.Err 报因）+chat_stream 拆分（handleStreamResponse 移 chat_stream_scan.go 解 🔴456>450）；S2 lastProgressAt/lastSafeLatest+checkpointStalled 谓词方法+1min stallTicker（链停不误报）；S3 通告 AUTH 才认证、失败显式 error（无 AUTH relay 静默）；S4 transport/请求构造错→Abort（对称 ≥400）；S5 删 verified=true 降级→createManualReviewDeposit helper 三路共用；S6 length→non-transient failoverErr。mutation：M1 删看门狗→挂起 RED；M2 S4 fail-open 复辟→RED；M5 S3 warn-continue 复辟→RED；M4 删 length 检查→err=nil RED；M3 恢复 verified=true→processEvent 端到端用例判别（ConfirmDeposit nil pg panic）。 | 🟦open（施工完成，待独立复审）S1 看门狗 atomic+Body.Close（stallTimeout=effectiveTimeout 复用钳位默认 5min）；S2 lastProgressAt+lastSafeLatest 字段（链停不误报）+stallTicker Error（复用 runner_health 惯例）；S3 `conn.Extension("AUTH")` 判定（未通告=合法无认证 relay 保留，通告但失败=error）；S4 transport+请求构造错→Abort 对称 >=400；S5 双败→MANUAL_REVIEW+抽 createManualReviewDeposit helper 消重复 Create；S6 `length`→non-transient failoverErr（同参重试必再截断勿烧预算）。边界如实记：execCommand 非-2 退出 fail-open 同族另债；流式 FinishReason 消费方处置；content_filter 语义。 | 🟦open（设计实查完成 2026-09-19，待施工）|

**🟦open（施工完成，待独立复审）**（builder commit 见 git log）：S1 SSE 看门狗（stallTimeout=effectiveTimeout 钳位默认 5min，atomic lastActivity/stalled，watchdog 关 body 解除 scanner 阻塞，stalled 判定先于 scanner.Err 报因）+chat_stream 拆分（handleStreamResponse 移 chat_stream_scan.go 解 🔴456>450）；S2 monitor lastProgressAt/lastSafeLatest+1min stallTicker（链停 safeLatest≤lastBlock 不误报，15min 阈值 log.Error）；S3 SMTP 通告 AUTH 才认证、auth 失败显式 error（无 AUTH relay 静默）；S4 webhook Do err/NewRequest err→Abort（与 ≥400 对称）；S5 删 verified=true 降级→抽 createManualReviewDeposit helper 三路共用（inconclusive/未确认/confirm 失败）；S6 finish_reason=length→non-transient failoverErr（不烧 failover 预算）。mutation：M1 删看门狗→S1 挂起超时 RED；M2 S4 fail-open 复辟→RED；M5 S3 warn-continue 复辟→RED（err=smtp mail 连接关闭而非 smtp auth——fail-open 实证）；M4 删 length 检查→err=nil RED；M3 恢复 verified=true→helper 直测不判别（分支需 verify 链路集成，如实记录待裁定）。 | 🟦open（施工完成，待独立复审） |
