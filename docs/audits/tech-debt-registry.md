...
| TZ-SWEEP-AFFECTED-1 | **`timestamp` 列 vs 非 UTC Go time 参数同型 bug 批次（P2，RECONCILE-TZ-WINDOW-1 sweep §2）**：`marketplace/analytics.go:45` `since := time.Now()`（CST，缺 `.UTC()`）驱动 :54/:66/:79/:91/:101/:126/:137/:179/:182 共 ~9 站统计查询（wallet_transactions/user_subscriptions/marketplace_strategies 的 created_at 均为 UTC DEFAULT）→ GMV/手续费/订阅/新策略/退款等周期统计窗口起点偏 +8h，少计 8h 内交易；`repository/backtest_run_worker.go:144` `backtest_runs.created_at` vs `since` 参数（当前无调用方休眠 API，启用前须约束）。修法：`analytics.go:45` 单点改 `time.Now().UTC()`（一处愈全站）；worker 加参数 UTC 约束注释。 | 🟦open |
