...
- **复用核对（Reuse Preflight）—— 避免重复造轮子，强制门**：写任何**新 file/function** 前必须：①跑 `bash scripts/gen_capability_map.sh` 刷新 `docs/CAPABILITIES.md`；②用**动词+别名**（见该文件「动词/别名索引」）搜索；③在 PR 描述里逐条给结论，二选一——`REUSE: <symbol> @ <file:line>`（复用现成）或 `NEW: 无现成能力（已搜：<关键词>）`（确认真空白）；④发现某能力的状态/注释过时（如代码注释说"不存在"但其实已实现），必须同步修正注释与 `docs/CAPABILITIES.md`。**缺少 `REUSE:`/`NEW:` 引用 = 该 Task 直接判失败。**（典型教训：`MtHubService.ModifyOrder` 早已实现，却被误判"不存在"并另写绕路方案。）
