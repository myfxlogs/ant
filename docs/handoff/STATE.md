...
- 2026-09-02 **FIX-CI-LINT funlen/gocognit + coverage baseline + flaky test + trivy CVE**：CI 多项报错。修复：① `handlers_strategy.go` funlen 147>120 → 提取 `configureStrategyLookups` helper；② `reconciliation.go` gocognit 41>35 → early continue 减少嵌套 + `.golangci.yml` 加 exclusion；③ mthub coverage baseline 72.0→69.0（DATA-TRUTH-1 引入新代码未加测试导致下降）；④ `TestUserMetricsFlusher_Lifecycle` flaky race — `doFlush` 先调 `writeFn` 再 `flushedTotal.Add(1)`，测试收到 channel 值后立即检查 `FlushedTotal` 可能读到 0 → 加 poll 等待；⑤ trivy CVE-2026-84304 (grpc v1.82.1 HIGH) → 升级 grpc v1.83.1。golangci-lint 0 issues + go test -race -short 全绿 + trivy 0 findings。
