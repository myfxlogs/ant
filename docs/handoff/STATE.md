...
- 2026-09-02 **FIX-CI-LINT funlen/gocognit**：CI golangci-lint 报错——`handlers_strategy.go` configureStrategyExecution 147>120 (funlen) + `reconciliation.go` reconcileAccount 41>35 (gocognit)。修复：提取 `configureStrategyLookups` helper + `importGhostOrders` helper。golangci-lint 0 issues。
