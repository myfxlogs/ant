...
| 子任务 | 状态 | 锚点 |
|--------|------|------|
| VERIFY-CHAIN-SEMANTIC-1 交易台账 hash 链端到端语义修正（union 全局验链+写侧 hash 规范化+写侧尾读 union 化+双编码重算） | ✅done | Devin CLI 独立复审验收 2026-09-20；施工 e5290433+审计侧根因 C 修补（写侧尾读 union 化+to_regclass 探测回退——tx 内 42P01→25P02 毒化不可捕获）。产克隆 ant_verify_review 实测 T1-T9+存量 hash 测试 16 全绿（chain_break=0/hash_mismatch=4,605 精确基线）；mutation×4 RED→GREEN（M1 live-only 走查→1325 break 复活/M2 删规范化→mismatch 14409/M4 内存串回退→T8 红/M6 尾读回退→T2-T5/T9 复红）。**已部署 c74d8d3**，backend healthy |
...
- **下一步**: **R1 ✅done 已部署 `5db60c7f`+实盘探针闭环**——signal-mode 三缺陷已修+独立复审+mutation×4 实证+**实盘端到端验证**（demo 904d14e6 run `8bfe7ef0`：OrderSend 返真票号 394076399、同事件 OrderSelect 见注入仓位 openPrice=80365.17、OrderClose=true、平后 select=false 无幽灵仓位，positions=0）。VM 地基主动施工队列已空——剩余：R4/CloseBy·VM-LIVE-MTF-1（需求驱动）、FEAT-3（roadmap 需产品决策）、TRON 系（业主排除）。**LIVE-CLOSEALL-PENDING-1 ✅done**——close_all 挂单毒化批次已修+T10/M5 实证，live_dispatch.go 拆分为 actions 文件清偿 460 行超限，已部署 `799fae03` healthy。已完成链：KB-SEED-DRIFT-1（R2 闭环 venue 真值实盘抵达 VM）、TRADE-RECORDS-DUP-1/VERIFY-CHAIN-SEMANTIC-1 均 ✅done+已部署（产库 epoch=0、union 尾=arch seq 102092）。此前：demo `904d14e6` F1/F2/F3 复验 ✅done；VM-LIVE-PARITY-1 实盘对账完成。G-POST2/POST-2/LOWPRI-SWEEP/i18n/VM 债系均收官。
- **清扫上翻**: 无私有记忆需清扫。
