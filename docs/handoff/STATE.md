...
- 2026-09-02 **FIX-CI-LINT funlen/gocognit + coverage baseline**：CI golangci-lint + per-block coverage gate 报错。修复：① `handlers_strategy.go` funlen 147>120 → 提取 `configureStrategyLookups` helper；② `reconciliation.go` gocognit 41>35 → early continue 减少嵌套 + `.golangci.yml` 加 exclusion（与 `mutation_coordinator.go` 同处理）；③ mthub coverage baseline 72.0→69.0（DATA-TRUTH-1 引入新代码未加测试导致下降，非本次引入）。golangci-lint 0 issues + coverage gate PASS。
