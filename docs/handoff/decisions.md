...
### D-013 2026-09-16 最终决策者出件前强制自审（对称 D-012）

- **背景**: D-012 固化施工方自审后，业主要求对最终决策者同样强制：每个决策和施工提示词，必须先完成自我审计并修复，才允许出可转发开工指令。
- **决定**: ① 出任何决策类交付物（spec/adr/缺陷清单/decisions 条目/验收结论）或 builder-handoff 提示词前，决策方必须先完成决策自审：a) 代码事实回验——提示词中每个 file:line/字段/方法签名逐一对照当前源码核实，禁"或/待核对"歧义；b) 冲突检查——与 registry open 条目、已验收 ✅done 不变量、既有 ADR 逐一对照，不静默逆转；c) 复用核对——cap.sh + 邻近实现，不重复造基础设施；d) 模板完整性——S1–Sn 各含目标/坐标/落点/验证 + 对抗证明 + 边界/不做 + 验收标准。② 自审发现的问题必须先修复才可发派工指令；自审记录（核验项+修正项）随交付物落档。③ 未附自审记录的派工提示词视为未就绪，施工方有权 [转交决策]。
...
- **署名**: 最终决策：Devin CLI（[角色:决策终] 激活）
- **修订（同日 QS-1.3 实战补项）**: QS-1.3 派工单自审后仍连漏三处（黑名单误读白名单 / GlobalSlots 编译顺序污染洞 / for 循环域消亡回退），根因相同——只验"符号存在"未验"路径可达性"与"生命周期"。§3.1 检查项补两条硬要求：②路径可达性回验（条目在哪个表、谁消费、何阶段拦截、处方代码是否真可达）③生命周期推演（push/pop、消亡时机、读写方时序、编译顺序、跨事件）。三层拦截各拦一条证明机制有效，但出件端漏项成本最高，自此列为必查。
